...
들여쓰기 코드 블록 판정 시 list 컨텍스트를 구분한다. CommonMark/GFM
사양에서 4칸 들여쓰기는 두 가지 의미를 갖는다.

- **들여쓰기 코드 블록** — 빈 줄 다음에 오는 4칸 이상 들여쓰기 줄 묶음.
  이 경우만 펜스 코드 블록으로 변환한다.
- **list item 의 nested list 또는 continuation** — 직전에 list 항목
  (`-`, `*`, `+`, 또는 `1.` 같은 순서 목록 마커) 이 있고 현재 list block
  이 닫히지 않은 상태에서의 4칸 들여쓰기. 이는 sublist 또는 list 항목의
  paragraph continuation 이며 **변환 대상에서 제외**한다.

판정 알고리즘:

1. 줄 단위로 스캔하며 "현재 줄이 list block 안에 있는지" 상태를 추적한다.
   list 항목 (`-`, `*`, `+`, `\d+\.`, `\d+\)`) 이 등장하면 list block 시작
   또는 연속.
2. list block 은 비어 있지 않은 줄이 들여쓰기 없이 (또는 list 마커가 아닌
   다른 블록 요소로) 시작하기 전까지 유지된다. 빈 줄이 1개 이어도 다음 줄이
   같은 들여쓰기 깊이면 list 가 이어진다.
3. list block 안에서의 4칸 들여쓰기 줄은 nested list / continuation 으로
   판정하고 변환 대상에서 제외한다.
4. list block 밖에서의 (빈 줄 다음의) 4칸 들여쓰기 줄 묶음만 펜스로 감싼다.
...
### 데이터 URI placeholder 치환

해당 데이터는 번역 대상이 아니므로 (코드/식별자 보호 영역과 동일 성격)
청크 분할 이전에 본문에서 추출해 짧은 placeholder 로 치환한다.

치환 규칙:

- 대상 패턴: 마크다운 이미지 `![alt](URL)` 또는 링크 `[label](URL)` 의
  URL 이 `data:` 스킴이고 일정 길이 (예: 1000 자) 를 초과하는 경우.
- 치환 토큰: `__INLINE_DATA_URI_{n}__` 형태의 고유 식별자 (n 은 파일 내
  0 부터 증가). ASCII 만 사용해 LLM 이 임의로 변형하기 어렵게 한다.
- 매핑 테이블: `{토큰: 원본 URL}` 을 파일 단위로 보관해 번역 후 복원에
  사용한다.
- 코드 블록 안의 `data:` URI 는 치환하지 않는다 (코드 보호 영역 규칙으로
  이미 원문 그대로 보존됨).

수행 시점:

1. 원문 캐시 동기화 직후, **코드 블록 정규화 이후, 청크 분할 이전**.
2. LLM 호출 시 본문에는 짧은 placeholder 만 들어가 토큰 사용량이 크게
   감소한다.
3. 파일 단위 합본 직후 placeholder 를 원본 URL 로 되돌린다.

검증:

- 복원 단계에서 원본 placeholder 집합과 번역본 placeholder 집합이
  일치해야 한다. 누락·중복·변형은 Validation 분류로 처리해 재시도한다.
- 복원 후 본문에 `__INLINE_DATA_URI_` 패턴이 잔존하면 실패로 본다.

이 메커니즘은 분할 불가능한 큰 인라인 콘텐츠가 단 한 줄짜리 영역에서
토큰 한계를 초과하는 경우를 회복한다. 보호 영역 (펜스 코드 블록, 표)
본문 자체가 거대한 별도 시나리오에는 적용되지 않는다.
...
한 줄짜리 긴 문장이 들어간 청크는 토큰 한계를 초과할 수 있다.
...
강제 분할하지 않는다. 청크가 토큰 한계를 초과해 LLM 호출이 실패하면,
해당 파일은 실패 목록에 기록된다.
...
**순서 보존 (list 비교)** — 본문 구조와 위치가 의미를 갖는 항목:

- **Anchor 보존**: 원문 청크의 `<a name="..."></a>` 가 번역본에 동일한
  순서로 존재한다. 누락·추가·이동 모두 실패.
- **코드 블록 보존**: 펜스 코드 블록의 내용·등장 순서·위치가 byte-exact
  로 동일.

**multiset 보존 (Counter 비교)** — 토큰 자체의 보존이 본질이고 등장
순서는 한국어 어순에 따라 자유로운 항목:

- **인라인 코드 보존**: 원문의 모든 `` `code` `` 토큰이 번역본에 같은
  multiset 으로 등장한다. 각 토큰의 글자는 byte-exact, 등장 순서는
  한국어 어순에 따라 변경 허용. 토큰의 추가·삭제·변형·개수 변화는 실패.
- **URL 보존**: 마크다운 링크의 URL, autolink, reference link 정의의
  URL 이 같은 multiset 으로 등장한다 (등장 순서 변경 허용).

multiset 보존은 prompt.md 의 어순 변환 정책 (영어 SVO → 한국어 자연
어순) 과 일치한다. LLM 이 토큰을 변형하거나 누락하지는 않으면서 한국어
어순에 맞춰 토큰 위치만 옮긴 경우는 검증 통과한다.
