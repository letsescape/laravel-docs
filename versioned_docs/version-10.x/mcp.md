...
[Laravel MCP](https://github.com/laravel/mcp)는 AI 클라이언트가 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro)을 통해 Laravel 애플리케이션과 상호작용할 수 있는 단순하고 우아한 방법을 제공합니다. 애플리케이션과 AI 기반 상호작용을 가능하게 하는 서버, 툴, 리소스, 프롬프트를 정의하기 위한 표현력 있고 유창한 인터페이스를 제공합니다.
...
시작하려면 Composer 패키지 관리자를 사용해 프로젝트에 Laravel MCP를 설치합니다.
...
Laravel MCP를 설치한 뒤에는 MCP 서버를 정의할 `routes/ai.php` 파일을 게시하기 위해 `vendor:publish` Artisan 명령어를 실행합니다.
...
서버를 만들었다면 접근할 수 있도록 `routes/ai.php` 파일에 등록해야 합니다. Laravel MCP는 서버를 등록하는 두 가지 메서드를 제공합니다. HTTP로 접근 가능한 서버용 `web`과 명령줄 서버용 `local`입니다.
...
툴을 사용하면 서버가 AI 클라이언트가 호출할 수 있는 기능을 노출할 수 있습니다. 툴은 언어 모델이 작업을 수행하거나, 코드를 실행하거나, 외부 시스템과 상호작용할 수 있게 합니다.
...
기본적으로 툴의 이름과 제목은 클래스 이름에서 파생됩니다. 예를 들어 `CurrentWeatherTool`의 이름은 `current-weather`, 제목은 `Current Weather Tool`이 됩니다. 툴의 `$name`과 `$title` 프로퍼티를 정의해 이 값을 사용자 지정할 수 있습니다.
...
툴 설명은 자동으로 생성되지 않습니다. 툴에 `$description` 프로퍼티를 정의해 항상 의미 있는 설명을 제공해야 합니다.
...
> 설명은 AI 모델이 툴을 언제, 어떻게 효과적으로 사용해야 하는지 이해하도록 돕는 툴 메타데이터의 중요한 부분입니다.
...
툴은 AI 클라이언트에서 어떤 인수를 받을지 지정하기 위해 입력 스키마를 정의할 수 있습니다. Laravel의 `Illuminate\JsonSchema\JsonSchema` 빌더를 사용해 툴의 입력 요구 사항을 정의합니다.
...
JSON Schema 정의는 툴 인수의 기본 구조를 제공하지만, 더 복잡한 유효성 검증 규칙을 강제해야 할 수도 있습니다.
...
Laravel MCP는 Laravel의 [validation features](/docs/10.x/validation)과 매끄럽게 통합됩니다. 들어오는 툴 인수는 툴의 `handle` 메서드 안에서 검증할 수 있습니다.
...
Laravel [service container](/docs/10.x/container)는 모든 툴을 resolve하는 데 사용됩니다. 따라서 툴의 생성자에서 필요한 의존성을 타입 힌트할 수 있습니다. 선언된 의존성은 자동으로 resolve되어 툴 인스턴스에 주입됩니다.
...
생성자 주입 외에도 툴의 `handle()` 메서드에서 의존성을 타입 힌트할 수 있습니다. 메서드가 호출될 때 서비스 컨테이너가 의존성을 자동으로 resolve하고 주입합니다.
...
[annotations](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations)으로 툴을 보강해 AI 클라이언트에 추가 메타데이터를 제공할 수 있습니다. 이러한 어노테이션은 AI 모델이 툴의 동작과 기능을 이해하는 데 도움을 줍니다. 어노테이션은 attribute를 통해 툴에 추가합니다.
...
사용할 수 있는 어노테이션은 다음과 같습니다.

| 어노테이션         | 타입    | 설명                                                                                     |
| ------------------ | ------- | ----------------------------------------------------------------------------------------------- |
| `#[IsReadOnly]`    | boolean | 툴이 환경을 수정하지 않음을 나타냅니다.                                            |
| `#[IsDestructive]` | boolean | 툴이 파괴적인 업데이트를 수행할 수 있음을 나타냅니다(read-only가 아닐 때만 의미가 있습니다).       |
| `#[IsIdempotent]`  | boolean | 같은 인수로 반복 호출해도 추가 효과가 없음을 나타냅니다(read-only가 아닐 때).   |
...
툴 클래스에 `shouldRegister` 메서드를 구현해 런타임에 조건부로 툴을 등록할 수 있습니다. 이 메서드를 사용하면 애플리케이션 상태, 설정, 요청 파라미터를 기준으로 툴을 사용할 수 있어야 하는지 결정할 수 있습니다.
```php
<?php
...
툴은 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. Response 클래스는 여러 종류의 응답을 생성할 수 있는 편리한 메서드를 제공합니다.

<!-- For simple text responses, use the `text` method: -->
단순한 텍스트 응답에는 `text` 메서드를 사용합니다.
...
툴은 `Response` 인스턴스 배열을 반환하여 여러 콘텐츠 조각을 반환할 수 있습니다.
...
오래 실행되는 작업이나 실시간 데이터 스트리밍에서는 툴의 `handle` 메서드에서 [generator](https://www.php.net/manual/en/language.generators.overview.php)를 반환할 수 있습니다. 이를 통해 최종 응답 전에 중간 업데이트를 클라이언트에 보낼 수 있습니다.
...
웹 기반 서버를 사용할 때 스트리밍 응답은 자동으로 SSE(Server-Sent Events) 스트림을 열고, yield된 각 메시지를 이벤트로 클라이언트에 보냅니다.
...
프롬프트를 생성한 후 서버의 `$prompts` 프로퍼티에 등록합니다.
...
기본적으로 프롬프트의 이름과 제목은 클래스 이름에서 파생됩니다. 예를 들어 `DescribeWeatherPrompt`의 이름은 `describe-weather`, 제목은 `Describe Weather Prompt`가 됩니다. 프롬프트에 `$name`과 `$title` 프로퍼티를 정의하여 이 값을 직접 지정할 수 있습니다.
...
프롬프트 설명은 자동으로 생성되지 않습니다. 프롬프트에 `$description` 프로퍼티를 정의하여 항상 의미 있는 설명을 제공해야 합니다.
...
> 설명은 프롬프트 메타데이터의 중요한 부분입니다. AI 모델이 프롬프트를 언제, 어떻게 가장 잘 활용할 수 있는지 이해하도록 돕기 때문입니다.
...
프롬프트는 AI 클라이언트가 특정 값으로 프롬프트 템플릿을 커스터마이즈할 수 있도록 인수를 정의할 수 있습니다. 프롬프트가 허용하는 인수를 정의하려면 `arguments` 메서드를 사용합니다.
...
프롬프트 인수는 정의를 기준으로 자동 검증되지만, 더 복잡한 유효성 검증 규칙을 적용하고 싶을 수도 있습니다.
...
Laravel MCP는 Laravel의 [validation features](/docs/10.x/validation)과 매끄럽게 통합됩니다. 프롬프트의 `handle` 메서드 안에서 들어오는 프롬프트 인수를 검증할 수 있습니다.
...
<!-- On validation failure, AI clients will act based on the error messages you provide. As such, is critical to provide clear and actionable error messages: -->
유효성 검증에 실패하면 AI 클라이언트는 제공된 오류 메시지를 기준으로 동작합니다. 따라서 명확하고 실행 가능한 오류 메시지를 제공하는 것이 매우 중요합니다.
...
Laravel [service container](/docs/10.x/container)는 모든 프롬프트를 resolve하는 데 사용됩니다. 따라서 프롬프트 생성자에 필요한 의존성을 타입 힌트할 수 있습니다. 선언된 의존성은 자동으로 resolve되어 프롬프트 인스턴스에 주입됩니다.
...
생성자 주입뿐 아니라 프롬프트의 `handle` 메서드에서도 의존성을 타입 힌트할 수 있습니다. 메서드가 호출될 때 서비스 컨테이너가 의존성을 자동으로 resolve하고 주입합니다.
...
프롬프트 클래스에 `shouldRegister` 메서드를 구현하면 런타임에 프롬프트를 조건부로 등록할 수 있습니다. 이 메서드를 사용하면 애플리케이션 상태, 설정, 요청 파라미터를 기준으로 프롬프트를 사용할 수 있어야 하는지 결정할 수 있습니다.
...
프롬프트는 단일 `Laravel\Mcp\Response` 또는 `Laravel\Mcp\Response` 인스턴스의 iterable을 반환할 수 있습니다. 이 응답은 AI 클라이언트로 전송될 콘텐츠를 캡슐화합니다.
...
`asAssistant()` 메서드를 사용하면 응답 메시지를 AI 어시스턴트가 보낸 것으로 처리해야 함을 나타낼 수 있으며, 일반 메시지는 사용자 입력으로 처리됩니다.
...
[Resources](https://modelcontextprotocol.io/specification/2025-06-18/server/resources)를 사용하면 서버가 AI 클라이언트가 언어 모델과 상호작용할 때 읽고 컨텍스트로 사용할 수 있는 데이터와 콘텐츠를 노출할 수 있습니다. 리소스는 문서, 설정처럼 정적이거나 동적인 정보 또는 AI 응답에 도움이 되는 모든 데이터를 공유하는 방법을 제공합니다.
...
리소스를 만든 뒤에는 서버의 `$resources` 프로퍼티에 등록합니다.
...
> 설명은 리소스 메타데이터의 핵심 부분입니다. AI 모델이 리소스를 언제, 어떻게 효과적으로 사용할지 이해하는 데 도움이 됩니다.
...
기본적으로 리소스의 URI는 리소스 이름을 기준으로 생성됩니다. 따라서 `WeatherGuidelinesResource`의 URI는 `weather://resources/weather-guidelines`입니다. 기본 MIME 타입은 `text/plain`입니다.
...
툴과 프롬프트와 달리, 리소스는 입력 스키마나 인수를 정의할 수 없습니다. 다만 리소스의 `handle` 메서드 안에서 요청 객체와 상호작용할 수 있습니다.
...
Laravel [service container](/docs/10.x/container)는 모든 리소스를 해석하는 데 사용됩니다. 따라서 리소스가 필요로 하는 의존성을 생성자에 타입 힌트할 수 있습니다. 선언된 의존성은 자동으로 해석되어 리소스 인스턴스에 주입됩니다.
...
생성자 주입뿐 아니라 리소스의 `handle` 메서드에도 의존성을 타입 힌트할 수 있습니다. 메서드가 호출될 때 서비스 컨테이너가 의존성을 자동으로 해석하고 주입합니다.
...
리소스 클래스에 `shouldRegister` 메서드를 구현하면 런타임에 리소스를 조건부로 등록할 수 있습니다. 이 메서드를 사용하면 애플리케이션 상태, 설정, 요청 파라미터를 기준으로 리소스를 사용할 수 있어야 하는지 결정할 수 있습니다.
...
리소스는 반드시 `Laravel\Mcp\Response` 인스턴스를 반환해야 합니다. Response 클래스는 다양한 타입의 응답을 만들 수 있는 몇 가지 편리한 메서드를 제공합니다.

<!-- For simple text content, use the `text` method: -->
간단한 텍스트 콘텐츠에는 `text` 메서드를 사용합니다.
...
라우트와 마찬가지로 미들웨어를 사용하여 웹 MCP 서버를 인증할 수 있습니다. 이렇게 하면 사용자가 서버의 기능을 사용하기 전에 인증해야 합니다.
...
MCP 서버 접근을 인증하는 방법은 두 가지입니다. [Laravel Sanctum](/docs/10.x/sanctum)을 사용하는 간단한 토큰 기반 인증이나 `Authorization` HTTP 헤더로 전달되는 임의의 API 토큰을 사용할 수 있습니다. 또는 [Laravel Passport](/docs/10.x/passport)를 사용해 OAuth로 인증할 수 있습니다.
...
웹 기반 MCP 서버를 보호하는 가장 견고한 방법은 [Laravel Passport](/docs/10.x/passport)를 통한 OAuth입니다.
...
OAuth로 MCP 서버를 인증할 때는 `routes/ai.php` 파일에서 `Mcp::oauthRoutes` 메서드를 호출하여 필요한 OAuth2 discovery 및 클라이언트 등록 라우트를 등록합니다. 그런 다음 `routes/ai.php` 파일의 `Mcp::web` 라우트에 Passport의 `auth:api` 미들웨어를 적용합니다.
...
애플리케이션에서 아직 Laravel Passport를 사용하지 않는다면, 먼저 Passport의 [installation and deployment steps](/docs/10.x/passport#installation)를 따르십시오. 계속 진행하기 전에 `OAuthenticatable` 모델, 새 인증 가드, Passport 키가 준비되어 있어야 합니다.

<!-- Next, you should publish Laravel MCP's provided Passport authorization view: -->
다음으로 Laravel MCP가 제공하는 Passport 인가 뷰를 배포해야 합니다.
...
그런 다음 `Passport::authorizationView` 메서드를 사용하여 Passport가 이 뷰를 사용하도록 지정합니다. 일반적으로 이 메서드는 애플리케이션의 `AppServiceProvider`에 있는 `boot` 메서드에서 호출해야 합니다.
...
> 이 시나리오에서는 OAuth를 기반 인증 가능 모델로 이어지는 변환 계층으로만 사용합니다. 스코프와 같은 OAuth의 여러 측면은 무시합니다.
...
애플리케이션에서 이미 Laravel Passport를 사용하고 있다면, Laravel MCP는 기존 Passport 설치 안에서 원활하게 동작해야 합니다. 다만 OAuth는 주로 기반 인증 가능 모델로 이어지는 변환 계층으로 사용되므로, 현재 커스텀 스코프는 지원하지 않습니다.
...
OAuth2.1은 Model Context Protocol 명세에 문서화된 인증 메커니즘이며, MCP 클라이언트에서 가장 널리 지원됩니다. 이러한 이유로 가능하다면 Passport를 사용하는 것을 권장합니다.
...
애플리케이션에서 이미 [Sanctum](/docs/10.x/sanctum)을 사용하고 있다면 Passport를 추가하는 일이 번거로울 수 있습니다. 이 경우 OAuth만 지원하는 MCP 클라이언트를 사용해야 하는 명확하고 필요한 요구사항이 생기기 전까지는 Passport 없이 Sanctum을 사용하는 것을 권장합니다.
...
[Sanctum](/docs/10.x/sanctum)을 사용하여 MCP 서버를 보호하려면 `routes/ai.php` 파일에서 서버에 Sanctum 인증 미들웨어를 추가하면 됩니다. 그런 다음 인증이 성공하도록 MCP 클라이언트가 `Authorization: Bearer <token>` 헤더를 제공하는지 확인합니다.
...
애플리케이션이 자체 커스텀 API 토큰을 발급한다면, 원하는 미들웨어를 `Mcp::web` 라우트에 할당하여 MCP 서버를 인증할 수 있습니다. 커스텀 미들웨어는 `Authorization` 헤더를 직접 검사하여 들어오는 MCP 요청을 인증할 수 있습니다.
...
`$request->user()` 메서드로 현재 인증된 사용자에 접근할 수 있으므로, MCP 툴과 리소스 안에서 [authorization checks](/docs/10.x/authorization)을 수행할 수 있습니다.
...
[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector)는 MCP 서버를 테스트하고 디버깅하기 위한 대화형 도구입니다. 서버에 연결하고 인증을 확인하며 툴, 리소스, 프롬프트를 시험해 볼 때 사용합니다.

<!-- You may run the inspector for any registered server (for example, a local server named "weather"): -->
등록된 어떤 서버에 대해서도 Inspector를 실행할 수 있습니다. 예를 들어 "weather"라는 로컬 서버는 다음처럼 실행합니다.
...
이 명령어는 MCP Inspector를 실행하고, 모든 설정이 올바른지 확인할 수 있도록 MCP 클라이언트에 복사할 클라이언트 설정을 제공합니다. 웹 서버가 인증 미들웨어로 보호된다면 연결할 때 `Authorization` bearer 토큰 같은 필수 헤더를 포함해야 합니다.
...
응답을 받은 뒤에는 다양한 assertion 메서드로 응답의 콘텐츠와 상태를 확인할 수 있습니다.
...
`assertOk` 메서드를 사용하여 응답이 성공했는지 assertion할 수 있습니다. 이 메서드는 응답에 오류가 없는지 확인합니다.
...
`assertSee` 메서드를 사용하여 응답에 특정 텍스트가 포함되어 있는지 assertion할 수 있습니다.
...
`assertHasErrors` 메서드를 사용하여 응답에 오류가 포함되어 있는지 assertion할 수 있습니다.
...
`assertHasNoErrors` 메서드를 사용하여 응답에 오류가 포함되어 있지 않은지 assertion할 수 있습니다.
...
`assertName()`, `assertTitle()`, `assertDescription()` 메서드를 사용하여 응답에 특정 메타데이터가 포함되어 있는지 assertion할 수 있습니다.
...
`assertSentNotification` 및 `assertNotificationCount` 메서드를 사용하여 알림이 전송되었는지 assertion할 수 있습니다.
...
마지막으로 원시 응답 콘텐츠를 살펴보고 싶다면 `dd` 또는 `dump` 메서드를 사용하여 디버깅 목적으로 응답을 출력할 수 있습니다.
