...
이런 이유로, 반드시 **애플리케이션의 설정 파일(`config` 디렉터리 내부)에서만** `env` 함수를 호출해야 합니다. 라라벨 기본 설정 파일들을 보면 이런 방식을 따르고 있음을 확인할 수 있습니다. 설정 값은 언제든 [위에서 설명한](#accessing-configuration-values) `config` 함수를 통해 접근할 수 있습니다.
