...
<!-- Laravel Precognition allows you to anticipate the outcome of a future HTTP request. One of the primary use cases of Precognition is the ability to provide "live" validation for your frontend JavaScript application without having to duplicate your application's backend validation rules. Precognition pairs especially well with Laravel's Inertia-based [starter kits](/docs/10.x/starter-kits). -->
Laravel Precognition을 사용하면 앞으로 발생할 HTTP 요청의 결과를 미리 예측할 수 있습니다. Precognition의 주요 사용 사례 중 하나는 프론트엔드 JavaScript 애플리케이션에서 백엔드의 유효성 검증 규칙을 중복하지 않고도 "실시간" 유효성 검증 기능을 제공할 수 있다는 점입니다. Precognition은 특히 Laravel의 Inertia 기반 [starter kits](/docs/10.x/starter-kits)과 함께 사용할 때 매우 잘 어울립니다.

<!-- When Laravel receives a "precognitive request", it will execute all of the route's middleware and resolve the route's controller dependencies, including validating [form requests](/docs/10.x/validation#form-request-validation) - but it will not actually execute the route's controller method. -->
Laravel이 "프리코그니션 요청(precognitive request)"을 받으면 해당 라우트의 모든 미들웨어를 실행하고, 라우트의 컨트롤러 의존성도 모두 resolve하며, [form requests](/docs/10.x/validation#form-request-validation) 유효성 검증도 수행합니다. 단, 실제로 라우트의 컨트롤러 메서드를 실행하지는 않습니다.
...
<!-- First, to enable Precognition for a route, the `HandlePrecognitiveRequests` middleware should be added to the route definition. You should also create a [form request](/docs/10.x/validation#form-request-validation) to house the route's validation rules: -->
먼저, Precognition을 라우트에 적용하려면 `HandlePrecognitiveRequests` 미들웨어를 라우트 정의에 추가해야 합니다. 또한 라우트의 유효성 검증 규칙을 담을 [form request](/docs/10.x/validation#form-request-validation)도 만들어야 합니다.
...
> [!NOTE]
> Vue와 Inertia로 Laravel 애플리케이션을 개발할 계획이라면, [starter kits](/docs/10.x/starter-kits)을 활용해 빠르게 시작하는 것을 추천합니다. Laravel의 스타터 킷은 새로운 애플리케이션에서 백엔드와 프론트엔드 인증 기능을 기본적으로 제공해줍니다.
...
Vue와 Inertia에서 Precognition을 사용하기 전에, 먼저 [using Precognition with Vue](#using-vue) 내용을 확인하세요. Vue + Inertia 환경에서는 NPM을 통해 Inertia 호환 Precognition 라이브러리를 설치해야 합니다.
...
폼 헬퍼의 `submit` 메서드는 더 간단하게 설계되어, HTTP 메서드나 URL을 별도로 지정할 필요가 없습니다. 대신 Inertia의 [visit options](https://inertiajs.com/manual-visits)을 첫 번째이자 유일한 인자로 전달합니다. 또한, 위의 Vue 예제와 달리 해당 `submit` 메서드는 Promise를 반환하지 않습니다. 대신, `submit` 메서드에 전달하는 visit options에 Inertia가 지원하는 [event callbacks](https://inertiajs.com/manual-visits#event-callbacks)을 직접 지정하여 사용할 수 있습니다.
...
<!-- First, to enable Precognition for a route, the `HandlePrecognitiveRequests` middleware should be added to the route definition. You should also create a [form request](/docs/10.x/validation#form-request-validation) to house the route's validation rules: -->
먼저, Precognition을 라우트에 적용하려면 `HandlePrecognitiveRequests` 미들웨어를 라우트에 추가해야 하며, 라우트의 유효성 검증 규칙을 담을 [form request](/docs/10.x/validation#form-request-validation)를 만들어야 합니다.
...
> [!NOTE]
> React와 Inertia로 Laravel 애플리케이션을 개발하고 싶다면, [starter kits](/docs/10.x/starter-kits)을 사용해 빠르게 시작하는 것을 권장합니다. Laravel의 스타터 킷은 프론트엔드/백엔드 인증 스캐폴딩을 기본으로 제공합니다.
...
Precognition을 React와 Inertia 조합에서 사용하기 전엔 [using Precognition with React](#using-react)를 반드시 참고하세요. React + Inertia 환경에서는 NPM을 통해 Inertia 전용 Precognition 라이브러리를 설치해야 합니다.
...
폼 헬퍼의 `submit` 메서드는 더 간단하게 설계되어, HTTP 메서드나 URL을 별도로 지정할 필요가 없습니다. 대신 Inertia의 [visit options](https://inertiajs.com/manual-visits)을 첫 번째이자 유일한 인자로 전달합니다. 또한, 위의 React 예제와 달리 해당 `submit` 메서드는 Promise를 반환하지 않습니다. 대신, `submit` 메서드에 전달하는 visit options에 Inertia가 지원하는 [event callbacks](https://inertiajs.com/manual-visits#event-callbacks)을 직접 지정해 사용할 수 있습니다.
...
<!-- First, to enable Precognition for a route, the `HandlePrecognitiveRequests` middleware should be added to the route definition. You should also create a [form request](/docs/10.x/validation#form-request-validation) to house the route's validation rules: -->
먼저 Precognition을 라우트에 적용하려면 `HandlePrecognitiveRequests` 미들웨어를 추가해야 하며, 라우트의 유효성 검증 규칙을 포함할 [form request](/docs/10.x/validation#form-request-validation)도 작성해야 합니다.
...
실시간 유효성 검증을 위해서는 폼의 데이터를 각 입력값에 바인딩하고, 각 입력 필드의 `change` 이벤트를 감지해야 합니다. `change` 이벤트 핸들러에서 폼의 `validate` 메서드를 호출하고, 인풋의 이름을 함께 넘깁니다.
...
이러한 기본 동작을 고려해, [customizes the corresponding form request's validation rules](#customizing-validation-rules)하여 실제 폼 최종 제출에서만 해당 필드가 필수임을 지정해야 합니다.
