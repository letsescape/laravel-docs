...
<!-- To obtain an instance of the current HTTP request via dependency injection, you should type-hint the `Illuminate\Http\Request` class on your route closure or controller method. The incoming request instance will automatically be injected by the Laravel [service container](/docs/10.x/container): -->
현재 HTTP 요청 인스턴스를 의존성 주입을 통해 받으려면, 라우트 클로저 또는 컨트롤러 메서드에서 `Illuminate\Http\Request` 클래스를 타입 힌트로 지정하면 됩니다. 들어오는 요청 인스턴스는 Laravel [service container](/docs/10.x/container)에 의해 자동으로 주입됩니다.
...
<!-- Using the `routeIs` method, you may determine if the incoming request has matched a [named route](/docs/10.x/routing#named-routes): -->
`routeIs` 메서드를 사용하면, 요청이 [named route](/docs/10.x/routing#named-routes)에 매칭되는지 확인할 수 있습니다.
...
`Illuminate\Http\Request` 인스턴스의 `header` 메서드를 이용하여 요청 헤더 값을 가져올 수 있습니다. 해당하는 헤더가 없으면 `null`이 반환됩니다. 단, `header` 메서드는 선택적인 두 번째 인자를 받아, 헤더가 요청에 없을 때 해당 값을 반환합니다.
...
[PSR-7 standard](https://www.php-fig.org/psr/psr-7/)은 HTTP 메시지(요청 및 응답)를 위한 인터페이스를 정의합니다. Laravel 요청 대신 PSR-7 요청 인스턴스를 받고 싶다면, 먼저 몇 가지 라이브러리를 설치해야 합니다. Laravel은 *Symfony HTTP Message Bridge* 컴포넌트를 이용해 일반적인 요청 및 응답을 PSR-7 호환 구현체로 변환합니다.
...
> [!NOTE]
> 라우트나 컨트롤러에서 PSR-7 응답 인스턴스를 반환하는 경우, 프레임워크에 의해 자동으로 다시 Laravel 응답 인스턴스로 변환되어 화면에 표시됩니다.
...
<!-- Using the `collect` method, you may retrieve all of the incoming request's input data as a [collection](/docs/10.x/collections): -->
`collect` 메서드를 사용하면 요청의 모든 입력값을 [collection](/docs/10.x/collections)으로도 가져올 수 있습니다.
...
`input` 메서드의 두 번째 인자로 기본값을 전달할 수 있습니다. 요청에 해당 입력값이 없을 때 이 값이 반환됩니다.
...
<!-- Instead of retrieving the request's input data as a primitive `string`, you may use the `string` method to retrieve the request data as an instance of [`Illuminate\Support\Stringable`](/docs/10.x/helpers#fluent-strings): -->
...
체크박스 같은 HTML 요소를 처리할 때, 애플리케이션은 실제로는 문자열인 "truthy"(참 같은) 값을 받을 수 있습니다. 예를 들어 "true" 또는 "on"이 그렇습니다. 편의를 위해 `boolean` 메서드를 사용하면 이러한 값을 불리언으로 가져올 수 있습니다. `boolean` 메서드는 1, "1", true, "true", "on", "yes"에 대해 `true`를 반환하며, 그 외의 모든 값에 대해서는 `false`를 반환합니다.
...
날짜/시간 값을 입력받을 때 편리하게 `date` 메서드로 입력값을 Carbon 인스턴스로 변환해 가져올 수 있습니다. 값이 없으면 `null`이 반환됩니다.
...
`date` 메서드가 받는 두 번째, 세 번째 인자로 각각 날짜의 포맷과 타임존을 지정할 수도 있습니다.
...
입력값이 존재하지만 포맷이 올바르지 않다면 `InvalidArgumentException`이 발생하니, `date` 메서드를 호출하기 전에 입력값을 검증하는 것이 좋습니다.
...
[PHP enums](https://www.php.net/manual/en/language.types.enumerations.php)과 연결되는 입력값도 요청에서 받아올 수 있습니다. 해당 이름의 입력값이 없거나 enum의 값과 일치하는 백킹 값이 없다면 `null`이 반환됩니다. `enum` 메서드는 입력값의 이름과 enum 클래스를 첫 번째, 두 번째 인자로 받습니다.
...
입력값 중 일부만 가져오고 싶다면 `only`와 `except` 메서드를 사용할 수 있습니다. 두 메서드는 하나의 `array` 또는 동적으로 나열한 여러 인자를 받을 수 있습니다.
...
입력값이 요청에 포함되어 있는지 확인하려면 `has` 메서드를 사용하세요. `has` 메서드는 값이 요청에 있으면 `true`를 반환합니다.
...
배열을 전달하면 `has` 메서드는 지정된 모든 값이 요청에 있는지 확인합니다.
...
가끔, 요청의 기존 입력값에 추가로 값을 직접 병합하고 싶을 때가 있습니다. 이럴 때는 `merge` 메서드를 사용하면 됩니다. 이미 요청에 존재하는 입력 키라면 `merge` 메서드에 전달한 데이터로 덮어씁니다.
...
<!-- Laravel allows you to keep input from one request during the next request. This feature is particularly useful for re-populating forms after detecting validation errors. However, if you are using Laravel's included [validation features](/docs/10.x/validation), it is possible that you will not need to manually use these session input flashing methods directly, as some of Laravel's built-in validation facilities will call them automatically. -->
Laravel에서는 이전 요청의 입력값을 다음 요청까지 유지할 수 있습니다. 이 기능은 주로 유효성 검증 후 폼 입력값을 다시 채우고 싶을 때 매우 유용합니다. 다만, Laravel의 [validation features](/docs/10.x/validation) 기능을 사용한다면, 해당 기능이 내부적으로 이 입력값 저장(플래시) 과정을 자동으로 처리하므로 직접 호출할 필요가 없을 수 있습니다.

<a name="flashing-input-to-the-session"></a>
<!-- #### Flashing Input to the Session -->
#### Flashing Input to the Session

<!-- The `flash` method on the `Illuminate\Http\Request` class will flash the current input to the [session](/docs/10.x/session) so that it is available during the user's next request to the application: -->
`Illuminate\Http\Request` 클래스의 `flash` 메서드는 현재 입력값을 [session](/docs/10.x/session)에 저장(플래시)하여 다음 요청에서도 사용할 수 있게 해줍니다.
...
이전 요청에서 플래시된 입력값을 가져오려면, `Illuminate\Http\Request` 인스턴스의 `old` 메서드를 호출하세요. `old` 메서드는 [session](/docs/10.x/session)에 저장된 이전 입력값을 반환합니다.
...
또한 Laravel에서는 전역 `old` 헬퍼도 제공됩니다. [Blade template](/docs/10.x/blade)에서 폼에 이전 값을 다시 채울 때는 `old` 헬퍼를 사용하는 것이 더 편리합니다. 만약 해당 필드에 이전 값이 없으면 `null`이 반환됩니다.
...
Laravel에서는 기본적으로 `App\Http\Middleware\TrimStrings`와 `Illuminate\Foundation\Http\Middleware\ConvertEmptyStringsToNull` 미들웨어를 전체 글로벌 미들웨어 스택에 포함시킵니다. 이 미들웨어들은 `App\Http\Kernel` 클래스에 의해 글로벌 미들웨어 스택에 등록됩니다. 즉, 이 미들웨어들은 들어오는 모든 요청의 문자열 필드를 자동으로 trim(양쪽 공백 제거)하고, 빈 문자열을 `null`로 변환해줍니다. 따라서 라우트나 컨트롤러에서 이 부분을 신경 쓸 필요가 없습니다.
...
특정 요청에 대해서만 문자열 트리밍 또는 빈 문자열 변환을 비활성화하고 싶다면, 각 미들웨어가 제공하는 `skipWhen` 메서드를 사용할 수 있습니다. 이 메서드는 입력값 정규화를 건너뛸지(`true`/`false`)를 나타내는 값을 반환하는 클로저를 인자로 받습니다. 일반적으로 `skipWhen` 메서드는 애플리케이션의 `AppServiceProvider`의 `boot` 메서드 내부에서 호출합니다.
...
`UploadedFile` 인스턴스에는 이 외에도 다양한 메서드가 있습니다. 보다 자세한 정보는 [API documentation for the class](https://github.com/symfony/symfony/blob/6.0/src/Symfony/Component/HttpFoundation/File/UploadedFile.php)를 참조하세요.

<a name="storing-uploaded-files"></a>
<!-- ### Storing Uploaded Files -->
### Storing Uploaded Files

<!-- To store an uploaded file, you will typically use one of your configured [filesystems](/docs/10.x/filesystem). The `UploadedFile` class has a `store` method that will move an uploaded file to one of your disks, which may be a location on your local filesystem or a cloud storage location like Amazon S3. -->
업로드된 파일을 저장하려면, 보통 미리 설정해둔 [filesystems](/docs/10.x/filesystem)을 사용하게 됩니다. `UploadedFile` 클래스의 `store` 메서드는 업로드된 파일을 로컬 디스크 또는 Amazon S3와 같은 클라우드 저장소에 옮길 수 있습니다.
...
또한 `store` 메서드는 파일을 저장할 디스크 이름을 선택적인 두 번째 인자로 받을 수도 있습니다. 이 메서드는 디스크의 루트 기준 저장 경로를 반환합니다.
...
> [!NOTE]
> Laravel 파일 저장에 관해 더 많은 정보가 필요하다면, [file storage documentation](/docs/10.x/filesystem)를 참고하세요.
...
이런 경우에는 Laravel 애플리케이션에 포함된 `App\Http\Middleware\TrustProxies` 미들웨어를 설정하세요. 이 미들웨어는 애플리케이션에서 신뢰할 로드 밸런서 또는 프록시를 간편하게 지정할 수 있게 해줍니다. 신뢰할 프록시는 이 미들웨어의 `$proxies` 속성에 배열로 지정하며, 신뢰할 프록시 `$headers`도 별도로 지정할 수 있습니다.
...
> [!NOTE]
> AWS Elastic Load Balancing을 사용한다면, `$headers` 값은 `Request::HEADER_X_FORWARDED_AWS_ELB`여야 합니다. `$headers` 속성에 사용 가능한 상수에 대한 자세한 내용은 Symfony의 [trusting proxies](https://symfony.com/doc/current/deployment/proxies.html)를 참고하세요.
