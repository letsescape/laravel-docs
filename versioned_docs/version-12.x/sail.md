...
Sail을 설치할 때 [Meilisearch](https://www.meilisearch.com) 서비스를 설치하도록 선택했다면, 애플리케이션의 `compose.yaml` 파일에는 [Laravel Scout](/docs/12.x/scout)와 통합되는 강력한 검색 엔진 항목이 포함됩니다. 컨테이너를 시작한 뒤에는 `MEILISEARCH_HOST` 환경 변수를 `http://meilisearch:7700`로 설정하여 애플리케이션 안에서 Meilisearch 인스턴스에 연결할 수 있습니다.
...
Sail을 설치할 때 [Typesense](https://typesense.org) 서비스를 설치하도록 선택했다면, 애플리케이션의 `compose.yaml` 파일에는 [Laravel Scout](/docs/12.x/scout#typesense)와 네이티브로 통합되는 매우 빠른 오픈 소스 검색 엔진 항목이 포함됩니다. 컨테이너를 시작한 뒤에는 다음 환경 변수를 설정하여 애플리케이션 안에서 Typesense 인스턴스에 연결할 수 있습니다.
...
Laravel은 기본적으로 훌륭한 테스트 지원을 제공하며, Sail의 `test` 명령어를 사용하여 애플리케이션의 [feature and unit tests](/docs/12.x/testing)를 실행할 수 있습니다. Pest / PHPUnit이 허용하는 모든 CLI 옵션도 `test` 명령어에 전달할 수 있습니다.
...
[Laravel Dusk](/docs/12.x/dusk)는 표현력이 좋고 사용하기 쉬운 브라우저 자동화 및 테스트 API를 제공합니다. Sail 덕분에 로컬 컴퓨터에 Selenium이나 다른 도구를 설치하지 않고도 이러한 테스트를 실행할 수 있습니다. 시작하려면 애플리케이션의 `compose.yaml` 파일에서 Selenium 서비스의 주석을 해제하십시오.
...
> [!NOTE]
> `share` 명령어는 [Expose](https://github.com/beyondcode/expose)를 기반으로 동작하며, 이는 [BeyondCode](https://beyondco.de)에서 만든 오픈 소스 터널링 서비스입니다.
