...
현재 애플리케이션 환경은 `.env` 파일의 `APP_ENV` 변수로 결정됩니다. 이 값은 `App` [facade](/docs/12.x/facades)의 `environment` 메서드를 통해 얻을 수 있습니다.
...
> [!NOTE]
> 제공하는 키의 길이는 사용되는 암호화 알고리즘의 키 길이와 일치해야 합니다. 기본적으로 Laravel은 32자 키가 필요한 `AES-256-CBC` 알고리즘을 사용합니다. 명령어 실행 시 `--cipher` 옵션을 넘기면 Laravel의 [encrypter](/docs/12.x/encryption)가 지원하는 다른 알고리즘도 사용할 수 있습니다.
...
애플리케이션이 유지보수 모드일 때는 [queued jobs](/docs/12.x/queues)이 수행되지 않습니다. 유지보수 모드를 해제하면 큐 작업은 다시 정상적으로 처리됩니다.
