---
title: 캐시 (Cache) - Laravel 12.x 한국어 문서
description: '애플리케이션에서 데이터를 가져오거나 처리하는 작업 중에는 CPU를 많이 사용하거나 완료하는 데 몇 초가 걸리는 경우도 있습니다. 이런 상황에서 동일한 데이터에 대한 이후 요청마다 빠르게 응답하기 위해, 가져온 데이터를 일정 시간 동안 캐싱(임시 저장)하는 것이 일반적입니다.'
keywords: [라라벨 캐시, Laravel cache, Laravel 12.x]
---
# 캐시 (Cache)
