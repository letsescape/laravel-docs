...
OAuth provider에서 사용자 정보를 받아오면, 해당 사용자가 애플리케이션 데이터베이스에 존재하는지 확인한 뒤 [authenticate the user](/docs/12.x/authentication#authenticate-a-user-instance)을 진행할 수 있습니다. 데이터베이스에 사용자가 없다면 일반적으로 새 사용자 레코드를 생성하게 됩니다.
...
봇 토큰을 생성할 때도 `user` 메서드는 `Laravel\Socialite\Two\User` 인스턴스를 반환하지만, 이때는 오직 `token` 속성만 채워져 있습니다. 이 토큰은 [send notifications to the authenticated user's Slack workspaces](/docs/12.x/notifications#notifying-external-slack-workspaces)에 활용할 수 있도록 저장해둘 수 있습니다.
