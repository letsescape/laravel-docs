...
Laravel은 다양한 전역 "helper" PHP 함수를 제공합니다. 이러한 함수 중 많은 것은 프레임워크 내부에서 사용되지만, 편리하다면 애플리케이션에서도 자유롭게 사용할 수 있습니다.
...
`Arr::add` 메서드는 지정한 키가 배열에 존재하지 않거나 `null`일 때만 해당 키/값 쌍을 배열에 추가합니다.
...
`Arr::array` 메서드는 "dot" 표기법을 사용해 깊이 중첩된 배열에서 값을 가져옵니다([Arr::get()](#method-array-get)과 동일한 방식). 단, 요청한 값이 `array` 타입이 아니면 `InvalidArgumentException` 예외가 발생합니다.
...
`Arr::boolean` 메서드는 "dot" 표기법을 사용하여 깊이 중첩된 배열에서 값을 가져옵니다. 요청한 값이 `boolean` 타입이 아닐 경우 `InvalidArgumentException` 예외가 발생합니다.
...
(문서가 매우 길기 때문에 한 번에 모두 출력하면 메시지 제한을 초과합니다. 필요하시면 **나머지 섹션도 동일한 형식으로 이어서 전체 번역을 계속 제공하겠습니다.**)