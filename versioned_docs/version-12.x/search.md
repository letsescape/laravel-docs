...
인덱스 생성에 대한 더 자세한 정보는 [migration documentation](/docs/12.x/migrations#available-index-types)를 참고하십시오.
...
`orWhereFullText` 메서드를 사용하면 전문 검색 조건을 "or" 조건으로 추가할 수도 있습니다. 자세한 내용은 [query builder documentation](/docs/12.x/queries#full-text-where-clauses)를 참고하세요.
...
임베딩 provider 설정, 임베딩 차원 수 사용자 지정, 캐싱 등은 [AI SDK documentation](/docs/12.x/ai-sdk#embeddings)를 참고하세요.
...
벡터 컬럼과 인덱스에 대한 자세한 내용은 [migration documentation](/docs/12.x/migrations#available-column-types)를 참고하세요.
...
벡터 쿼리를 더 세밀하게 제어하고 싶다면 `whereVectorDistanceLessThan`, `selectVectorDistance`, `orderByVectorDistance` 메서드도 제공됩니다. 이들은 유사도(score) 대신 거리 값으로 작업해서, 거리 결과를 컬럼에 포함하거나 수동 정렬이 필요할 때 사용할 수 있습니다. 자세한 내용은 [query builder documentation](/docs/12.x/queries#vector-similarity-clauses) 및 [AI SDK documentation](/docs/12.x/ai-sdk#querying-embeddings)를 참고하세요.
...
리랭킹 provider 설정 및 옵션 등 자세한 내용은 [AI SDK documentation](/docs/12.x/ai-sdk#reranking)를 참고하세요.
...
> [!WARNING]
> 특정 컬럼을 전문 쿼리로 지정하려면, 해당 컬럼에 [full-text index](/docs/12.x/migrations#available-index-types)가 반드시 추가되어 있어야 합니다.
...
데이터베이스 엔진은 일반적인 범위의 검색 요구에 적합하며, Scout의 인덱스 자동 동기화 편리함을 누리고자 할 때 적합합니다. 필터링, 페이지네이션, 소프트 삭제 레코드 처리 등 주요 검색 기능도 잘 지원합니다. 자세한 내용은 [Scout documentation](/docs/12.x/scout#database-engine)를 참고하세요.
...
서드파티 엔진 설정 등 자세한 내용은 [Scout documentation](/docs/12.x/scout)를 참고하십시오.
