...
Laravel MCP는 Laravel의 [유효성 검사 기능](/docs/12.x/validation)과 원활하게 통합됩니다. Tool의 `handle` 메서드 내에서 들어오는 Tool 인수를 유효성 검사할 수 있습니다.
...
모든 Tool은 Laravel [서비스 컨테이너](/docs/12.x/container)를 통해 리졸브됩니다. 따라서 Tool의 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다. 선언된 의존성은 자동으로 리졸브되어 Tool 인스턴스에 주입됩니다.
...
Laravel MCP는 Laravel의 [유효성 검사 기능](/docs/12.x/validation)과 원활하게 통합됩니다. Prompt의 `handle` 메서드 내에서 들어오는 Prompt 인수를 유효성 검사할 수 있습니다.
...
모든 Prompt는 Laravel [서비스 컨테이너](/docs/12.x/container)를 통해 리졸브됩니다. 따라서 Prompt의 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다. 선언된 의존성은 자동으로 리졸브되어 Prompt 인스턴스에 주입됩니다.
...
모든 Resource는 Laravel [서비스 컨테이너](/docs/12.x/container)를 통해 리졸브됩니다. 따라서 Resource의 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다. 선언된 의존성은 자동으로 리졸브되어 Resource 인스턴스에 주입됩니다.
...
애플리케이션에서 아직 Laravel Passport를 사용하지 않는 경우, Passport의 [설치 및 배포 가이드](/docs/12.x/passport#installation)를 따라 애플리케이션에 Passport를 추가하세요. 진행하기 전에 `OAuthenticatable` 모델, 새로운 인증 가드, Passport 키가 준비되어 있어야 합니다.
...
`$request->user()` 메서드를 통해 현재 인증된 사용자에 접근하여 MCP Tool과 Resource 내에서 [인가 확인](/docs/12.x/authorization)을 수행할 수 있습니다.
