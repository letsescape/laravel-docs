...
HTTP/3, 최신 압축, [Laravel Octane](/docs/12.x/octane) 연동 등 FrankenPHP가 지원하는 고급 기능이나, Laravel 애플리케이션을 독립 실행형 바이너리로 패키징하는 것과 같은 기능을 활용하려면 FrankenPHP의 [Laravel documentation](https://frankenphp.dev/docs/laravel/)를 참고하십시오.
...
이 라우트에 HTTP 요청이 오면, Laravel은 또한 `Illuminate\Foundation\Events\DiagnosingHealth` 이벤트를 디스패치합니다. 이를 통해 애플리케이션에서 추가적인 상태 점검을 할 수 있습니다. 이 이벤트의 [listener](/docs/12.x/events) 내부에서 데이터베이스나 캐시 상태를 점검할 수 있으며, 문제가 감지되었다면 리스너에서 예외를 던지면 됩니다.
