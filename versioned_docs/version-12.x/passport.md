...
> [!NOTE]
> 이 문서는 사용자가 이미 OAuth2에 익숙하다고 가정합니다. OAuth2에 대해 전혀 모른다면 계속 진행하기 전에 OAuth2의 일반적인 [terminology](https://oauth2.thephpleague.com/terminology/)와 기능을 먼저 익히는 것을 권장합니다.
...
> [!WARNING]
> Passport 데이터베이스 테이블의 `expires_at` 컬럼은 읽기 전용이며 표시 목적으로만 사용됩니다. 토큰을 발급할 때 Passport는 만료 정보를 서명 및 암호화된 토큰 안에 저장합니다. 토큰을 무효화해야 한다면 [revoke it](#revoking-tokens)해야 합니다.
...
그런 다음 Passport의 [its routes file](https://github.com/laravel/passport/blob/master/routes/web.php)에 정의된 라우트를 애플리케이션의 `routes/web.php` 파일로 복사하고 원하는 대로 수정할 수 있습니다.
...
퍼스트 파티 클라이언트를 인가하는 경우처럼 인가 프롬프트를 건너뛰고 싶을 때가 있습니다. 이 작업은 [extending the `Client` model](#overriding-default-models)하고 `skipsAuthorization` 메서드를 정의하여 수행할 수 있습니다. `skipsAuthorization`가 `true`를 반환하면, 사용하는 애플리케이션이 인가를 위해 리디렉션할 때 `prompt` 파라미터를 명시적으로 설정하지 않은 한 클라이언트가 승인되고 사용자는 즉시 `redirect_uri`로 다시 리디렉션됩니다.
...
애플리케이션의 `routes/console.php` 파일에서 [scheduled job](/docs/12.x/scheduling)을 설정하여 일정에 따라 토큰을 자동으로 정리할 수도 있습니다.
...
```
> [!NOTE]
> access token은 기본적으로 수명이 깁니다. 하지만 필요하다면 [configure your maximum access token lifetime](#configuration)을 자유롭게 설정할 수 있습니다.
...
애플리케이션에서 둘 이상의 [authentication user provider](/docs/12.x/authentication#introduction)를 사용하는 경우, `artisan passport:client --password` 명령어로 client를 생성할 때 `--provider` 옵션을 제공하여 password grant client가 사용할 user provider를 지정할 수 있습니다. 제공하는 provider 이름은 애플리케이션의 `config/auth.php` 설정 파일에 정의된 유효한 provider와 일치해야 합니다. 그런 다음 [protect your route using middleware](#multiple-authentication-guards)하면 해당 guard에 지정된 provider의 사용자만 인가되도록 할 수 있습니다.
...
> [!WARNING]
> 이제 implicit grant token 사용은 권장하지 않습니다. 대신 [a grant type that is currently recommended by OAuth2 Server](https://oauth2.thephpleague.com/authorization-server/which-grant/)을 선택해야 합니다.
...
애플리케이션에서 둘 이상의 [authentication user provider](/docs/12.x/authentication#introduction)를 사용하는 경우, `artisan passport:client --personal` 명령어로 client를 생성할 때 `--provider` 옵션을 제공하여 personal access grant client가 사용할 user provider를 지정할 수 있습니다. 제공하는 provider 이름은 애플리케이션의 `config/auth.php` 설정 파일에 정의된 유효한 provider와 일치해야 합니다. 그런 다음 [protect your route using middleware](#multiple-authentication-guards)하면 해당 guard에 지정된 provider의 사용자만 인가되도록 할 수 있습니다.
...
personal access client를 생성한 뒤에는 `App\Models\User` model 인스턴스의 `createToken` 메서드를 사용하여 특정 사용자에게 token을 발급할 수 있습니다. `createToken` 메서드는 첫 번째 인수로 token의 이름을 받고, 두 번째 인수로 선택 사항인 [scopes](#token-scopes) 배열을 받습니다.
...
> [!WARNING]
> [client credentials grant](#client-credentials-grant)를 사용하는 경우, route를 보호할 때 `auth:api` middleware 대신 [the `Laravel\Passport\Http\Middleware\EnsureClientIsResourceOwner` middleware](#client-credentials-grant)를 사용해야 합니다.
...
> [!NOTE]
> Passport에서 여러 user provider를 사용하는 방법에 대한 자세한 내용은 [personal access tokens documentation](#customizing-the-user-provider-for-pat)와 [password grant documentation](#customizing-the-user-provider)를 참고하십시오.
...
Passport는 액세스 토큰과 리프레시 토큰을 발급할 때 이벤트를 발생시킵니다. 데이터베이스의 다른 액세스 토큰을 정리하거나 폐기하기 위해 [listen for these events](/docs/12.x/events)할 수 있습니다.
