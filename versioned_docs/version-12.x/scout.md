...
서드파티 엔진 사용 시에는 이 콜백이 이미 검색 엔진에서 모델을 받아온 후에 실행되므로, "필터링" 용도로 사용해서는 안 됩니다. 대신 [Scout where 절](#where-clauses)을 사용하세요. 하지만 데이터베이스 엔진 사용 시에는 `query` 메서드의 제약이 바로 데이터베이스 쿼리에 반영되므로 필터링에도 활용할 수 있습니다.
