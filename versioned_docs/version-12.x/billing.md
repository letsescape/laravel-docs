...
또한 여러 제품의 구독에 대한 항목 세율도 동기화됩니다. 애플리케이션이 여러 제품에 대한 구독을 제공하는 경우 청구 가능한 모델이 [위에서 설명한](#subscription-taxes) `priceTaxRates` 메서드를 구현하는지 확인해야 합니다.
...
> [!WARNING]
> 청구 가능한 모델 클래스 정의 내에서 `trial_ends_at` 속성에 대해 [날짜 캐스트](/docs/12.x/eloquent-mutators#date-casting)를 추가해야 합니다.
...
Stripe 웹훅은 Laravel의 [CSRF 보호](/docs/12.x/csrf)를 우회해야 하므로 Laravel가 수신 Stripe 웹훅에 대한 CSRF 토큰의 유효성을 검사하려고 시도하지 않도록 해야 합니다. 이를 달성하려면 애플리케이션의 `bootstrap/app.php` 파일의 CSRF 보호에서 `stripe/*`를 제외해야 합니다.
...
두 이벤트 모두 Stripe 웹훅의 전체 페이로드를 포함합니다. 예를 들어, `invoice.payment_succeeded` 웹훅을 처리하려는 경우 이벤트를 처리할 [리스너](/docs/12.x/events#defining-listeners)를 등록할 수 있습니다.
