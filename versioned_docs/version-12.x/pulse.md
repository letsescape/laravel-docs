...
> [!NOTE]
> 인증된 사용자를 캡처하고 조회하는 방법을 완전히 사용자 지정하려면 `Laravel\Pulse\Contracts\ResolvesUsers` 계약을 구현하고, Laravel의 [service container](/docs/12.x/container#binding-a-singleton)에 바인딩할 수 있습니다.
...
`<livewire:pulse.slow-outgoing-requests />` 카드는 Laravel의 [HTTP client](/docs/12.x/http-client)로 발생시킨 외부 요청 중 설정된 임계값(기본값 1,000ms)을 초과한 요청을 보여줍니다.
...
> [!NOTE]
> Pulse는 [cache](/docs/12.x/cache)를 사용해 재시작 신호를 저장하므로, 이 기능을 사용하기 전에 애플리케이션에 캐시 드라이버가 제대로 구성되어 있는지 확인하세요.
...
`CacheInteractions` 레코더는 [cache](/docs/12.x/cache)의 적중/실패 정보를 [Cache](#cache-card) 카드에 표시하기 위해 수집합니다.
...
`SlowOutgoingRequests` 레코더는 Laravel의 [HTTP client](/docs/12.x/http-client)를 사용해 발생한 외부 요청 중 임계값을 초과한 요청 정보를 [Slow Outgoing Requests](#slow-outgoing-requests-card) 카드에 표시합니다.
...
Pulse에서 사용하는 [database connection](/docs/12.x/database#configuration)은 `PULSE_DB_CONNECTION` 환경 변수를 통해 지정할 수 있습니다.
...
Pulse는 기본적으로 기본 [Redis connection](/docs/12.x/redis#configuration)을 사용하지만, `PULSE_REDIS_CONNECTION` 환경 변수로 따로 지정할 수 있습니다:
...
> [!NOTE]
> Pulse는 [cache](/docs/12.x/cache)를 이용해 재시작 신호를 저장하므로, 이 기능을 사용하기 전에 캐시 드라이버가 애플리케이션에 제대로 구성되어 있는지 확인하세요.
...
커스텀 카드가 애플리케이션 코드베이스 내에 있고, Laravel [Vite integration](/docs/12.x/vite)을 사용한다면, `vite.config.js` 파일에서 카드용 CSS 엔트리포인트를 추가할 수 있습니다:
