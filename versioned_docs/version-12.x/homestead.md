...
> [!WARNING]
> Laravel Homestead는 더 이상 적극적으로 유지 관리되지 않는 레거시 패키지입니다. [Laravel 항해](/docs/12.x/sail)는 현대적인 대안으로 사용될 수 있습니다.
...
Laravel는 매분 실행되도록 단일 `schedule:run` Artisan 명령을 예약하여 [cron 작업 예약](/docs/12.x/scheduling)하는 편리한 방법을 제공합니다. `schedule:run` 명령은 `routes/console.php` 파일에 정의된 작업 일정을 검사하여 실행할 예약된 작업을 결정합니다.
...
Homestead 내에서 [Laravel Dusk](/docs/12.x/dusk) 테스트를 실행하려면 Homestead 구성에서 [webdriver 기능](#installing-optional-features)을 활성화해야 합니다:
