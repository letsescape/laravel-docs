...
`FileSearch` 프로바이더 도구를 사용하면 에이전트가 [files](#files)에 저장된 [vector stores](#vector-stores)을 검색할 수 있습니다. 이를 통해 에이전트가 업로드된 문서에서 관련 정보를 검색할 수 있으므로 검색 증강 생성(RAG)을 구현할 수 있습니다.
