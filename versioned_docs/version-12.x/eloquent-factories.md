...
애플리케이션을 테스트하거나 데이터베이스에 시드 데이터를 삽입할 때, 일부 레코드를 데이터베이스에 추가해야 할 수 있습니다. 각 컬럼의 값을 일일이 지정하는 대신, Laravel에서는 각 [Eloquent models](/docs/12.x/eloquent)에 대해 모델 팩토리를 이용하여 기본 속성 집합을 정의할 수 있도록 지원합니다.
...
팩토리를 생성하려면 `make:factory` [Artisan command](/docs/12.x/artisan)를 실행하십시오.
...
상태 변환 메서드는 보통 Laravel의 기본 팩토리 클래스에서 제공하는 `state` 메서드를 호출합니다. `state` 메서드는 팩토리의 원시 속성 배열을 인자로 받아, 수정할 속성 배열을 반환해야 하는 클로저를 인자로 받습니다:
...
Eloquent 모델이 [soft deleted](/docs/12.x/eloquent#soft-deleting)를 지원한다면, 내장된 `trashed` 상태 메서드를 호출하여 생성된 모델이 소프트 삭제 처리되었음을 바로 지정할 수 있습니다. `trashed` 상태는 모든 팩토리에서 자동으로 사용할 수 있기 때문에 별도의 정의가 필요하지 않습니다:
...
정의한 [states](#factory-states)를 원하는 모델에 적용할 수도 있습니다. 여러 상태 변환을 적용하고 싶다면, 상태 변환 메서드를 연속해서 호출하면 됩니다:
...
> [!NOTE]
> 팩토리를 사용해 모델을 생성할 때는 [Mass assignment protection](/docs/12.x/eloquent#mass-assignment)가 자동으로 비활성화됩니다.
...
위 예시에서는 5명의 유저가 `admin` 값이 `Y`로 생성되고, 5명이 `admin` 값이 `N`으로 생성됩니다.
...
클로저 내부에서는 클로저에 주입되는 시퀀스 인스턴스의 `$index` 속성에 접근할 수 있습니다. `$index` 속성은 지금까지 시퀀스가 반복된 횟수를 담고 있습니다:
...
편의상, 내부적으로 `state` 메서드를 호출하는 `sequence` 메서드로도 시퀀스를 적용할 수 있습니다. `sequence` 메서드는 클로저 또는 시퀀스 속성 배열을 전달받습니다:
...
이번에는 Laravel의 유연한 팩토리 메서드를 이용해 Eloquent 모델 간 연관관계를 생성하는 방법을 살펴봅니다. 예를 들어, 애플리케이션에 `App\Models\User` 모델과 `App\Models\Post` 모델이 있고, `User` 모델은 `Post`와 `hasMany` 연관관계를 가진다고 가정합시다. 이때, Laravel 팩토리가 제공하는 `has` 메서드를 사용하여 세 개의 게시글을 가진 유저를 생성할 수 있습니다. `has` 메서드는 팩토리 인스턴스를 인자로 받습니다:
...
더 편리하게 Laravel의 "매직 팩토리 연관관계 메서드"도 지원합니다. 아래 예시에서, Laravel은 규칙에 따라 `User` 모델의 `posts` 연관관계 메서드를 이용해 관련 모델을 생성합니다:
...
[has many relationships](#has-many-relationships)와 마찬가지로, "many to many" 관계 역시 `has` 메서드를 사용해 생성할 수 있습니다:
...
[Polymorphic relationships](/docs/12.x/eloquent-relationships#polymorphic-relationships) 역시 팩토리를 사용해 생성할 수 있습니다. 폴리모픽 "morph many" 관계는 일반적인 "has many" 관계와 동일하게 팩토리를 활용하면 됩니다. 예를 들어, `App\Models\Post` 모델이 `App\Models\Comment` 모델과 `morphMany` 관계를 가질 때:
...
`morphTo` 연관관계를 만들 때는 매직 메서드를 사용할 수 없습니다. 대신, `for` 메서드를 직접 사용하고, 관계의 이름도 명시적으로 전달해야 합니다. 예를 들어, `Comment` 모델이 `morphTo` 관계를 정의하는 `commentable` 메서드를 가진다면, `for` 메서드를 직접 사용하여 하나의 게시글에 속한 댓글 세 개를 생성할 수 있습니다:
...
또한, `recycle` 메서드는 기존 모델 컬렉션도 받을 수 있습니다. 컬렉션이 `recycle` 메서드에 전달되면, 팩토리가 해당 타입의 모델을 필요로 할 때 컬렉션에서 무작위로 하나씩 선택해 사용하게 됩니다:
