...
현재 HTTP 요청 인스턴스를 의존성 주입(dependency injection)을 통해 얻으려면, 라우트 클로저 또는 컨트롤러 메서드에서 `Illuminate\Http\Request` 클래스를 타입힌트로 지정하면 됩니다. Laravel의 [service container](/docs/master/container)는 자동으로 요청 인스턴스를 주입합니다.
...
`routeIs` 메서드를 사용하면 들어온 요청이 [named route](/docs/master/routing#named-routes)와 일치하는지 확인할 수 있습니다.
...
`Illuminate\Http\Request` 인스턴스에서 `header` 메서드를 사용해 요청 헤더를 조회할 수 있습니다. 해당 헤더가 요청에 없다면 `null`을 반환하며, `header` 메서드의 두 번째 인수로 기본값을 지정할 수도 있습니다.
...
`collect` 메서드를 사용해 입력값을 [collection](/docs/master/collections)으로 받을 수도 있습니다.
...
값이 없을 때 반환할 기본값은 `input` 메서드의 두 번째 인수로 지정할 수 있습니다.
...
`query` 메서드를 인수 없이 호출하면 모든 쿼리 스트링 값을 연관 배열 형태로 반환합니다.
...
입력값을 원시 `string`이 아닌 [Illuminate\Support\Stringable](/docs/master/strings) 인스턴스로 받고 싶다면, `string` 메서드를 사용하세요.
...
체크박스 같은 HTML 요소는 실제로는 문자열 형태의 "truthy" 값을 보낼 수 있습니다. 예를 들어, "true" 또는 "on"이 그런 값입니다. `boolean` 메서드는 1, "1", true, "true", "on", "yes"를 모두 `true`로 변환합니다. `boolean` 메서드는 그 외의 값에 대해 `false`를 반환합니다.
...
`date` 메서드의 두 번째, 세 번째 인수로 날짜 형식과 타임존을 명시적으로 지정할 수 있습니다.
...
입력값 일부만 필요할 때는 `only`와 `except` 메서드를 사용할 수 있습니다. 두 메서드는 하나의 `array` 또는 가변 인수로 키 목록을 받습니다.
...
`has` 메서드를 사용해 특정 값이 요청에 존재하는지 확인할 수 있습니다. 값이 있으면 `has` 메서드는 `true`를 반환합니다.
...
배열을 인수로 넘기면, 지정한 모든 값이 요청에 존재하는지 `has` 메서드가 검사합니다.
...
값이 없을 때 실행할 두 번째 클로저도 `whenHas` 메서드에 전달할 수 있습니다.
...
배열로 전달하면, 지정한 모든 값이 누락되었거나 비어 있는지 `isNotFilled` 메서드가 검사합니다.
...
값이 비어있을 때 실행할 두 번째 클로저도 `whenFilled` 메서드에 지정할 수 있습니다.
...
기존 요청 입력값에 추가 데이터를 직접 병합하고 싶을 때는 `merge` 메서드를 사용합니다. 이미 입력값에 같은 키가 있다면 `merge` 메서드에 전달한 새로운 값이 덮어써집니다.
...
Laravel에서는 한 번의 요청에서 제출한 입력값을 다음 요청까지 유지할 수 있습니다. 이 기능은 유효성 검증에서 에러가 발생한 후, 폼을 다시 채워주는 데 유용합니다. Laravel의 [validation features](/docs/master/validation)을 사용할 때는 해당 메서드를 직접 사용할 일이 줄어들 수 있습니다. 일부 내장 기능에서 자동으로 입력값을 세션에 플래시(flashing)하기 때문입니다.
...
`Illuminate\Http\Request` 클래스의 `flash` 메서드는, 현재의 입력값을 [session](/docs/master/session)에 플래시하여 다음 요청 때 사용 가능하게 해줍니다.
...
이전 요청에서 플래시된 입력값을 가져오려면, `Illuminate\Http\Request` 인스턴스의 `old` 메서드를 호출합니다. `old` 메서드는 이 값들을 [session](/docs/master/session)에서 꺼내옵니다.
...
Laravel은 전역 `old` 헬퍼도 제공합니다. [Blade template](/docs/master/blade)에서 폼을 다시 채워줄 때는 `old` 헬퍼를 사용하는 것이 더욱 편리합니다. 만약 해당 입력값이 없다면 `null`을 반환합니다.
...
특정 요청에서만 문자열 트리밍/빈 문자열 변환을 비활성화하려면 `bootstrap/app.php` 파일에서 각각 `trimStrings`, `convertEmptyStringsToNull` 메서드를 사용하면 됩니다. 두 메서드 모두 클로저 배열을 인수로 받으며, 해당 클로저가 `true` 또는 `false`를 반환해 정규화 과정 생략 여부를 나타냅니다.
...
업로드된 파일은 `Illuminate\Http\Request` 인스턴스에서 `file` 메서드나 동적 프로퍼티를 통해 가져올 수 있습니다. `file` 메서드가 반환하는 값은 PHP의 `SplFileInfo` 클래스를 확장한 `Illuminate\Http\UploadedFile`입니다. 여러 메서드를 통해 파일과 상호작용할 수 있습니다.
...
업로드된 파일을 저장하려면 보통 미리 설정된 [filesystems](/docs/master/filesystem) 중 하나를 사용합니다. `UploadedFile` 클래스의 `store` 메서드는 업로드 파일을 로컬/클라우드 저장소(예: Amazon S3)의 디스크로 이동시킵니다.
...
옵션으로 `store` 메서드의 두 번째 인수에 사용할 디스크 이름을 넘길 수도 있습니다. 반환 값은 해당 디스크 루트에 대한 경로입니다.
...
> [!NOTE]
> Laravel 파일 저장소에 대한 자세한 정보는 [file storage documentation](/docs/master/filesystem)를 참고하세요.
...
> [!NOTE]
> AWS Elastic Load Balancing을 사용하는 경우, `headers` 값은 `Request::HEADER_X_FORWARDED_AWS_ELB`여야 합니다. 표준 [RFC 7239](https://www.rfc-editor.org/rfc/rfc7239#section-4)의 `Forwarded` 헤더를 사용하는 로드 밸런서라면 `headers` 값은 `Request::HEADER_FORWARDED`를 사용해야 합니다. `headers` 값에 사용할 수 있는 상수에 대한 자세한 내용은 Symfony의 [trusting proxies](https://symfony.com/doc/current/deployment/proxies.html)를 참고하세요.
...
`TrustHosts` 미들웨어를 사용하려면, 애플리케이션의 `bootstrap/app.php` 파일에서 `trustHosts` 미들웨어 메서드를 호출하세요. `at` 인수에 호스트네임(정규식)을 지정합니다. 다른 `Host` 헤더를 가진 요청은 거부됩니다.
