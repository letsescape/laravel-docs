...
HTTP 테스트를 단순화하는 것 외에도, Laravel은 애플리케이션에서 작성한 [custom console commands](/docs/master/artisan)를 테스트할 수 있도록 간단한 API를 제공합니다.
...
<!-- If you are utilizing the `search` or `multisearch` functions provided by [Laravel Prompts](/docs/master/prompts), you may use the `expectsSearch` assertion to mock the user's input, search results, and selection: -->
[Laravel Prompts](/docs/master/prompts)에서 제공하는 `search` 또는 `multisearch` 기능을 사용하는 경우, `expectsSearch` 검증 메서드를 통해 사용자 입력, 검색 결과, 선택값을 모킹할 수 있습니다:
