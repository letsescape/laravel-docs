...
웹 애플리케이션을 만들다 보면 업로드된 CSV 파일을 파싱하고 저장하는 작업처럼, 일반적인 웹 요청 중에 수행하기에는 시간이 너무 오래 걸리는 작업이 있을 수 있습니다. 다행히 Laravel은 백그라운드에서 처리할 수 있는 큐 Job을 쉽게 만들 수 있게 해줍니다. 시간이 많이 걸리는 작업을 큐로 옮기면 애플리케이션은 웹 요청에 매우 빠르게 응답할 수 있고, 사용자에게 더 나은 경험을 제공할 수 있습니다.
...
Laravel 큐는 [Amazon SQS](https://aws.amazon.com/sqs/), [Redis](https://redis.io), 관계형 데이터베이스 등 다양한 큐 백엔드 전반에서 통일된 큐 API를 제공합니다.
...
Laravel의 큐 설정 옵션은 애플리케이션의 `config/queue.php` 설정 파일에 저장됩니다. 이 파일에는 프레임워크에 포함된 각 큐 드라이버의 연결 설정이 들어 있습니다. 여기에는 database, [Amazon SQS](https://aws.amazon.com/sqs/), [Redis](https://redis.io), [Beanstalkd](https://beanstalkd.github.io/) 드라이버가 포함되며, 개발 또는 테스트 중에 사용할 수 있도록 Job을 즉시 실행하는 동기 드라이버도 포함됩니다. 큐에 등록된 Job을 버리는 `null` 큐 드라이버도 함께 제공됩니다.

> [!NOTE]
> Laravel Horizon은 Redis 기반 큐를 위한 보기 좋은 대시보드이자 설정 시스템입니다. 자세한 내용은 전체 [Horizon documentation](/docs/master/horizon)를 확인하세요.
...
Laravel 큐를 시작하기 전에 "연결"과 "큐"의 차이를 이해하는 것이 중요합니다. `config/queue.php` 설정 파일에는 `connections` 설정 배열이 있습니다. 이 옵션은 Amazon SQS, Beanstalk, Redis 같은 백엔드 큐 서비스에 대한 연결을 정의합니다. 하지만 하나의 큐 연결은 여러 개의 "큐"를 가질 수 있으며, 이는 큐에 등록된 Job을 담아 두는 서로 다른 스택이나 묶음이라고 생각할 수 있습니다.

<!-- Note that each connection configuration example in the `queue` configuration file contains a `queue` attribute. This is the default queue that jobs will be dispatched to when they are sent to a given connection. In other words, if you dispatch a job without explicitly defining which queue it should be dispatched to, the job will be placed on the queue that is defined in the `queue` attribute of the connection configuration: -->
`queue` 설정 파일의 각 연결 설정 예시에는 `queue` 속성이 포함되어 있습니다. 이 값은 Job이 특정 연결로 전송될 때 기본적으로 디스패치되는 큐입니다. 즉, 어떤 큐로 디스패치할지 명시적으로 정의하지 않고 Job을 디스패치하면, 해당 Job은 연결 설정의 `queue` 속성에 정의된 큐에 배치됩니다.
...
일부 애플리케이션은 여러 큐에 Job을 넣을 필요 없이 단순히 하나의 큐만 사용하는 방식을 선호할 수 있습니다. 하지만 Laravel 큐 워커는 처리할 큐를 우선순위 순서로 지정할 수 있으므로, Job 처리 방식을 우선순위별로 나누거나 구분하려는 애플리케이션에서는 여러 큐에 Job을 넣는 방식이 특히 유용합니다. 예를 들어 Job을 `high` 큐에 넣는다면, 해당 큐를 더 높은 처리 우선순위로 다루는 워커를 실행할 수 있습니다.
...
<!-- In order to use the `database` queue driver, you will need a database table to hold the jobs. Typically, this is included in Laravel's default `0001_01_01_000002_create_jobs_table.php` [database migration](/docs/master/migrations); however, if your application does not contain this migration, you may use the `make:queue-table` Artisan command to create it: -->
`database` 큐 드라이버를 사용하려면 Job을 저장할 데이터베이스 테이블이 필요합니다. 일반적으로 이 테이블은 Laravel의 기본 `0001_01_01_000002_create_jobs_table.php` [database migration](/docs/master/migrations)에 포함되어 있습니다. 하지만 애플리케이션에 이 마이그레이션이 없다면 `make:queue-table` Artisan 명령어로 만들 수 있습니다.
...
`redis` 큐 드라이버를 사용하려면 `config/database.php` 설정 파일에 Redis 데이터베이스 연결을 설정해야 합니다.
...
Redis 큐 연결이 [Redis Cluster](https://redis.io/docs/latest/operate/rs/databases/durability-ha/clustering)를 사용하는 경우, 큐 이름에는 [key hash tag](https://redis.io/docs/latest/develop/using-commands/keyspace/#hashtags)가 포함되어야 합니다. 이는 특정 큐에 대한 모든 Redis 키가 같은 해시 슬롯에 배치되도록 하기 위해 필요합니다.
...
Redis 큐를 사용할 때는 `block_for` 설정 옵션을 사용해, Job을 사용할 수 있을 때까지 드라이버가 얼마나 오래 기다린 뒤 워커 루프를 다시 돌며 Redis 데이터베이스를 다시 폴링할지 지정할 수 있습니다.
...
큐 부하에 맞게 이 값을 조정하면 새 Job을 찾기 위해 Redis 데이터베이스를 계속 폴링하는 것보다 더 효율적일 수 있습니다. 예를 들어 값을 `5`로 설정하면, 드라이버가 Job을 사용할 수 있을 때까지 5초 동안 블로킹하도록 지정할 수 있습니다.
...
> [!WARNING]
> `block_for`를 `0`으로 설정하면 큐 워커는 Job을 사용할 수 있을 때까지 무기한 블로킹됩니다. 또한 다음 Job이 처리될 때까지 `SIGTERM` 같은 시그널도 처리되지 않습니다.
...
아래 큐 드라이버를 사용하려면 다음 의존성이 필요합니다. 이러한 의존성은 Composer 패키지 매니저를 통해 설치할 수 있습니다.
...
기본적으로 애플리케이션의 모든 큐 가능 Job은 `app/Jobs` 디렉터리에 저장됩니다. `app/Jobs` 디렉터리가 없다면 `make:job` Artisan 명령어를 실행할 때 생성됩니다.
...
생성된 클래스는 `Illuminate\Contracts\Queue\ShouldQueue` 인터페이스를 구현합니다. 이는 해당 Job을 비동기로 실행하기 위해 큐에 넣어야 한다는 것을 Laravel에 알려줍니다.

> [!NOTE]
> Job 스텁은 [stub publishing](/docs/master/artisan#stub-customization)을 사용해 커스터마이징할 수 있습니다.
...
Job 클래스는 매우 단순하며, 일반적으로 큐에서 Job이 처리될 때 호출되는 `handle` 메서드만 포함합니다. 시작을 위해 예시 Job 클래스를 살펴보겠습니다. 이 예시에서는 우리가 팟캐스트 발행 서비스를 운영하며, 업로드된 팟캐스트 파일을 발행 전에 처리해야 한다고 가정하겠습니다.
...
<!-- In this example, note that we were able to pass an [Eloquent model](/docs/master/eloquent) directly into the queued job's constructor. Because of the `Queueable` trait that the job is using, Eloquent models and their loaded relationships will be gracefully serialized and unserialized when the job is processing. -->
이 예시에서 [Eloquent model](/docs/master/eloquent)을 큐 Job의 생성자에 직접 전달할 수 있다는 점에 주목하세요. Job이 사용하는 `Queueable` 트레이트 덕분에, Eloquent 모델과 로드된 연관관계는 Job이 처리될 때 자연스럽게 직렬화되고 역직렬화됩니다.

<!-- If your queued job accepts an Eloquent model in its constructor, only the identifier for the model will be serialized onto the queue. When the job is actually handled, the queue system will automatically re-retrieve the full model instance and its loaded relationships from the database. This approach to model serialization allows for much smaller job payloads to be sent to your queue driver. -->
큐 Job이 생성자에서 Eloquent 모델을 받는 경우, 큐에는 해당 모델의 식별자만 직렬화됩니다. Job이 실제로 처리될 때 큐 시스템은 전체 모델 인스턴스와 로드된 연관관계를 데이터베이스에서 자동으로 다시 조회합니다. 이러한 모델 직렬화 방식 덕분에 큐 드라이버로 전송되는 Job 페이로드를 훨씬 작게 유지할 수 있습니다.
...
`handle` 메서드는 Job이 큐에서 처리될 때 호출됩니다. Job의 `handle` 메서드에서 의존성을 타입 힌트로 지정할 수 있다는 점에 주목하세요. Laravel [service container](/docs/master/container)는 이러한 의존성을 자동으로 주입합니다.
...
컨테이너가 `handle` 메서드에 의존성을 주입하는 방식을 완전히 제어하고 싶다면, 컨테이너의 `bindMethod` 메서드를 사용할 수 있습니다. `bindMethod` 메서드는 Job과 컨테이너를 받는 콜백을 인수로 받습니다. 콜백 안에서는 원하는 방식으로 `handle` 메서드를 호출할 수 있습니다. 일반적으로 이 메서드는 `App\Providers\AppServiceProvider` [service provider](/docs/master/providers)의 `boot` 메서드에서 호출해야 합니다.
...
> [!WARNING]
> 원본 이미지 내용과 같은 바이너리 데이터는 큐 Job에 전달하기 전에 `base64_encode` 함수를 통해 전달해야 합니다. 그렇지 않으면 Job이 큐에 배치될 때 JSON으로 제대로 직렬화되지 않을 수 있습니다.
...
Job이 큐에 등록될 때 로드된 모든 Eloquent 모델 연관관계도 함께 직렬화되므로, 직렬화된 Job 문자열이 때때로 매우 커질 수 있습니다. 또한 Job이 역직렬화되고 모델 연관관계가 데이터베이스에서 다시 조회될 때, 해당 연관관계는 전체가 조회됩니다. Job을 큐에 등록하는 과정에서 모델이 직렬화되기 전에 적용되었던 기존 연관관계 제약은 Job이 역직렬화될 때 적용되지 않습니다. 따라서 특정 연관관계의 일부만 다루고 싶다면 큐 Job 안에서 해당 연관관계에 다시 제약을 걸어야 합니다.
...
[PHP constructor property promotion](https://www.php.net/manual/en/language.oop5.decon.php#language.oop5.decon.constructor.promotion)을 사용하면서 Eloquent 모델의 관계가 직렬화되지 않아야 한다는 것을 표시하고 싶다면, `WithoutRelations` 속성을 사용할 수 있습니다.
...
편의를 위해 모든 모델을 연관관계 없이 직렬화하고 싶다면, 각 모델에 속성을 적용하는 대신 전체 클래스에 `WithoutRelations` 속성을 적용할 수 있습니다.
...
Job이 단일 모델 대신 Eloquent 모델 컬렉션이나 배열을 받는 경우, 해당 컬렉션 안의 모델들은 Job이 역직렬화되어 실행될 때 연관관계가 복원되지 않습니다. 이는 많은 수의 모델을 다루는 Job에서 과도한 리소스 사용을 방지하기 위한 것입니다.
...
> [!WARNING]
> 고유 Job에는 [locks](/docs/master/cache#atomic-locks)을 지원하는 캐시 드라이버가 필요합니다. 현재 `memcached`, `redis`, `dynamodb`, `database`, `file`, `array` 캐시 드라이버가 원자적 락을 지원합니다.

> [!WARNING]
> 고유 Job 제약은 배치 안의 Job에는 적용되지 않습니다.
...
때로는 특정 Job의 인스턴스가 어느 시점에도 큐에 하나만 존재하도록 보장하고 싶을 수 있습니다. Job 클래스에 `ShouldBeUnique` 인터페이스를 구현하면 이를 달성할 수 있습니다. 이 인터페이스는 클래스에 추가 메서드를 정의하도록 요구하지 않습니다.
...
위 예시에서 `UpdateSearchIndex` Job은 고유합니다. 따라서 해당 Job의 다른 인스턴스가 이미 큐에 있고 아직 처리가 끝나지 않았다면, 새 Job은 디스패치되지 않습니다.
...
특정한 경우에는 Job을 고유하게 만드는 특정 "키"를 정의하거나, Job이 더 이상 고유 상태로 유지되지 않을 타임아웃을 지정하고 싶을 수 있습니다. 이를 위해 `UniqueFor` 속성을 사용하고 Job 클래스에 `uniqueId` 메서드를 정의할 수 있습니다.
...
위 예시에서 `UpdateSearchIndex` Job은 상품 ID를 기준으로 고유합니다. 따라서 같은 상품 ID를 가진 해당 Job의 새로운 디스패치는 기존 Job 처리가 완료될 때까지 무시됩니다. 또한 기존 Job이 1시간 안에 처리되지 않으면 고유 락이 해제되고, 같은 고유 키를 가진 다른 Job을 큐에 디스패치할 수 있습니다.

> [!WARNING]
> 애플리케이션이 여러 웹 서버나 컨테이너에서 Job을 디스패치한다면, Laravel이 Job의 고유 여부를 정확히 판단할 수 있도록 모든 서버가 동일한 중앙 캐시 서버와 통신하도록 해야 합니다.
...
기본적으로 고유 Job은 Job 처리가 완료되거나 모든 재시도 시도가 실패한 뒤 "잠금 해제"됩니다. 하지만 Job이 처리되기 직전에 즉시 잠금 해제되도록 하고 싶은 상황도 있을 수 있습니다. 이를 위해 Job은 `ShouldBeUnique` 계약 대신 `ShouldBeUniqueUntilProcessing` 계약을 구현해야 합니다.
...
<!-- Behind the scenes, when a `ShouldBeUnique` job is dispatched, Laravel attempts to acquire a [lock](/docs/master/cache#atomic-locks) with the `uniqueId` key. If the lock is already held, the job is not dispatched. This lock is released when the job completes processing or fails all of its retry attempts. By default, Laravel will use the default cache driver to obtain this lock. However, if you wish to use another driver for acquiring the lock, you may define a `uniqueVia` method that returns the cache driver that should be used: -->
내부적으로 `ShouldBeUnique` Job이 디스패치되면 Laravel은 `uniqueId` 키로 [lock](/docs/master/cache#atomic-locks)을 획득하려고 시도합니다. 이미 락이 잡혀 있다면 Job은 디스패치되지 않습니다. 이 락은 Job 처리가 완료되거나 모든 재시도 시도가 실패하면 해제됩니다. 기본적으로 Laravel은 이 락을 얻기 위해 기본 캐시 드라이버를 사용합니다. 하지만 락 획득에 다른 드라이버를 사용하고 싶다면 사용할 캐시 드라이버를 반환하는 `uniqueVia` 메서드를 정의할 수 있습니다.
...
```
> [!NOTE]
> 작업의 동시 처리만 제한하면 되는 경우에는 대신 [WithoutOverlapping](/docs/master/queues#preventing-job-overlaps) 작업 미들웨어를 사용하십시오.

<a name="encrypted-jobs"></a>
<!-- ### Encrypted Jobs -->
### Encrypted Jobs

<!-- Laravel allows you to ensure the privacy and integrity of a job's data via [encryption](/docs/master/encryption). To get started, simply add the `ShouldBeEncrypted` interface to the job class. Once this interface has been added to the class, Laravel will automatically encrypt your job before pushing it onto a queue: -->
Laravel은 [encryption](/docs/master/encryption)를 통해 작업 데이터의 개인정보 보호와 무결성을 보장할 수 있게 해줍니다. 시작하려면 작업 클래스에 `ShouldBeEncrypted` 인터페이스를 추가하기만 하면 됩니다. 이 인터페이스가 클래스에 추가되면, Laravel은 작업을 큐에 넣기 전에 자동으로 암호화합니다.
...
작업 미들웨어를 사용하면 큐 작업이 실행되는 과정 앞뒤에 사용자 정의 로직을 감쌀 수 있어, 작업 클래스 자체의 반복 코드를 줄일 수 있습니다. 예를 들어, 다음 `handle` 메서드는 Laravel의 Redis 속도 제한 기능을 활용하여 5초마다 하나의 작업만 처리되도록 허용합니다.
...
이 코드는 유효하지만, `handle` 메서드 구현이 Redis 속도 제한 로직으로 채워져 복잡해집니다. 또한 속도 제한을 적용하려는 다른 작업이 있다면 이 속도 제한 로직을 매번 중복해서 작성해야 합니다. handle 메서드 안에서 속도를 제한하는 대신, 속도 제한을 처리하는 작업 미들웨어를 정의할 수 있습니다.
...
보시다시피, [route middleware](/docs/master/middleware)와 마찬가지로 작업 미들웨어는 처리 중인 작업과, 작업 처리를 계속 진행하기 위해 호출해야 하는 콜백을 전달받습니다.
...
`make:job-middleware` Artisan 명령어를 사용하여 새 작업 미들웨어 클래스를 생성할 수 있습니다. 작업 미들웨어를 만든 뒤에는 작업의 `middleware` 메서드에서 이를 반환하여 작업에 연결할 수 있습니다. 이 메서드는 `make:job` Artisan 명령어로 생성된 작업에는 존재하지 않으므로, 작업 클래스에 직접 추가해야 합니다.
...
> [!NOTE]
> 작업 미들웨어는 [queueable event listeners](/docs/master/events#queued-event-listeners), [mailables](/docs/master/mail#queueing-mail), [notifications](/docs/master/notifications#queueing-notifications)에도 지정할 수 있습니다.
...
방금 직접 속도 제한 작업 미들웨어를 작성하는 방법을 살펴보았지만, Laravel에는 작업 속도 제한에 활용할 수 있는 속도 제한 미들웨어가 실제로 포함되어 있습니다. [route rate limiters](/docs/master/routing#defining-rate-limiters)와 마찬가지로, 작업 속도 제한기는 `RateLimiter` 파사드의 `for` 메서드를 사용하여 정의합니다.
...
예를 들어, 프리미엄 고객에게는 이러한 제한을 적용하지 않으면서 일반 사용자는 한 시간에 한 번만 데이터를 백업할 수 있도록 허용하고 싶을 수 있습니다. 이를 위해 `AppServiceProvider`의 `boot` 메서드에서 `RateLimiter`를 정의할 수 있습니다.
...
위 예제에서는 시간당 속도 제한을 정의했습니다. 그러나 `perMinute` 메서드를 사용하면 분 단위의 속도 제한도 쉽게 정의할 수 있습니다. 또한 속도 제한의 `by` 메서드에는 원하는 값을 전달할 수 있지만, 이 값은 보통 고객별로 속도 제한을 구분하는 데 사용됩니다.
...
속도 제한을 정의한 뒤에는 `Illuminate\Queue\Middleware\RateLimited` 미들웨어를 사용하여 작업에 속도 제한기를 연결할 수 있습니다. 작업이 속도 제한을 초과할 때마다, 이 미들웨어는 속도 제한 기간에 따라 적절한 지연 시간을 적용하여 작업을 다시 큐로 되돌립니다.
...
속도 제한에 걸린 작업을 다시 큐로 되돌려도 작업의 전체 `attempts` 횟수는 증가합니다. 따라서 작업 클래스의 `Tries`와 `MaxExceptions` 속성을 상황에 맞게 조정해야 할 수 있습니다. 또는 [retryUntil method](#time-based-attempts)를 사용하여 더 이상 작업을 시도하지 않을 시점을 정의할 수도 있습니다.
...
`releaseAfter` 메서드를 사용하면 되돌려진 작업을 다시 시도하기 전에 지나야 하는 초 단위 시간도 지정할 수 있습니다.
...
작업이 속도 제한에 걸렸을 때 다시 시도하지 않게 하려면 `dontRelease` 메서드를 사용할 수 있습니다.
...
Redis를 사용하고 있다면 `Illuminate\Queue\Middleware\RateLimitedWithRedis` 미들웨어를 사용할 수 있습니다. 이 미들웨어는 Redis에 맞게 조정되어 있으며, 기본 속도 제한 미들웨어보다 더 효율적입니다.
...
Laravel에는 임의의 키를 기준으로 작업이 서로 겹쳐 실행되는 것을 방지할 수 있는 `Illuminate\Queue\Middleware\WithoutOverlapping` 미들웨어가 포함되어 있습니다. 이는 큐 작업이 한 번에 하나의 작업만 수정해야 하는 리소스를 변경할 때 유용합니다.
...
예를 들어, 사용자의 신용 점수를 업데이트하는 큐 작업이 있고, 같은 사용자 ID에 대한 신용 점수 업데이트 작업이 겹치지 않도록 하고 싶다고 가정해 보겠습니다. 이를 위해 작업의 `middleware` 메서드에서 `WithoutOverlapping` 미들웨어를 반환하면 됩니다.
...
겹치는 작업을 다시 큐로 되돌려도 작업의 전체 시도 횟수는 증가합니다. 따라서 작업 클래스의 `Tries`와 `MaxExceptions` 속성을 상황에 맞게 조정해야 할 수 있습니다. 예를 들어, `Tries`를 기본값인 1로 그대로 두면 겹치는 작업은 나중에 다시 시도되지 않습니다.
...
같은 유형의 겹치는 작업은 다시 큐로 되돌려집니다. 또한 되돌려진 작업을 다시 시도하기 전에 지나야 하는 초 단위 시간도 지정할 수 있습니다.
...
겹치는 작업을 즉시 삭제하여 다시 시도되지 않게 하려면 `dontRelease` 메서드를 사용할 수 있습니다.
...
`WithoutOverlapping` 미들웨어는 Laravel의 원자적 잠금 기능을 기반으로 동작합니다. 때로는 작업이 예기치 않게 실패하거나 시간 초과되어 잠금이 해제되지 않을 수 있습니다. 따라서 `expireAfter` 메서드를 사용하여 잠금 만료 시간을 명시적으로 정의할 수 있습니다. 예를 들어, 아래 예제는 작업 처리가 시작된 지 3분이 지나면 Laravel이 `WithoutOverlapping` 잠금을 해제하도록 지시합니다.
...
> [!WARNING]
> `WithoutOverlapping` 미들웨어를 사용하려면 [locks](/docs/master/cache#atomic-locks)을 지원하는 캐시 드라이버가 필요합니다. 현재 `memcached`, `redis`, `dynamodb`, `database`, `file`, `array` 캐시 드라이버가 원자적 잠금을 지원합니다.
...
기본적으로 `WithoutOverlapping` 미들웨어는 같은 클래스의 겹치는 작업만 방지합니다. 따라서 서로 다른 두 작업 클래스가 같은 잠금 키를 사용하더라도, 서로 겹쳐 실행되는 것은 방지되지 않습니다. 하지만 `shared` 메서드를 사용하면 Laravel이 해당 키를 여러 작업 클래스에 걸쳐 적용하도록 지시할 수 있습니다.
...
Laravel에는 예외를 스로틀링할 수 있는 `Illuminate\Queue\Middleware\ThrottlesExceptions` 미들웨어가 포함되어 있습니다. 작업이 지정된 횟수만큼 예외를 던지면, 이후의 모든 작업 실행 시도는 지정된 시간 간격이 지날 때까지 지연됩니다. 이 미들웨어는 특히 불안정한 타사 서비스와 상호작용하는 작업에 유용합니다.
...
예를 들어, 예외를 던지기 시작한 타사 API와 상호작용하는 큐 작업이 있다고 가정해 보겠습니다. 예외를 스로틀링하려면 작업의 `middleware` 메서드에서 `ThrottlesExceptions` 미들웨어를 반환하면 됩니다. 일반적으로 이 미들웨어는 [time based attempts](#time-based-attempts)를 구현하는 작업과 함께 사용해야 합니다.
...
미들웨어가 받는 첫 번째 생성자 인수는 스로틀링되기 전에 작업이 던질 수 있는 예외 횟수이고, 두 번째 생성자 인수는 스로틀링된 뒤 작업을 다시 시도하기 전에 지나야 하는 초 단위 시간입니다. 위 코드 예제에서는 작업이 연속으로 10번 예외를 던지면, 30분 시간 제한 안에서 작업을 다시 시도하기 전에 5분을 기다립니다.
...
작업이 예외를 던졌지만 예외 임계값에 아직 도달하지 않은 경우, 일반적으로 작업은 즉시 다시 시도됩니다. 하지만 작업에 미들웨어를 연결할 때 `backoff` 메서드를 호출하면, 이러한 작업을 몇 분 지연한 뒤 다시 시도할지 지정할 수 있습니다.
...
내부적으로 이 미들웨어는 Laravel의 캐시 시스템을 사용하여 속도 제한을 구현하며, 작업의 클래스 이름이 캐시 "key"로 사용됩니다. 작업에 미들웨어를 연결할 때 `by` 메서드를 호출하면 이 키를 재정의할 수 있습니다. 같은 타사 서비스와 상호작용하는 여러 작업이 있고, 단일 공유 제한을 지키도록 공통 스로틀링 "bucket"을 공유하게 하고 싶을 때 유용할 수 있습니다.
...
기본적으로 이 미들웨어는 모든 예외를 스로틀링합니다. 작업에 미들웨어를 연결할 때 `when` 메서드를 호출하면 이 동작을 수정할 수 있습니다. 그러면 `when` 메서드에 전달된 클로저가 `true`를 반환하는 경우에만 해당 예외가 스로틀링됩니다.
...
작업을 다시 큐로 되돌리거나 예외를 던지는 `when` 메서드와 달리, `deleteWhen` 메서드를 사용하면 지정한 예외가 발생했을 때 작업을 완전히 삭제할 수 있습니다.
...
스로틀링된 예외를 애플리케이션의 예외 핸들러에 보고하고 싶다면, 작업에 미들웨어를 연결할 때 `report` 메서드를 호출하면 됩니다. 선택 사항으로 `report` 메서드에 클로저를 전달할 수 있으며, 이 경우 주어진 클로저가 `true`를 반환할 때만 예외가 보고됩니다.
...
Redis를 사용하고 있다면, Redis에 맞게 세밀하게 조정되어 기본 예외 제한 Middleware보다 더 효율적인 `Illuminate\Queue\Middleware\ThrottlesExceptionsWithRedis` Middleware를 사용할 수 있습니다.
...
<!-- The `connection` method may be used to specify which Redis connection the middleware should use: -->
`connection` 메서드를 사용하여 Middleware가 사용할 Redis 커넥션을 지정할 수 있습니다.
...
`Skip` Middleware를 사용하면 작업의 로직을 수정하지 않고도 작업을 건너뛰거나 삭제하도록 지정할 수 있습니다. `Skip::when` 메서드는 주어진 조건이 `true`로 평가되면 작업을 삭제하고, `Skip::unless` 메서드는 조건이 `false`로 평가되면 작업을 삭제합니다.
...
작업 클래스를 작성한 후에는 작업 자체의 `dispatch` 메서드를 사용하여 작업을 디스패치할 수 있습니다. `dispatch` 메서드에 전달된 인수는 작업의 생성자에 전달됩니다.
...
조건에 따라 작업을 디스패치하려면 `dispatchIf` 및 `dispatchUnless` 메서드를 사용할 수 있습니다.
...
새 Laravel 애플리케이션에서는 `database` 커넥션이 기본 큐로 정의되어 있습니다. 애플리케이션의 `.env` 파일에서 `QUEUE_CONNECTION` 환경 변수를 변경하여 다른 기본 큐 커넥션을 지정할 수 있습니다.
...
큐 워커가 작업을 즉시 처리할 수 없도록 지정하고 싶다면, 작업을 디스패치할 때 `delay` 메서드를 사용할 수 있습니다. 예를 들어, 작업이 디스패치된 후 10분이 지나기 전까지는 처리할 수 없도록 지정해 보겠습니다.
...
경우에 따라 작업에 기본 지연 시간이 설정되어 있을 수 있습니다. 이 지연을 우회하고 작업을 즉시 처리하도록 디스패치해야 한다면 `withoutDelay` 메서드를 사용할 수 있습니다.
...
작업을 즉시(동기적으로) 디스패치하고 싶다면 `dispatchSync` 메서드를 사용할 수 있습니다. 이 메서드를 사용하면 작업은 큐에 들어가지 않고 현재 프로세스 안에서 즉시 실행됩니다.
...
지연된 동기 디스패치를 사용하면 현재 프로세스에서 처리될 작업을 디스패치하되, HTTP 응답이 사용자에게 전송된 이후에 처리되도록 할 수 있습니다. 이를 통해 사용자의 애플리케이션 사용 경험을 느리게 만들지 않으면서 “큐에 들어간” 작업을 동기적으로 처리할 수 있습니다. 동기 작업의 실행을 뒤로 미루려면 작업을 `deferred` 커넥션으로 디스패치하십시오.
...
`deferred` 커넥션은 기본 [failover queue](#queue-failover) 역할도 합니다.
...
마찬가지로 `background` 커넥션도 HTTP 응답이 사용자에게 전송된 후 작업을 처리합니다. 다만 이 작업은 별도로 생성된 PHP 프로세스에서 처리되므로, PHP-FPM / 애플리케이션 워커는 다른 들어오는 HTTP 요청을 처리할 수 있습니다.
...
데이터베이스 트랜잭션 안에서 작업을 디스패치하는 것 자체는 괜찮지만, 작업이 실제로 성공적으로 실행될 수 있는지 특별히 주의해야 합니다. 트랜잭션 안에서 작업을 디스패치하면, 부모 트랜잭션이 커밋되기 전에 워커가 해당 작업을 처리할 수 있습니다. 이런 일이 발생하면 데이터베이스 트랜잭션 중에 모델이나 데이터베이스 레코드에 적용한 변경 사항이 아직 데이터베이스에 반영되지 않았을 수 있습니다. 또한 트랜잭션 안에서 생성한 모델이나 데이터베이스 레코드가 아직 데이터베이스에 존재하지 않을 수도 있습니다.
...
다행히 Laravel은 이 문제를 해결할 수 있는 여러 방법을 제공합니다. 먼저, 큐 커넥션의 설정 배열에서 `after_commit` 커넥션 옵션을 설정할 수 있습니다.
...
`after_commit` 옵션이 `true`이면 데이터베이스 트랜잭션 안에서 작업을 디스패치할 수 있습니다. 하지만 Laravel은 열려 있는 부모 데이터베이스 트랜잭션이 커밋될 때까지 기다린 후 실제로 작업을 디스패치합니다. 물론 현재 열려 있는 데이터베이스 트랜잭션이 없다면 작업은 즉시 디스패치됩니다.
...
> [!NOTE]
> `after_commit` 설정 옵션을 `true`로 설정하면 큐에 들어간 이벤트 리스너, mailables, 알림, 브로드캐스트 이벤트도 열려 있는 모든 데이터베이스 트랜잭션이 커밋된 후 디스패치됩니다.
...
`after_commit` 큐 커넥션 설정 옵션을 `true`로 설정하지 않았더라도, 특정 작업이 열려 있는 모든 데이터베이스 트랜잭션이 커밋된 후 디스패치되어야 한다고 지정할 수 있습니다. 이를 위해 디스패치 작업에 `afterCommit` 메서드를 체이닝할 수 있습니다.
...
마찬가지로 `after_commit` 설정 옵션이 `true`로 설정되어 있더라도, 특정 작업은 열려 있는 데이터베이스 트랜잭션이 커밋되기를 기다리지 않고 즉시 디스패치되도록 지정할 수 있습니다.
...
작업 체이닝을 사용하면 기본 작업이 성공적으로 실행된 후 순서대로 실행되어야 하는 큐 작업 목록을 지정할 수 있습니다. 순서에 포함된 작업 중 하나라도 실패하면 나머지 작업은 실행되지 않습니다. 큐 작업 체인을 실행하려면 `Bus` 파사드가 제공하는 `chain` 메서드를 사용할 수 있습니다. Laravel의 명령어 버스는 큐 작업 디스패치가 그 위에 구축되어 있는 더 낮은 수준의 컴포넌트입니다.
...
> [!WARNING]
> 작업 안에서 `$this->delete()` 메서드를 사용하여 작업을 삭제해도 체이닝된 작업의 처리를 막지는 않습니다. 체인은 체인 안의 작업이 실패할 때만 실행을 중단합니다.
...
체이닝된 작업에 사용할 커넥션과 큐를 지정하고 싶다면 `onConnection` 및 `onQueue` 메서드를 사용할 수 있습니다. 이 메서드들은 큐 작업에 다른 커넥션 / 큐가 명시적으로 지정되지 않은 경우 사용할 큐 커넥션과 큐 이름을 지정합니다.
...
때로는 체인 안의 다른 작업에서 기존 작업 체인의 앞이나 뒤에 작업을 추가해야 할 수 있습니다. `prependToChain` 및 `appendToChain` 메서드를 사용하면 이를 수행할 수 있습니다.
...
> [!WARNING]
> 체인 콜백은 직렬화되어 나중에 Laravel 큐에서 실행되므로, 체인 콜백 안에서는 `$this` 변수를 사용하면 안 됩니다.
...
작업을 서로 다른 큐에 넣으면 큐 작업을 “분류”할 수 있고, 다양한 큐에 할당할 워커 수를 우선순위에 따라 조정할 수도 있습니다. 단, 이는 큐 설정 파일에 정의된 서로 다른 큐 “커넥션”으로 작업을 넣는 것이 아니라, 하나의 커넥션 안에 있는 특정 큐로만 작업을 넣는다는 점을 기억하십시오. 큐를 지정하려면 작업을 디스패치할 때 `onQueue` 메서드를 사용하십시오.
...
또는 작업의 생성자 안에서 `onQueue` 메서드를 호출하여 작업의 큐를 지정할 수 있습니다.
...
애플리케이션이 여러 큐 커넥션과 상호작용한다면, `onConnection` 메서드를 사용하여 작업을 어느 커넥션에 넣을지 지정할 수 있습니다.
...
`onConnection` 및 `onQueue` 메서드를 함께 체이닝하여 작업의 연결과 큐를 지정할 수 있습니다.
...
또는 작업의 생성자 안에서 `onConnection` 메서드를 호출하여 작업의 연결을 지정할 수 있습니다.
...
`Queue` 파사드의 `route` 메서드를 사용하여 특정 작업 클래스의 기본 연결과 큐를 정의할 수 있습니다. 이는 특정 작업이 항상 지정된 큐를 사용하도록 보장하면서도, 작업마다 연결이나 큐를 직접 지정하지 않아도 되게 할 때 유용합니다.
...
특정 작업 클래스를 라우팅하는 것 외에도, `route` 메서드에 인터페이스, trait, 또는 부모 클래스를 전달할 수도 있습니다. 이렇게 하면 해당 인터페이스를 구현하거나, trait를 사용하거나, 부모 클래스를 상속하는 모든 작업이 자동으로 설정된 연결과 큐를 사용합니다.
...
작업이 디스패치되면 큐에 추가됩니다. 그런 다음 워커가 작업을 가져와 실행을 시도합니다. 이것이 작업 시도입니다.
...
하지만 시도가 반드시 작업의 `handle` 메서드가 실행되었다는 뜻은 아닙니다. 시도 횟수는 다음과 같은 여러 방식으로도 "소비"될 수 있습니다.
...
- `$this->release()`를 사용하여 작업을 수동으로 다시 큐에 반환합니다.
- `WithoutOverlapping` 또는 `RateLimited` 같은 Middleware가 락을 획득하지 못해 작업을 반환합니다.
- 작업이 타임아웃됩니다.
...
작업을 무한히 계속 시도하고 싶지는 않을 가능성이 높습니다. 따라서 Laravel은 작업을 몇 번까지, 또는 얼마 동안 시도할 수 있는지 지정하는 다양한 방법을 제공합니다.

> [!NOTE]
> 기본적으로 Laravel은 작업을 한 번만 시도합니다. 작업이 `WithoutOverlapping` 또는 `RateLimited` 같은 Middleware를 사용하거나, 작업을 수동으로 반환하는 경우에는 `tries` 옵션을 통해 허용된 시도 횟수를 늘려야 할 가능성이 높습니다.
...
작업을 시도할 수 있는 최대 횟수를 지정하는 한 가지 방법은 Artisan 명령줄에서 `--tries` 스위치를 사용하는 것입니다. 처리 중인 작업 자체에 시도 가능 횟수가 지정되어 있지 않다면, 이 값은 워커가 처리하는 모든 작업에 적용됩니다.
...
작업이 최대 시도 횟수를 초과하면 "실패한" 작업으로 간주됩니다. 실패한 작업 처리에 대한 자세한 내용은 [failed job documentation](#dealing-with-failed-jobs)를 참고하십시오. `queue:work` 명령어에 `--tries=0`을 제공하면 작업은 무기한 재시도됩니다.
...
작업 클래스 자체에 `Tries` 속성을 사용하여 작업을 시도할 수 있는 최대 횟수를 정의하는 더 세밀한 방법도 있습니다. 작업에 최대 시도 횟수가 지정되어 있으면, 명령줄에서 제공한 `--tries` 값보다 우선합니다.
...
작업이 실패하기 전까지 몇 번 시도할 수 있는지 정의하는 대신, 작업을 더 이상 시도하지 않아야 하는 시점을 정의할 수도 있습니다. 이렇게 하면 주어진 시간 범위 안에서는 작업을 몇 번이든 시도할 수 있습니다. 작업을 더 이상 시도하지 않아야 하는 시점을 정의하려면 작업 클래스에 `retryUntil` 메서드를 추가하십시오. 이 메서드는 `DateTime` 인스턴스를 반환해야 합니다.
...
> [!NOTE]
> [queued event listeners](/docs/master/events#queued-event-listeners)와 [queued notifications](/docs/master/notifications#queueing-notifications)에도 `Tries` 속성 또는 `retryUntil` 메서드를 정의할 수 있습니다.
...
때로는 작업을 여러 번 시도할 수 있도록 하되, 재시도가 직접 `release` 메서드로 반환되어 발생한 것이 아니라 일정 횟수의 처리되지 않은 예외로 인해 발생한 경우에는 작업이 실패하도록 지정하고 싶을 수 있습니다. 이를 위해 작업 클래스에 `Tries` 및 `MaxExceptions` 속성을 사용할 수 있습니다.
...
이 예제에서 애플리케이션이 Redis 락을 획득하지 못하면 작업은 10초 동안 반환되며, 최대 25번까지 계속 재시도됩니다. 하지만 작업에서 처리되지 않은 예외가 세 번 발생하면 작업은 실패합니다.
...
큐에 등록된 작업이 대략 얼마나 오래 걸릴지 알고 있는 경우가 많습니다. 이러한 이유로 Laravel은 "타임아웃" 값을 지정할 수 있도록 합니다. 기본 타임아웃 값은 60초입니다. 작업이 타임아웃 값으로 지정된 초보다 오래 처리되면, 해당 작업을 처리하던 워커는 오류와 함께 종료됩니다. 일반적으로 워커는 [process manager configured on your server](#supervisor-configuration)에 의해 자동으로 다시 시작됩니다.
...
작업 클래스에 `Timeout` 속성을 사용하여 작업 실행을 허용할 최대 초 수를 정의할 수도 있습니다. 작업에 타임아웃이 지정되어 있으면, 명령줄에 지정된 어떤 타임아웃보다 우선합니다.
...
때로는 소켓이나 외부 HTTP 연결 같은 IO 블로킹 프로세스가 지정한 타임아웃을 따르지 않을 수 있습니다. 따라서 이러한 기능을 사용할 때는 해당 API에서도 항상 타임아웃을 지정해야 합니다. 예를 들어 [Guzzle](https://docs.guzzlephp.org)을 사용할 때는 연결 타임아웃과 요청 타임아웃 값을 항상 지정해야 합니다.

> [!WARNING]
> 작업 타임아웃을 지정하려면 [PCNTL](https://www.php.net/manual/en/book.pcntl.php) PHP 확장이 설치되어 있어야 합니다. 또한 작업의 "timeout" 값은 항상 ["retry after"](#job-expiration) 값보다 작아야 합니다. 그렇지 않으면 작업이 실제로 실행을 마치거나 타임아웃되기 전에 다시 시도될 수 있습니다.
...
타임아웃 시 작업을 [failed](#dealing-with-failed-jobs)로 표시하고 싶다면, 작업 클래스에 `FailOnTimeout` 속성을 사용할 수 있습니다.
...
> [!NOTE]
> 기본적으로 작업이 타임아웃되면 시도 횟수 하나가 소비되고, 재시도가 허용된 경우 다시 큐로 반환됩니다. 하지만 작업을 타임아웃 시 실패하도록 설정하면 tries에 설정된 값과 관계없이 재시도되지 않습니다.
...
Laravel은 [Amazon SQS FIFO (First-In-First-Out)](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-fifo-queues.html) 큐를 지원합니다. 이를 통해 메시지 중복 제거로 정확히 한 번만 처리되도록 보장하면서, 작업을 전송된 정확한 순서대로 처리할 수 있습니다.
...
Laravel은 작업을 디스패치할 때 메시지 그룹 ID를 지정할 수 있는 유창한 `onGroup` 메서드를 제공합니다.
...
SQS FIFO 큐는 정확히 한 번만 처리되도록 메시지 중복 제거를 지원합니다. 사용자 정의 중복 제거 ID를 제공하려면 작업 클래스에 `deduplicationId` 메서드를 구현하십시오.
...
<!-- To define the message group for a [queued event listener](/docs/master/events#queued-event-listeners), define a `messageGroup` method on the listener. You may also optionally define a `deduplicationId` method: -->
[queued event listener](/docs/master/events#queued-event-listeners)의 메시지 그룹을 정의하려면 리스너에 `messageGroup` 메서드를 정의하십시오. 선택 사항으로 `deduplicationId` 메서드도 정의할 수 있습니다.
...
<!-- When sending a [mail message](/docs/master/mail) that is going to be queued on a FIFO queue, you should invoke the `onGroup` method and optionally the `withDeduplicator` method when sending the notification: -->
FIFO 큐에 등록될 [mail message](/docs/master/mail)를 보낼 때는 메일을 보낼 때 `onGroup` 메서드를 호출하고, 선택 사항으로 `withDeduplicator` 메서드를 호출해야 합니다.
...
<!-- When sending a [notification](/docs/master/notifications) that is going to be queued on a FIFO queue, you should invoke the `onGroup` method and optionally the `withDeduplicator` method when sending the notification: -->
FIFO 큐에 등록될 [notification](/docs/master/notifications)을 보낼 때는 알림을 보낼 때 `onGroup` 메서드를 호출하고, 선택 사항으로 `withDeduplicator` 메서드를 호출해야 합니다.
...
`failover` 큐 드라이버는 작업을 큐에 푸시할 때 자동 페일오버 기능을 제공합니다. `failover` 설정의 기본 큐 연결이 어떤 이유로든 실패하면, Laravel은 목록에 설정된 다음 연결로 작업을 푸시하려고 자동으로 시도합니다. 이는 큐 안정성이 중요한 프로덕션 환경에서 높은 가용성을 보장하는 데 특히 유용합니다.
...
페일오버 큐 연결을 설정하려면 `failover` 드라이버를 지정하고, 순서대로 시도할 연결 이름 배열을 제공하십시오. 기본적으로 Laravel은 애플리케이션의 `config/queue.php` 설정 파일에 예시 페일오버 설정을 포함합니다.
...
`failover` 드라이버를 사용하는 연결을 설정한 뒤에는, 페일오버 기능을 사용하기 위해 애플리케이션의 `.env` 파일에서 페일오버 연결을 기본 큐 연결로 설정해야 합니다.
...
다음으로, 페일오버 연결 목록의 각 연결마다 최소 하나의 워커를 시작하십시오.
...
> [!NOTE]
> `sync`, `background`, 또는 `deferred` 큐 드라이버를 사용하는 연결은 현재 PHP 프로세스 안에서 작업을 처리하므로 워커를 실행할 필요가 없습니다.
...
큐 연결 작업이 실패하여 페일오버가 활성화되면, Laravel은 `Illuminate\Queue\Events\QueueFailedOver` 이벤트를 디스패치합니다. 이를 통해 큐 연결 실패를 보고하거나 로그로 남길 수 있습니다.

> [!NOTE]
> Laravel Horizon을 사용하는 경우, Horizon은 Redis 큐만 관리한다는 점을 기억하십시오. 페일오버 목록에 `database`가 포함되어 있다면 Horizon과 함께 일반 `php artisan queue:work database` 프로세스를 실행해야 합니다.
...
작업이 처리되는 동안 예외가 발생하면, 해당 작업은 다시 시도될 수 있도록 자동으로 큐에 반환됩니다. 작업은 애플리케이션에서 허용한 최대 시도 횟수에 도달할 때까지 계속 반환됩니다. 최대 시도 횟수는 `queue:work` Artisan 명령어에서 사용하는 `--tries` 스위치로 정의됩니다. 또는 작업 클래스 자체에 최대 시도 횟수를 정의할 수도 있습니다. 큐 워커 실행에 대한 자세한 내용은 [can be found below](#running-the-queue-worker).
...
가끔 작업을 나중에 다시 시도할 수 있도록 수동으로 큐에 다시 반환하고 싶을 수 있습니다. 이 작업은 `release` 메서드를 호출하여 수행할 수 있습니다.
...
기본적으로 `release` 메서드는 작업을 큐에 다시 반환하여 즉시 처리할 수 있게 합니다. 그러나 `release` 메서드에 정수 또는 날짜 인스턴스를 전달하면, 지정한 초가 지난 뒤에야 작업을 처리할 수 있도록 큐에 지시할 수 있습니다.
...
때로는 작업을 수동으로 "실패" 상태로 표시해야 할 수 있습니다. 이때는 `fail` 메서드를 호출하면 됩니다.
...
잡아낸 예외 때문에 작업을 실패로 표시하려면 해당 예외를 `fail` 메서드에 전달할 수 있습니다. 또는 편의를 위해 문자열 오류 메시지를 전달할 수도 있으며, 이 문자열은 자동으로 예외로 변환됩니다.
...
> [!NOTE]
> 실패한 작업에 대한 자세한 내용은 [documentation on dealing with job failures](#dealing-with-failed-jobs)를 확인하세요.
...
`FailOnException` [job middleware](#job-middleware)는 특정 예외가 던져졌을 때 재시도를 즉시 중단할 수 있게 해 줍니다. 이를 통해 외부 API 오류처럼 일시적인 예외에는 재시도하고, 사용자의 권한이 취소되는 경우처럼 지속적인 예외에는 작업을 영구적으로 실패 처리할 수 있습니다.
...
Laravel의 작업 배치 기능을 사용하면 여러 작업을 하나의 배치로 쉽게 실행하고, 해당 작업 배치의 실행이 완료되었을 때 특정 동작을 수행할 수 있습니다. 시작하기 전에 작업 배치에 대한 메타 정보, 예를 들어 완료율 같은 정보를 담을 테이블을 만들기 위한 데이터베이스 마이그레이션을 생성해야 합니다. 이 마이그레이션은 `make:queue-batches-table` Artisan 명령어로 생성할 수 있습니다.
...
배치 가능한 작업을 정의하려면 일반적인 방식으로 [create a queueable job](#creating-jobs)을 생성하면 됩니다. 다만 작업 클래스에 `Illuminate\Bus\Batchable` trait를 추가해야 합니다. 이 trait는 현재 작업이 실행 중인 배치를 가져오는 데 사용할 수 있는 `batch` 메서드를 제공합니다.
...
작업 배치를 디스패치하려면 `Bus` 파사드의 `batch` 메서드를 사용해야 합니다. 물론 배치는 주로 완료 콜백과 함께 사용할 때 유용합니다. 따라서 `then`, `catch`, `finally` 메서드를 사용하여 배치의 완료 콜백을 정의할 수 있습니다. 이 콜백들은 호출될 때 각각 `Illuminate\Bus\Batch` 인스턴스를 전달받습니다.
...
여러 큐 워커를 실행하는 경우, 배치 안의 작업은 병렬로 처리됩니다. 따라서 작업이 완료되는 순서는 작업을 배치에 추가한 순서와 다를 수 있습니다. 여러 작업을 순차적으로 실행하는 방법은 [job chains and batches](#chains-and-batches) 문서를 참고하세요.

<!-- In this example, we will imagine we are queueing a batch of jobs that each process a given number of rows from a CSV file: -->
이 예제에서는 CSV 파일에서 지정된 수의 행을 각각 처리하는 작업 배치를 큐에 넣는다고 가정해 보겠습니다.
...
`$batch->id` 속성으로 접근할 수 있는 배치의 ID는 배치가 디스패치된 후 해당 배치에 대한 정보를 얻기 위해 [query the Laravel command bus](#inspecting-batches)에 사용할 수 있습니다.
...
<!-- Some tools such as [Laravel Horizon](/docs/master/horizon) and [Laravel Telescope](/docs/master/telescope) may provide more user-friendly debug information for batches if batches are named. To assign an arbitrary name to a batch, you may call the `name` method while defining the batch: -->
[Laravel Horizon](/docs/master/horizon), [Laravel Telescope](/docs/master/telescope) 같은 도구는 배치에 이름이 지정되어 있으면 더 사용자 친화적인 디버그 정보를 제공할 수 있습니다. 배치에 임의의 이름을 지정하려면 배치를 정의하는 동안 `name` 메서드를 호출하면 됩니다.
...
체인으로 연결된 작업을 배열 안에 배치하여, 하나의 배치 안에서 [chained jobs](#job-chaining) 집합을 정의할 수 있습니다. 예를 들어 두 개의 작업 체인을 병렬로 실행하고, 두 작업 체인이 모두 처리를 마쳤을 때 콜백을 실행할 수 있습니다.
...
반대로, [chain](#job-chaining) 안에 배치를 정의하여 작업 배치를 실행할 수도 있습니다. 예를 들어 먼저 여러 팟캐스트를 공개하는 작업 배치를 실행한 다음, 공개 알림을 보내는 작업 배치를 실행할 수 있습니다.
...
때로는 배치에 포함된 작업 안에서 같은 배치에 추가 작업을 더하는 것이 유용할 수 있습니다. 이 패턴은 수천 개의 작업을 배치해야 해서 웹 요청 중에 모두 디스패치하기에는 시간이 너무 오래 걸릴 수 있는 경우에 유용합니다. 이런 경우 먼저 배치에 더 많은 작업을 채워 넣는 "로더" 작업으로 초기 배치를 디스패치할 수 있습니다.
...
이 예제에서는 `LoadImportBatch` 작업을 사용해 배치에 추가 작업을 채워 넣습니다. 이를 위해 작업의 `batch` 메서드로 접근할 수 있는 배치 인스턴스에서 `add` 메서드를 사용할 수 있습니다.
...
배치 완료 콜백에 전달되는 `Illuminate\Bus\Batch` 인스턴스는 특정 작업 배치와 상호작용하고 이를 검사하는 데 도움이 되는 다양한 속성과 메서드를 제공합니다.
...
모든 `Illuminate\Bus\Batch` 인스턴스는 JSON으로 직렬화할 수 있습니다. 즉, 애플리케이션의 라우트에서 이를 직접 반환하여 완료 진행률을 포함한 배치 정보를 담은 JSON 페이로드를 가져올 수 있습니다. 이를 통해 애플리케이션 UI에서 배치 완료 진행률에 대한 정보를 편리하게 표시할 수 있습니다.
...
때로는 특정 배치의 실행을 취소해야 할 수 있습니다. 이 작업은 `Illuminate\Bus\Batch` 인스턴스에서 `cancel` 메서드를 호출하여 수행할 수 있습니다.
...
앞선 예제에서 보았듯이, 배치 작업은 일반적으로 계속 실행하기 전에 자신이 속한 배치가 취소되었는지 확인해야 합니다. 하지만 편의를 위해 작업에 `SkipIfBatchCancelled` [middleware](#job-middleware)를 지정할 수도 있습니다. 이름에서 알 수 있듯이, 이 Middleware는 해당 작업이 속한 배치가 취소된 경우 Laravel이 그 작업을 처리하지 않도록 지시합니다.
...
배치에 포함된 작업이 실패하면 `catch` 콜백이 지정되어 있는 경우 해당 콜백이 호출됩니다. 이 콜백은 배치 안에서 처음 실패한 작업에 대해서만 호출됩니다.
...
배치 내의 작업이 실패하면 Laravel은 자동으로 해당 배치를 "취소됨"으로 표시합니다. 원한다면 이 동작을 비활성화하여 작업 실패가 배치를 자동으로 취소됨으로 표시하지 않도록 할 수 있습니다. 배치를 디스패치할 때 `allowFailures` 메서드를 호출하면 됩니다.
...
편의를 위해 Laravel은 주어진 배치에서 실패한 모든 작업을 쉽게 재시도할 수 있는 `queue:retry-batch` Artisan 명령어를 제공합니다. 이 명령어는 실패한 작업을 재시도할 배치의 UUID를 인수로 받습니다.
...
정리하지 않으면 `job_batches` 테이블에 레코드가 매우 빠르게 쌓일 수 있습니다. 이를 완화하려면 `queue:prune-batches` Artisan 명령어가 매일 실행되도록 [schedule](/docs/master/scheduling)해야 합니다.
...
기본적으로 완료된 지 24시간이 지난 모든 배치는 정리됩니다. 명령어를 호출할 때 `hours` 옵션을 사용하여 배치 데이터를 얼마나 오래 보관할지 지정할 수 있습니다. 예를 들어 다음 명령어는 48시간보다 오래전에 완료된 모든 배치를 삭제합니다.
...
때로는 작업이 실패했고 그 작업이 성공적으로 재시도되지 않은 배치처럼, 정상적으로 완료되지 않은 배치 레코드가 `jobs_batches` 테이블에 쌓일 수 있습니다. `unfinished` 옵션을 사용하면 `queue:prune-batches` 명령어가 이러한 미완료 배치 레코드를 정리하도록 지시할 수 있습니다.
...
마찬가지로 취소된 배치의 배치 레코드도 `jobs_batches` 테이블에 쌓일 수 있습니다. `cancelled` 옵션을 사용하면 `queue:prune-batches` 명령어가 이러한 취소된 배치 레코드를 정리하도록 지시할 수 있습니다.
...
일반적으로 이 테이블의 이름은 `job_batches`여야 하지만, 애플리케이션의 `queue` 설정 파일에 있는 `queue.batching.table` 설정값을 기준으로 테이블 이름을 정해야 합니다.
...
`job_batches` 테이블에는 `application`이라는 문자열 기본 파티션 키와 `id`라는 문자열 기본 정렬 키가 있어야 합니다. 키의 `application` 부분에는 애플리케이션의 `app` 설정 파일에 있는 `name` 설정값으로 정의된 애플리케이션 이름이 들어갑니다. 애플리케이션 이름이 DynamoDB 테이블 키의 일부이므로, 동일한 테이블을 사용해 여러 Laravel 애플리케이션의 작업 배치를 저장할 수 있습니다.
...
그런 다음 `queue.batching.driver` 설정 옵션의 값을 `dynamodb`로 설정합니다. 또한 `batching` 설정 배열 안에 `key`, `secret`, `region` 설정 옵션을 정의해야 합니다. 이 옵션들은 AWS 인증에 사용됩니다. `dynamodb` 드라이버를 사용할 때는 `queue.batching.database` 설정 옵션이 필요하지 않습니다.
...
작업 배치 정보를 저장하는 데 [DynamoDB](https://aws.amazon.com/dynamodb)를 사용하는 경우, 관계형 데이터베이스에 저장된 배치를 정리할 때 사용하는 일반적인 정리 명령어는 동작하지 않습니다. 대신 [DynamoDB's native TTL functionality](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/TTL.html)을 사용하여 오래된 배치 레코드를 자동으로 제거할 수 있습니다.
...
DynamoDB 테이블에 `ttl` 속성을 정의했다면, Laravel이 배치 레코드를 어떻게 정리할지 알려주는 설정 파라미터를 정의할 수 있습니다. `queue.batching.ttl_attribute` 설정값은 TTL을 담고 있는 속성의 이름을 정의하고, `queue.batching.ttl` 설정값은 레코드가 마지막으로 업데이트된 시점을 기준으로 몇 초가 지난 뒤 해당 배치 레코드를 DynamoDB 테이블에서 제거할 수 있는지를 정의합니다.
...
작업 클래스를 큐에 디스패치하는 대신 클로저를 디스패치할 수도 있습니다. 이는 현재 요청 사이클 밖에서 실행되어야 하는 빠르고 간단한 작업에 적합합니다. 클로저를 큐에 디스패치할 때는 클로저의 코드 내용이 암호학적으로 서명되므로 전송 중에 수정될 수 없습니다.
...
큐 리포팅 대시보드에서 사용할 수 있고 `queue:work` 명령어에서도 표시될 수 있는 이름을 큐에 들어간 클로저에 지정하려면 `name` 메서드를 사용할 수 있습니다.
...
`catch` 메서드를 사용하면 큐에 들어간 클로저가 큐의 [configured retry attempts](#max-job-attempts-and-timeout)를 모두 소진한 뒤에도 성공적으로 완료되지 못했을 때 실행될 클로저를 제공할 수 있습니다.
...
> [!WARNING]
> `catch` 콜백은 직렬화된 뒤 나중에 Laravel 큐에서 실행되므로, `catch` 콜백 안에서 `$this` 변수를 사용하면 안 됩니다.
...
Laravel에는 큐 워커를 시작하고 새 작업이 큐에 추가될 때 처리하는 Artisan 명령어가 포함되어 있습니다. `queue:work` Artisan 명령어를 사용하여 워커를 실행할 수 있습니다. `queue:work` 명령어가 시작되면 수동으로 중지하거나 터미널을 닫을 때까지 계속 실행된다는 점에 유의하세요.
...
> [!NOTE]
> `queue:work` 프로세스를 백그라운드에서 영구적으로 실행 상태로 유지하려면 [Supervisor](#supervisor-configuration) 같은 프로세스 모니터를 사용하여 큐 워커가 중지되지 않도록 해야 합니다.
...
큐 워커는 오래 실행되는 프로세스이며 부트된 애플리케이션 상태를 메모리에 저장한다는 점을 기억하세요. 따라서 시작된 뒤에는 코드베이스의 변경 사항을 인식하지 못합니다. 그러므로 배포 과정에서는 반드시 [restart your queue workers](#queue-workers-and-deployment)해야 합니다. 또한 애플리케이션에서 생성되거나 수정된 모든 정적 상태는 작업 사이에 자동으로 초기화되지 않는다는 점도 기억해야 합니다.
...
대안으로 `queue:listen` 명령어를 실행할 수도 있습니다. `queue:listen` 명령어를 사용하면 업데이트된 코드를 다시 로드하거나 애플리케이션 상태를 초기화하고 싶을 때 워커를 수동으로 재시작할 필요가 없습니다. 하지만 이 명령어는 `queue:work` 명령어보다 훨씬 비효율적입니다.
...
하나의 큐에 여러 워커를 할당하고 작업을 동시에 처리하려면 여러 `queue:work` 프로세스를 시작하면 됩니다. 로컬에서는 터미널의 여러 탭을 통해 실행할 수 있고, 프로덕션에서는 프로세스 관리자의 설정을 통해 실행할 수 있습니다. [When using Supervisor](#supervisor-configuration)는 `numprocs` 설정값을 사용할 수 있습니다.
...
기본적으로 `queue:work` 명령어는 주어진 연결의 기본 큐에 있는 작업만 처리합니다. 하지만 주어진 연결에서 특정 큐만 처리하도록 큐 워커를 더 세부적으로 사용자 정의할 수 있습니다. 예를 들어 모든 이메일이 `redis` 큐 연결의 `emails` 큐에서 처리된다면, 다음 명령어를 실행하여 해당 큐만 처리하는 워커를 시작할 수 있습니다.
...
`--max-jobs` 옵션을 사용하면 워커가 지정한 수의 작업을 처리한 뒤 종료하도록 지시할 수 있습니다. 이 옵션은 [Supervisor](#supervisor-configuration)와 함께 사용할 때 유용할 수 있습니다. 워커가 지정한 수의 작업을 처리한 뒤 자동으로 재시작되도록 하여, 그동안 누적되었을 수 있는 메모리를 해제할 수 있기 때문입니다.
...
`--stop-when-empty` 옵션을 사용하면 워커가 모든 작업을 처리한 뒤 정상적으로 종료하도록 지시할 수 있습니다. Docker 컨테이너 안에서 Laravel 큐를 처리하고 있으며 큐가 비면 컨테이너를 종료하고 싶은 경우에 유용할 수 있습니다.
...
`--max-time` 옵션을 사용하면 워커가 지정한 초 동안 작업을 처리한 뒤 종료하도록 지시할 수 있습니다. 이 옵션은 [Supervisor](#supervisor-configuration)와 함께 사용할 때 유용할 수 있습니다. 워커가 지정한 시간 동안 작업을 처리한 뒤 자동으로 재시작되도록 하여, 그동안 누적되었을 수 있는 메모리를 해제할 수 있기 때문입니다.
...
큐에 작업이 있으면 워커는 작업 사이에 지연 없이 계속 작업을 처리합니다. 하지만 사용 가능한 작업이 없을 때 워커가 몇 초 동안 "대기"할지는 `sleep` 옵션이 결정합니다. 물론 대기 중에는 워커가 새 작업을 처리하지 않습니다.
...
<!-- While your application is in [maintenance mode](/docs/master/configuration#maintenance-mode), no queued jobs will be handled. The jobs will continue to be handled as normal once the application is out of maintenance mode. -->
애플리케이션이 [maintenance mode](/docs/master/configuration#maintenance-mode)에 있는 동안에는 큐에 들어간 작업이 처리되지 않습니다. 애플리케이션이 유지보수 모드에서 벗어나면 작업은 평소처럼 계속 처리됩니다.

<!-- To force your queue workers to process jobs even if maintenance mode is enabled, you may use `--force` option: -->
유지보수 모드가 활성화되어 있더라도 큐 워커가 작업을 처리하도록 강제하려면 `--force` 옵션을 사용할 수 있습니다.
...
데몬 큐 워커는 각 작업을 처리하기 전에 프레임워크를 "재부팅"하지 않습니다. 따라서 각 작업이 완료된 뒤에는 무거운 리소스를 해제해야 합니다. 예를 들어 [GD library](https://www.php.net/manual/en/book.image.php)로 이미지 조작을 수행한다면, 이미지 처리가 끝났을 때 `imagedestroy`로 메모리를 해제해야 합니다.
...
때로는 큐가 처리되는 방식을 우선순위에 따라 조정하고 싶을 수 있습니다. 예를 들어 `config/queue.php` 설정 파일에서 `redis` 연결의 기본 `queue`를 `low`로 설정할 수 있습니다. 하지만 가끔은 다음과 같이 작업을 `high` 우선순위 큐에 넣고 싶을 수 있습니다.
...
`low` 큐의 작업으로 넘어가기 전에 모든 `high` 큐 작업이 처리되도록 확인하는 워커를 시작하려면, 쉼표로 구분된 큐 이름 목록을 `work` 명령어에 전달합니다.
...
큐 워커는 오래 실행되는 프로세스이므로 재시작하지 않으면 코드 변경 사항을 인식하지 못합니다. 따라서 큐 워커를 사용하는 애플리케이션을 배포하는 가장 간단한 방법은 배포 과정에서 워커를 재시작하는 것입니다. `queue:restart` 명령어를 실행하면 모든 워커를 정상적으로 재시작할 수 있습니다.
...
이 명령어는 기존 작업이 손실되지 않도록 모든 큐 워커에게 현재 처리 중인 작업을 끝낸 뒤 정상적으로 종료하라고 지시합니다. `queue:restart` 명령어가 실행되면 큐 워커가 종료되므로, [Supervisor](#supervisor-configuration) 같은 프로세스 관리자를 실행하여 큐 워커를 자동으로 재시작하도록 해야 합니다.

> [!NOTE]
> 큐는 재시작 신호를 저장하기 위해 [cache](/docs/master/cache)를 사용하므로, 이 기능을 사용하기 전에 애플리케이션에 캐시 드라이버가 올바르게 설정되어 있는지 확인해야 합니다.
...
`config/queue.php` 설정 파일에서 각 큐 연결은 `retry_after` 옵션을 정의합니다. 이 옵션은 처리 중인 작업을 다시 시도하기 전에 큐 연결이 몇 초 동안 기다려야 하는지를 지정합니다. 예를 들어 `retry_after` 값이 `90`으로 설정되어 있으면, 작업이 90초 동안 처리 중이면서 큐에 반환되거나 삭제되지 않은 경우 해당 작업은 큐로 다시 반환됩니다. 일반적으로 `retry_after` 값은 작업이 처리 완료에 합리적으로 걸릴 수 있는 최대 초 단위 시간으로 설정해야 합니다.

> [!WARNING]
> `retry_after` 값이 없는 유일한 큐 연결은 Amazon SQS입니다. SQS는 AWS 콘솔에서 관리되는 [Default Visibility Timeout](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/AboutVT.html)을 기준으로 작업을 다시 시도합니다.
...
`queue:work` Artisan 명령어는 `--timeout` 옵션을 제공합니다. 기본적으로 `--timeout` 값은 60초입니다. 작업이 타임아웃 값으로 지정된 초보다 오래 처리되고 있으면, 해당 작업을 처리 중인 워커는 오류와 함께 종료됩니다. 일반적으로 워커는 [process manager configured on your server](#supervisor-configuration)에 의해 자동으로 재시작됩니다.
...
`retry_after` 설정 옵션과 `--timeout` CLI 옵션은 서로 다르지만, 함께 동작하여 작업이 손실되지 않고 작업이 한 번만 성공적으로 처리되도록 보장합니다.

> [!WARNING]
> `--timeout` 값은 항상 `retry_after` 설정값보다 최소 몇 초 더 짧아야 합니다. 이렇게 해야 멈춘 작업을 처리 중인 워커가 해당 작업이 재시도되기 전에 항상 종료됩니다. `--timeout` 옵션이 `retry_after` 설정값보다 길면 작업이 두 번 처리될 수 있습니다.
...
때로는 큐 워커를 완전히 중지하지 않고 새 작업 처리를 일시적으로 막아야 할 수 있습니다. 예를 들어 시스템 유지보수 중에는 작업 처리를 일시 중지하고 싶을 수 있습니다. Laravel은 큐 워커를 일시 중지하고 재개하기 위한 `queue:pause` 및 `queue:continue` Artisan 명령어를 제공합니다.

<!-- To pause a specific queue, provide the queue connection name and the queue name: -->
특정 큐를 일시 중지하려면 큐 연결 이름과 큐 이름을 제공합니다.
...
이 예에서 `database`는 큐 연결 이름이고 `default`는 큐 이름입니다. 큐가 일시 중지되면 해당 큐의 작업을 처리 중인 모든 워커는 현재 작업을 계속 완료하지만, 큐가 재개될 때까지 새 작업을 가져오지 않습니다.
...
큐를 재개한 뒤에는 워커가 즉시 해당 큐의 새 작업을 처리하기 시작합니다. 큐를 일시 중지해도 워커 프로세스 자체가 중지되는 것은 아니라는 점에 유의하세요. 지정한 큐에서 새 작업을 처리하지 못하게 할 뿐입니다.
...
기본적으로 큐 워커는 각 작업 반복마다 캐시 드라이버에서 재시작 및 일시 중지 신호를 폴링합니다. 이 폴링은 `queue:restart` 및 `queue:pause` 명령어에 응답하기 위해 필수적이지만, 약간의 성능 오버헤드를 유발합니다.
...
성능을 최적화해야 하고 이러한 중단 기능이 필요하지 않다면, `Queue` 파사드에서 `withoutInterruptionPolling` 메서드를 호출하여 이 폴링을 전역으로 비활성화할 수 있습니다. 일반적으로 이 작업은 `AppServiceProvider`의 `boot` 메서드에서 수행해야 합니다.
...
또는 `Illuminate\Queue\Worker` 클래스의 정적 `$restartable` 또는 `$pausable` 속성을 설정하여 재시작 폴링이나 일시 중지 폴링을 개별적으로 비활성화할 수 있습니다.
...
> [!WARNING]
> 중단 폴링이 비활성화되면 워커는 `queue:restart` 또는 `queue:pause` 명령어에 응답하지 않습니다. 어떤 기능이 비활성화되었는지에 따라 달라집니다.
...
프로덕션에서는 `queue:work` 프로세스를 계속 실행 상태로 유지할 방법이 필요합니다. `queue:work` 프로세스는 워커 타임아웃 초과나 `queue:restart` 명령어 실행 등 다양한 이유로 실행이 중지될 수 있습니다.

<!-- For this reason, you need to configure a process monitor that can detect when your `queue:work` processes exit and automatically restart them. In addition, process monitors can allow you to specify how many `queue:work` processes you would like to run concurrently. Supervisor is a process monitor commonly used in Linux environments and we will discuss how to configure it in the following documentation. -->
이 때문에 `queue:work` 프로세스가 종료되었는지 감지하고 자동으로 재시작할 수 있는 프로세스 모니터를 설정해야 합니다. 또한 프로세스 모니터를 사용하면 동시에 실행할 `queue:work` 프로세스의 수를 지정할 수 있습니다. Supervisor는 Linux 환경에서 흔히 사용되는 프로세스 모니터이며, 다음 문서에서 설정 방법을 살펴보겠습니다.
...
Supervisor는 Linux 운영체제용 프로세스 모니터이며, `queue:work` 프로세스가 실패하면 자동으로 재시작합니다. Ubuntu에 Supervisor를 설치하려면 다음 명령어를 사용할 수 있습니다.
...
> [!NOTE]
> Supervisor를 직접 설정하고 관리하는 일이 부담스럽다면, Laravel 큐 워커 실행을 위한 완전 관리형 플랫폼을 제공하는 [Laravel Cloud](https://cloud.laravel.com) 사용을 고려해 보세요.
...
Supervisor 설정 파일은 일반적으로 `/etc/supervisor/conf.d` 디렉터리에 저장됩니다. 이 디렉터리 안에 supervisor가 프로세스를 어떻게 모니터링해야 하는지 지시하는 설정 파일을 원하는 수만큼 만들 수 있습니다. 예를 들어 `queue:work` 프로세스를 시작하고 모니터링하는 `laravel-worker.conf` 파일을 만들어 보겠습니다.
```ini
...
이 예제에서 `numprocs` 지시어는 Supervisor에게 여덟 개의 `queue:work` 프로세스를 실행하고 모두 모니터링하도록 지시하며, 실패하면 자동으로 다시 시작합니다. 원하는 큐 연결과 워커 옵션을 반영하도록 설정의 `command` 지시어를 변경해야 합니다.

> [!WARNING]
> `stopwaitsecs` 값은 가장 오래 실행되는 작업이 소요하는 초보다 크게 설정해야 합니다. 그렇지 않으면 Supervisor가 작업 처리가 끝나기 전에 해당 작업을 종료할 수 있습니다.
...
설정 파일이 생성되면 다음 명령어를 사용하여 Supervisor 설정을 업데이트하고 프로세스를 시작할 수 있습니다.
...
큐에 들어간 작업은 때때로 실패할 수 있습니다. 걱정하지 마십시오. 모든 일이 항상 계획대로 진행되지는 않습니다! Laravel은 [specify the maximum number of times a job should be attempted](#max-job-attempts-and-timeout)하는 편리한 방법을 제공합니다. 비동기 작업이 이 시도 횟수를 초과하면 `failed_jobs` 데이터베이스 테이블에 삽입됩니다. 실패한 [Synchronously dispatched jobs](/docs/master/queues#synchronous-dispatching)은 이 테이블에 저장되지 않으며, 해당 예외는 애플리케이션에서 즉시 처리됩니다.
...
새 Laravel 애플리케이션에는 일반적으로 `failed_jobs` 테이블을 생성하는 마이그레이션이 이미 포함되어 있습니다. 하지만 애플리케이션에 이 테이블을 위한 마이그레이션이 없다면 `make:queue-failed-table` 명령어를 사용하여 마이그레이션을 생성할 수 있습니다.
...
`--backoff` 옵션을 사용하면 예외가 발생한 작업을 다시 시도하기 전에 Laravel이 몇 초 동안 기다릴지 지정할 수 있습니다. 기본적으로 작업은 즉시 다시 시도될 수 있도록 큐에 다시 반환됩니다.
...
예외가 발생한 작업을 다시 시도하기 전에 Laravel이 기다릴 시간을 작업별로 설정하려면 작업 클래스에 `Backoff` 속성을 사용할 수 있습니다.
...
작업의 백오프 시간을 결정하는 데 더 복잡한 로직이 필요하다면 작업 클래스에 `backoff` 메서드를 정의할 수 있습니다.
...
백오프 값 배열을 정의하여 "지수적" 백오프를 쉽게 설정할 수 있습니다. 이 예제에서는 첫 번째 재시도는 1초, 두 번째 재시도는 5초, 세 번째 재시도는 10초를 지연하며, 남은 시도가 더 있다면 이후의 모든 재시도는 10초를 지연합니다.
...
특정 작업이 실패하면 사용자에게 알림을 보내거나 작업에서 부분적으로 완료한 동작을 되돌리고 싶을 수 있습니다. 이를 위해 작업 클래스에 `failed` 메서드를 정의할 수 있습니다. 작업 실패의 원인이 된 `Throwable` 인스턴스가 `failed` 메서드로 전달됩니다.
...
> [!WARNING]
> `failed` 메서드를 호출하기 전에 작업의 새 인스턴스가 생성됩니다. 따라서 `handle` 메서드 안에서 발생했을 수 있는 클래스 속성 변경 사항은 모두 사라집니다.
...
- 작업이 수동으로 또는 middleware에 의해 큐로 다시 반환되었습니다.
...
마지막 시도가 작업 실행 중 발생한 예외 때문에 실패하면 해당 예외가 작업의 `failed` 메서드로 전달됩니다. 하지만 허용된 최대 시도 횟수에 도달하여 작업이 실패한 경우 `$exception`은 `Illuminate\Queue\MaxAttemptsExceededException`의 인스턴스가 됩니다. 마찬가지로 설정된 타임아웃을 초과하여 작업이 실패한 경우 `$exception`은 `Illuminate\Queue\TimeoutExceededException`의 인스턴스가 됩니다.
...
`queue:failed` 명령어는 작업 ID, 연결, 큐, 실패 시간, 그리고 작업에 대한 기타 정보를 나열합니다. 작업 ID는 실패한 작업을 다시 시도하는 데 사용할 수 있습니다. 예를 들어 ID가 `ce7bb17c-cdd8-41f0-a8ec-7b4fef4e5ece`인 실패한 작업을 다시 시도하려면 다음 명령어를 실행하십시오.
...
모든 실패한 작업을 다시 시도하려면 `queue:retry` 명령어를 실행하고 ID로 `all`을 전달하십시오.
...
> [!NOTE]
> [Horizon](/docs/master/horizon)을 사용하는 경우 실패한 작업을 삭제할 때 `queue:forget` 명령어 대신 `horizon:forget` 명령어를 사용해야 합니다.
...
`queue:flush` 명령어는 실패한 작업이 얼마나 오래되었는지와 관계없이 큐에서 모든 실패한 작업 레코드를 제거합니다. `--hours` 옵션을 사용하면 특정 시간 이전에 실패한 작업만 삭제할 수 있습니다.
...
편의를 위해 작업 클래스에 `DeleteWhenMissingModels` 속성을 사용하여 누락된 모델이 있는 작업을 자동으로 삭제하도록 선택할 수 있습니다. 이 속성이 있으면 Laravel은 예외를 발생시키지 않고 작업을 조용히 버립니다.
...
기본적으로 24시간보다 오래된 모든 실패한 작업 레코드가 정리됩니다. 명령어에 `--hours` 옵션을 제공하면 최근 N시간 이내에 삽입된 실패한 작업 레코드만 유지됩니다. 예를 들어 다음 명령어는 48시간보다 오래전에 삽입된 모든 실패한 작업 레코드를 삭제합니다.
...
Laravel은 실패한 작업 레코드를 관계형 데이터베이스 테이블 대신 [DynamoDB](https://aws.amazon.com/dynamodb)에 저장하는 기능도 지원합니다. 하지만 모든 실패한 작업 레코드를 저장할 DynamoDB 테이블은 직접 생성해야 합니다. 일반적으로 이 테이블의 이름은 `failed_jobs`가 되어야 하지만, 애플리케이션의 `queue` 설정 파일 안에 있는 `queue.failed.table` 설정 값에 따라 테이블 이름을 지정해야 합니다.
...
`failed_jobs` 테이블에는 `application`이라는 문자열 기본 파티션 키와 `uuid`라는 문자열 기본 정렬 키가 있어야 합니다. 키의 `application` 부분에는 애플리케이션의 `app` 설정 파일 안에 있는 `name` 설정 값으로 정의된 애플리케이션 이름이 포함됩니다. 애플리케이션 이름이 DynamoDB 테이블 키의 일부이므로, 동일한 테이블을 사용하여 여러 Laravel 애플리케이션의 실패한 작업을 저장할 수 있습니다.
...
다음으로 `queue.failed.driver` 설정 옵션 값을 `dynamodb`로 설정하십시오. 또한 실패한 작업 설정 배열 안에 `key`, `secret`, `region` 설정 옵션을 정의해야 합니다. 이 옵션들은 AWS 인증에 사용됩니다. `dynamodb` 드라이버를 사용할 때는 `queue.failed.database` 설정 옵션이 필요하지 않습니다.
...
`queue.failed.driver` 설정 옵션 값을 `null`로 설정하여 Laravel이 실패한 작업을 저장하지 않고 버리도록 지시할 수 있습니다. 일반적으로 이는 `QUEUE_FAILED_DRIVER` 환경 변수를 통해 설정할 수 있습니다.
...
> [!NOTE]
> [Horizon](/docs/master/horizon)을 사용하는 경우 큐에서 작업을 삭제할 때 `queue:clear` 명령어 대신 `horizon:clear` 명령어를 사용해야 합니다.
...
특정 연결과 큐에서 작업을 삭제하려면 `connection` 인수와 `queue` 옵션을 제공할 수도 있습니다.
...
> [!WARNING]
> 큐에서 작업을 삭제하는 기능은 SQS, Redis, database 큐 드라이버에서만 사용할 수 있습니다. 또한 SQS 메시지 삭제 과정은 최대 60초가 걸리므로, 큐를 삭제한 뒤 최대 60초 동안 SQS 큐로 전송된 작업도 삭제될 수 있습니다.
...
큐에 작업이 갑자기 몰리면 큐가 과부하 상태가 되어 작업 완료까지 오래 기다려야 할 수 있습니다. 원한다면 큐의 작업 수가 지정한 임계값을 초과할 때 Laravel이 알림을 보내도록 할 수 있습니다.
...
시작하려면 `queue:monitor` 명령어가 [run every minute](/docs/master/scheduling)되도록 예약해야 합니다. 이 명령어는 모니터링하려는 큐 이름과 원하는 작업 수 임계값을 받습니다.
...
이 명령어를 예약하는 것만으로는 큐가 과부하 상태임을 알려 주는 알림이 트리거되지 않습니다. 명령어가 임계값을 초과하는 작업 수를 가진 큐를 발견하면 `Illuminate\Queue\Events\QueueBusy` 이벤트가 디스패치됩니다. 애플리케이션의 `AppServiceProvider` 안에서 이 이벤트를 수신하여 자신이나 개발팀에 알림을 보낼 수 있습니다.
...
작업을 디스패치하는 코드를 테스트할 때는 작업 자체를 실제로 실행하지 않도록 Laravel에 지시하고 싶을 수 있습니다. 작업의 코드는 해당 작업을 디스패치하는 코드와 별도로 직접 테스트할 수 있기 때문입니다. 물론 작업 자체를 테스트하려면 테스트에서 작업 인스턴스를 생성하고 `handle` 메서드를 직접 호출할 수 있습니다.
...
큐에 들어갈 작업이 실제로 큐에 푸시되지 않도록 하려면 `Queue` 파사드의 `fake` 메서드를 사용할 수 있습니다. `Queue` 파사드의 `fake` 메서드를 호출한 뒤에는 애플리케이션이 큐에 작업을 푸시하려고 시도했는지 단언할 수 있습니다.
...
`assertPushed`, `assertNotPushed`, `assertClosurePushed`, `assertClosureNotPushed` 메서드에 클로저를 전달하여, 주어진 "truth test(조건 검사)"를 통과하는 작업이 푸시되었는지 단언할 수 있습니다. 주어진 조건 검사를 통과하는 작업이 하나 이상 푸시되었다면 단언은 성공합니다:
...
다른 작업은 정상적으로 실행되도록 두면서 특정 작업만 페이크 처리해야 한다면, 페이크 처리할 작업의 클래스명을 `fake` 메서드에 전달할 수 있습니다:
...
`except` 메서드를 사용하면 지정한 작업 집합을 제외한 모든 작업을 페이크 처리할 수 있습니다:
...
작업 체인을 테스트하려면 `Bus` facade의 페이크 기능을 사용해야 합니다. `Bus` facade의 `assertChained` 메서드를 사용하여 [chain of jobs](/docs/master/queues#job-chaining)이 디스패치되었는지 단언할 수 있습니다. `assertChained` 메서드는 첫 번째 인수로 체인에 포함된 작업 배열을 받습니다:
...
위 예제에서 볼 수 있듯이, 체인 작업 배열은 작업의 클래스명 배열일 수 있습니다. 하지만 실제 작업 인스턴스 배열을 제공할 수도 있습니다. 이 경우 Laravel은 작업 인스턴스가 애플리케이션에서 디스패치한 체인 작업과 같은 클래스이고, 동일한 속성 값을 가지는지 확인합니다:
...
`assertDispatchedWithoutChain` 메서드를 사용하여 작업이 작업 체인 없이 푸시되었는지 단언할 수 있습니다:
...
체인 작업이 [prepends or appends jobs to an existing chain](#adding-jobs-to-the-chain)하는 경우, 작업의 `assertHasChain` 메서드를 사용하여 해당 작업에 예상한 남은 작업 체인이 있는지 단언할 수 있습니다:
...
`assertDoesntHaveChain` 메서드를 사용하여 작업의 남은 체인이 비어 있는지 단언할 수 있습니다:
...
작업 체인에 [contains a batch of jobs](#chains-and-batches)가 포함되어 있다면, 체인 단언 안에 `Bus::chainedBatch` 정의를 삽입하여 체인 배치가 예상과 일치하는지 단언할 수 있습니다:
...
`Bus` facade의 `assertBatched` 메서드를 사용하여 [batch of jobs](/docs/master/queues#job-batching)가 디스패치되었는지 단언할 수 있습니다. `assertBatched` 메서드에 전달하는 클로저는 `Illuminate\Bus\PendingBatch` 인스턴스를 받으며, 이를 사용해 배치 안의 작업을 검사할 수 있습니다:
...
대기 중인 배치에서 `hasJobs` 메서드를 사용하여 배치에 예상한 작업이 포함되어 있는지 확인할 수 있습니다. 이 메서드는 작업 인스턴스, 클래스명 또는 클로저 배열을 받습니다:
...
클로저를 사용할 때 클로저는 작업 인스턴스를 받습니다. 예상 작업 타입은 클로저의 타입 힌트에서 추론됩니다:
...
`assertBatchCount` 메서드를 사용하여 지정한 수의 배치가 디스패치되었는지 단언할 수 있습니다:
...
`assertNothingBatched`를 사용하여 어떤 배치도 디스패치되지 않았는지 단언할 수 있습니다:
...
또한 개별 작업이 그 작업이 속한 배치와 어떻게 상호작용하는지 테스트해야 할 때도 있습니다. 예를 들어, 작업이 해당 배치의 이후 처리를 취소했는지 테스트해야 할 수 있습니다. 이를 위해 `withFakeBatch` 메서드를 통해 작업에 가짜 배치를 할당해야 합니다. `withFakeBatch` 메서드는 작업 인스턴스와 가짜 배치를 포함하는 튜플을 반환합니다:
...
때로는 큐 작업이 [releases itself back onto the queue](#manually-releasing-a-job)하는지 테스트해야 할 수 있습니다. 또는 작업이 스스로 삭제되었는지 테스트해야 할 수도 있습니다. 이러한 큐 상호작용은 작업을 인스턴스화한 뒤 `withFakeQueueInteractions` 메서드를 호출하여 테스트할 수 있습니다.
...
작업의 큐 상호작용을 페이크 처리한 뒤에는 작업의 `handle` 메서드를 호출할 수 있습니다. 작업을 호출한 후에는 작업의 큐 상호작용을 확인하기 위한 다양한 단언 메서드를 사용할 수 있습니다:
...
`Queue` [facade](/docs/master/facades)의 `before` 및 `after` 메서드를 사용하여 큐 작업이 처리되기 전이나 후에 실행할 콜백을 지정할 수 있습니다. 이러한 콜백은 추가 로그를 남기거나 대시보드용 통계를 증가시키기에 좋은 위치입니다. 일반적으로 이 메서드들은 [service provider](/docs/master/providers)의 `boot` 메서드에서 호출해야 합니다. 예를 들어 Laravel에 포함된 `AppServiceProvider`를 사용할 수 있습니다:
...
`Queue` [facade](/docs/master/facades)의 `looping` 메서드를 사용하여 worker가 큐에서 작업을 가져오려고 시도하기 전에 실행되는 콜백을 지정할 수 있습니다. 예를 들어 이전에 실패한 작업 때문에 열려 있는 상태로 남은 트랜잭션을 롤백하는 클로저를 등록할 수 있습니다:
