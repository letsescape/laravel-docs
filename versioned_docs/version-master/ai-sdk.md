...
`FileSearch` 프로바이더 도구를 사용하면 에이전트가 [files](#vector-stores)에 저장된 [vector stores](#files)을 검색할 수 있습니다. 이를 통해 에이전트가 업로드한 문서에서 관련 정보를 검색할 수 있으므로 검색 증강 생성(RAG)이 가능합니다.
