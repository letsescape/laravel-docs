...
Laravel의 [AI SDK](/docs/master/ai-sdk)는 AI 모델을 사용해 쿼리에 대한 의미적 관련도 순으로 결과 집합을 재정렬하는 기능을 제공합니다. 재정렬 기능은 빠른 1차 검색 결과(예: 전문 검색) 이후, 관련성에 따라 결과를 다시 정렬하는 2차 처리로 사용할 때 특히 강력합니다. 이를 통해 속도와 의미 기반 정확도를 모두 얻을 수 있습니다.
...
Eloquent 모델과 검색 인덱스를 자동으로 동기화하는 `Searchable` 트레이트가 필요한 경우, [Laravel Scout](/docs/master/scout)는 내장 데이터베이스 엔진과 Algolia·Meilisearch·Typesense와 같은 외부 서비스용 드라이버를 모두 제공합니다.
...
인덱스 생성에 대한 더 자세한 정보는 [migration documentation](/docs/master/migrations#available-index-types)를 참고하십시오.
...
`orWhereFullText` 메서드를 사용하면 전문 검색 조건을 "or" 조건으로 추가할 수도 있습니다. 자세한 내용은 [query builder documentation](/docs/master/queries#full-text-where-clauses)를 참고하세요.
...
> [!NOTE]
> 벡터 검색은 PostgreSQL 데이터베이스의 `pgvector` 확장과 [Laravel AI SDK](/docs/master/ai-sdk)가 필요합니다. [Laravel Cloud](https://cloud.laravel.com)의 Serverless Postgres 데이터베이스에는 `pgvector`가 이미 포함되어 있습니다.
...
임베딩 provider 설정, 임베딩 차원 수 사용자 지정, 캐싱 등은 [AI SDK documentation](/docs/master/ai-sdk#embeddings)를 참고하세요.
...
벡터 컬럼과 인덱스에 대한 자세한 내용은 [migration documentation](/docs/master/migrations#available-column-types)를 참고하세요.
...
벡터 쿼리를 더 세밀하게 제어하고 싶다면 `whereVectorDistanceLessThan`, `selectVectorDistance`, `orderByVectorDistance` 메서드도 제공됩니다. 이들은 유사도(score) 대신 거리 값으로 작업해서, 거리 결과를 컬럼에 포함하거나 수동 정렬이 필요할 때 사용할 수 있습니다. 자세한 내용은 [query builder documentation](/docs/master/queries#vector-similarity-clauses) 및 [AI SDK documentation](/docs/master/ai-sdk#querying-embeddings)를 참고하세요.
...
리랭킹 provider 설정 및 옵션 등 자세한 내용은 [AI SDK documentation](/docs/master/ai-sdk#reranking)를 참고하세요.
...
위에서 소개한 검색 방식들은 코드에서 직접 쿼리 빌더 메서드를 호출해 사용하는 방법입니다. [Laravel Scout](/docs/master/scout)는 조금 다릅니다. Eloquent 모델에 `Searchable` 트레이트를 추가하면, Scout가 레코드 생성·수정·삭제 시 자동으로 검색 인덱스와 동기화해주기 때문에 모델이 항상 검색 가능하도록 관리가 편리합니다.
...
> [!WARNING]
> 특정 컬럼을 전문 쿼리로 지정하려면, 해당 컬럼에 [full-text index](/docs/master/migrations#available-index-types)가 반드시 추가되어 있어야 합니다.
...
데이터베이스 엔진은 일반적인 범위의 검색 요구에 적합하며, Scout의 인덱스 자동 동기화 편리함을 누리고자 할 때 적합합니다. 필터링, 페이지네이션, 소프트 삭제 레코드 처리 등 주요 검색 기능도 잘 지원합니다. 자세한 내용은 [Scout documentation](/docs/master/scout#database-engine)를 참고하세요.
...
서드파티 엔진 설정 등 자세한 내용은 [Scout documentation](/docs/master/scout)를 참고하십시오.
