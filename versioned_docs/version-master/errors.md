...
Laravel에서 예외 보고는 예외를 로그에 기록하거나 [Sentry](https://github.com/getsentry/sentry-laravel), [Flare](https://flareapp.io) 같은 외부 서비스로 보내는 데 사용됩니다. 기본적으로 예외는 [logging](/docs/master/logging) 설정에 따라 로그에 기록됩니다. 하지만 원하는 방식으로 자유롭게 예외를 기록할 수 있습니다.
...
> [!NOTE]
> 특정 예외의 예외 보고를 사용자 정의하려면 [reportable exceptions](/docs/master/errors#renderable-exceptions)를 활용할 수도 있습니다.
...
애플리케이션의 [logs](/docs/master/logging)에 메시지가 작성될 때, 메시지는 지정된 [log level](/docs/master/logging#log-levels)로 기록됩니다. 로그 레벨은 기록되는 메시지의 심각도나 중요도를 나타냅니다.
...
> [!NOTE]
> `report` 메서드에 필요한 의존성을 타입 힌트로 지정할 수 있으며, Laravel의 [service container](/docs/master/container)가 해당 의존성을 메서드에 자동으로 주입합니다.
