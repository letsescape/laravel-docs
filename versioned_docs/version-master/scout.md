...
[Laravel Scout](https://github.com/laravel/scout)는 [Eloquent models](/docs/master/eloquent)에 전체 텍스트 검색 기능을 간단하고 드라이버 기반으로 추가할 수 있는 솔루션을 제공합니다. Scout는 모델 옵저버를 활용하여 Eloquent 레코드와 검색 인덱스를 자동으로 동기화합니다.
...
Scout는 별도의 외부 서비스 없이 MySQL / PostgreSQL의 전체 텍스트 인덱스와 `LIKE` 절을 활용하는 내장 `database` 엔진을 기본 제공합니다. 대부분의 애플리케이션에서는 이 방식만으로도 충분합니다. Laravel에서 제공하는 다양한 검색 옵션의 개요는 [search documentation](/docs/master/search)를 참고하시기 바랍니다.
...
`database`나 `collection` 엔진이 아닌 다른 엔진을 사용할 경우, 라이브러리를 사용하기 전에 [queue driver](/docs/master/queues)를 반드시 설정하는 것이 좋습니다. 큐 워커를 실행하면 모델 정보를 검색 인덱스와 동기화하는 모든 작업을 큐로 처리하여, 애플리케이션 웹 인터페이스의 응답 속도를 대폭 향상시킬 수 있습니다.
...
[Meilisearch](https://www.meilisearch.com)는 빠르고 오픈 소스인 검색 엔진입니다. 로컬 환경에서 Meilisearch 설치 방법을 잘 모를 경우, Laravel의 공식 Docker 개발 환경인 [Laravel Sail](/docs/master/sail#meilisearch)을 활용할 수 있습니다.
...
[Laravel Sail](/docs/master/sail)을 사용하는 경우, `TYPESENSE_HOST` 값을 Docker 컨테이너 이름에 맞게 조정해야 할 수도 있습니다. 또한, 설치 환경의 포트, 경로, 프로토콜도 선택적으로 지정할 수 있습니다:
...
> [!WARNING]
> 컬럼에 전체 텍스트 쿼리를 적용하려면, 해당 컬럼에 [full text index](/docs/master/migrations#available-index-types)가 생성되어 있어야 합니다.
...
또는, [queued jobs](/docs/master/queues)으로 기존 레코드를 모두 임포트하려면 `scout:queue-import` 명령어를 사용할 수 있습니다:
...
> [!WARNING]
> 큐를 사용하여 배치 임포트할 때는 `makeAllSearchableUsing` 메서드가 적용되지 않을 수 있습니다. 큐 작업 처리 중에는 [not restored](/docs/master/queues#handling-relationships).
...
Eloquent 쿼리 결과로 가져온 모델 컬렉션을 검색 인덱스에 추가하려면, 쿼리 뒤에 `searchable` 메서드를 체이닝합니다. `searchable` 메서드는 [chunk the results](/docs/master/eloquent#chunking-results)하여 인덱스에 추가합니다. 큐 설정이 되어 있다면, 청크 전체가 큐 워커에 의해 백그라운드에서 처리됩니다:
...
인덱스에서 레코드를 삭제하려면 모델을 데이터베이스에서 `delete`하면 됩니다. [soft deleted](/docs/master/eloquent#soft-deleting) 모델도 마찬가지로 처리됩니다:
...
서드파티 엔진 사용 시에는 이 콜백이 이미 검색 엔진에서 모델을 받아온 후에 실행되므로, 필터링 용도가 아닌 추가 가공 용도로만 사용해야 합니다. 대신 [Scout where clauses](#where-clauses)을 사용하세요. 하지만 데이터베이스 엔진 사용 시에는 `query` 메서드의 제약이 바로 데이터베이스 쿼리에 반영되므로 필터링에도 활용할 수 있습니다.
...
모델 컬렉션을 단순히 조회하는 것 외에도, `paginate` 메서드로 검색 결과를 페이지네이션할 수 있습니다. 이 메서드는 [paginated a traditional Eloquent query](/docs/master/pagination)한 것과 동일하게 `Illuminate\Pagination\LengthAwarePaginator` 인스턴스를 반환합니다:
...
검색 결과와 페이지 링크는 [Blade](/docs/master/blade)에서 일반 Eloquent 페이지네이션처럼 렌더링할 수 있습니다:
...
인덱싱하는 모델이 [soft deleting](/docs/master/eloquent#soft-deleting)를 지원하고, 소프트 삭제된 모델도 검색 대상에 포함하려면, `config/scout.php`의 `soft_delete` 옵션을 `true`로 설정합니다:
