...
- [검색](#searching)
    ...
    - [엔진 검색 사용자 지정](#customizing-engine-searches)
- [커스텀 엔진](#custom-engines)

<a name="introduction"></a>
## 소개 (Introduction)

[Laravel Scout](https://github.com/laravel/scout)는 [Eloquent 모델](/docs/master/eloquent)에 전체 텍스트 검색 기능을 간단하고 드라이버 기반으로 추가할 수 있는 솔루션을 제공합니다. Scout는 모델 옵저버를 활용하여 Eloquent 레코드와 검색 인덱스를 자동으로 동기화합니다.

Scout는 별도의 외부 서비스 없이 MySQL / PostgreSQL의 전체 텍스트 인덱스와 `LIKE` 절을 활용하는 내장 `database` 엔진을 기본 제공합니다. 대부분의 애플리케이션에서는 이 방식만으로도 충분합니다. Laravel에서 제공하는 다양한 검색 옵션의 개요는 [검색 문서](/docs/master/search)를 참고하시기 바랍니다.
...
`database`나 `collection` 엔진이 아닌 다른 엔진을 사용할 경우, 라이브러리를 사용하기 전에 [큐 드라이버](/docs/master/queues)를 반드시 설정하는 것이 좋습니다. 큐 워커를 실행하면 모델 정보를 검색 인덱스와 동기화하는 모든 작업을 큐로 처리하여, 애플리케이션 웹 인터페이스의 응답 속도를 대폭 향상시킬 수 있습니다.
...
> [!WARNING]
> 컬럼에 전체 텍스트 쿼리를 적용하려면, 해당 컬럼에 [전체 텍스트 인덱스](/docs/master/migrations#available-index-types)가 생성되어 있어야 합니다.
...
Meilisearch에서 필터(>, < 등) 연산을 제대로 수행하려면 데이터의 타입이 정확해야 합니다. `toSearchableArray`를 사용자 지정할 때에는 숫자 값이 적절한 타입으로 캐스팅되어야 합니다:
...
또는, [큐 작업](/docs/master/queues)으로 배치 임포트를 진행하려면 `scout:queue-import` 명령어를 사용할 수 있으며, `--chunk=500` 옵션 등으로 한 번에 처리할 레코드 수를 지정할 수 있습니다:
...
> [!WARNING]
> 큐를 사용하여 배치 임포트할 때는 `makeAllSearchableUsing` 메서드가 적용되지 않을 수 있습니다. 큐 작업 처리 중에는 [관계가 복원되지 않습니다](/docs/master/queues#handling-relationships).
...
Eloquent 쿼리 결과로 가져온 모델 컬렉션을 검색 인덱스에 추가하려면, 쿼리 뒤에 `searchable` 메서드를 체이닝합니다. 이 메서드는 [쿼리 결과를 청크 단위로 분할](/docs/master/eloquent#chunking-results)하여 인덱스에 추가합니다. 큐 설정이 되어 있다면, 청크 전체가 큐 워커에 의해 백그라운드에서 처리됩니다:
...
인덱스에서 레코드를 삭제하려면 모델을 데이터베이스에서 삭제하면 됩니다. [소프트 삭제](/docs/master/eloquent#soft-deleting) 모델도 마찬가지로 처리됩니다:
...
Scout가 검색 엔진에서 일치하는 Eloquent 모델 목록을 받아온 뒤, Eloquent는 해당 모델의 기본 키로 실제 레코드를 불러옵니다. 이 과정에서 실행되는 쿼리를 사용자 지정하려면 `query` 메서드를 사용할 수 있습니다. 이 메서드는 Eloquent 쿼리 빌더 인스턴스를 인자로 받는 클로저를 인자로 받습니다:
...
서드파티 엔진 사용 시에는 이 콜백이 이미 검색 엔진에서 모델을 받아온 후에 실행되므로, 필터링 용도가 아닌 추가 가공 용도로만 사용해야 합니다. 대신 [Scout where 절](#where-clauses)을 사용하세요. 하지만 데이터베이스 엔진 사용 시에는 `query` 메서드의 제약이 바로 데이터베이스 쿼리에 반영되므로 필터링에도 활용할 수 있습니다.

<a name="pagination"></a>
### 페이지네이션

모델 컬렉션을 단순히 조회하는 것 외에도, `paginate` 메서드로 검색 결과를 페이지네이션할 수 있습니다. 이 메서드는 [일반 Eloquent 쿼리로 페이지네이션](/docs/master/pagination)한 것과 동일하게 `Illuminate\Pagination\LengthAwarePaginator` 인스턴스를 반환합니다:
...
인덱싱하는 모델이 [소프트 삭제](/docs/master/eloquent#soft-deleting)를 지원하고, 소프트 삭제된 모델도 검색 대상에 포함하려면, `config/scout.php`의 `soft_delete` 옵션을 `true`로 설정합니다:
