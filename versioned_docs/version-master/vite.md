...
[nonce attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/nonce) 적용을 위해 스크립트 및 스타일 태그에 [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP)을 추가하려면, 커스텀 [middleware](/docs/master/middleware)에서 `useCspNonce` 메서드를 호출하면 됩니다.
...
Laravel의 [Ziggy `@route` directive](https://github.com/tighten/ziggy#using-routes-with-a-content-security-policy)에 포함된 [starter kits](/docs/master/starter-kits)처럼 다른 곳에서도 nonce 값이 필요하다면, `cspNonce` 메서드로 값을 얻어 사용할 수 있습니다.
