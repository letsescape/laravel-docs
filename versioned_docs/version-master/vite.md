...
> [!NOTE]
> 아래 문서는 Laravel Vite 플러그인을 수동으로 설치하고 구성하는 방법에 대해 설명합니다. 하지만 Laravel의 [스타터 킷](/docs/master/starter-kits)에는 이미 모든 환경이 구성되어 있어, Laravel과 Vite를 가장 빠르게 시작하는 방법입니다.
...
최신 버전의 Node와 NPM은 [공식 Node 웹사이트](https://nodejs.org/en/download/)에서 제공하는 그래픽 설치 프로그램으로 쉽게 설치할 수 있습니다. 또는 [Laravel Sail](https://laravel.com/docs/master/sail)을 사용하는 경우 Sail을 통해 Node와 NPM을 실행할 수 있습니다.
...
[Laravel Herd](https://herd.laravel.com)에서 사이트를 보안 설정하거나, [Laravel Valet](/docs/master/valet)에서 [secure 명령어](/docs/master/valet#securing-sites)를 사용하고 있다면, Laravel Vite 플러그인이 자동으로 생성된 TLS 인증서를 감지해 설정합니다.
...
[WSL2 환경의 Sail](/docs/master/sail)에서 개발 서버를 실행한다면 [추가 설정](#configuring-hmr-in-sail-on-wsl2)이 필요할 수 있습니다.
...
> [!NOTE]
> Laravel의 [스타터 킷](/docs/master/starter-kits)은 Laravel, Vue, Vite 구성까지 모두 포함되어 있습니다. 이 스타터 킷을 활용하면 가장 빠르고 손쉽게 Laravel, Vue, Vite로 개발을 시작할 수 있습니다.
...
> [!NOTE]
> Laravel의 [스타터 킷](/docs/master/starter-kits)에는 Laravel, React, Vite 구성이 이미 포함되어 있어, 가장 빠르게 개발을 시작할 수 있습니다.
...
> [!NOTE]
> Laravel의 [스타터 킷](/docs/master/starter-kits)에는 Laravel, Svelte, Vite 구성이 모두 포함되어 있습니다.
...
> [!NOTE]
> Laravel의 [스타터 킷](/docs/master/starter-kits)에는 Laravel, Inertia, Vite 구성이 이미 포함되어 있습니다.
...
> [!NOTE]
> Laravel의 [스타터 킷](/docs/master/starter-kits)에는 올바르게 설정된 Tailwind 및 Vite 구성이 이미 포함되어 있습니다. Starter Kit을 사용하지 않고 Tailwind와 Laravel을 사용하려면 [Tailwind의 Laravel 설치 가이드](https://tailwindcss.com/docs/guides/laravel)를 참고하세요.
...
자바스크립트 애플리케이션에서는 특정 경로의 디렉터리를 [별칭(alias)](#aliases)으로 지정해 두는 것이 일반적입니다. Blade에서도 `Illuminate\Support\Facades\Vite` 클래스의 `macro` 메서드를 사용해 별칭을 만들 수 있습니다. 보통 [서비스 프로바이더](/docs/master/providers)의 `boot` 메서드에서 별칭을 등록합니다.
...
[서비스 프로바이더](/docs/master/providers)의 `boot` 메서드에서 `Vite::prefetch` 메서드를 호출해 에셋을 프리페치하도록 설정하세요.
...
기본적으로 프리페치 동작은 [page load 이벤트](https://developer.mozilla.org/en-US/docs/Web/API/Window/load_event)가 발생할 때 시작합니다. 프리페치 시작 시점을 사용자 지정하고 싶다면 Vite에서 감지할 이벤트명을 지정할 수 있습니다.
...
> [!NOTE]
> Laravel의 [스타터 킷](/docs/master/starter-kits)에는 Laravel, Inertia SSR, Vite 구성이 모두 포함되어 있습니다.
...
[Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) 적용을 위해 스크립트 및 스타일 태그에 [nonce 속성](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/nonce)을 추가하려면, 커스텀 [미들웨어](/docs/master/middleware)에서 `useCspNonce` 메서드를 호출하면 됩니다.
...
Laravel의 [스타터 킷](/docs/master/starter-kits)에 포함된 [Ziggy의 `@route` 디렉티브](https://github.com/tighten/ziggy#using-routes-with-a-content-security-policy)처럼 다른 곳에서도 nonce 값이 필요하다면, `cspNonce` 메서드로 값을 얻어 사용할 수 있습니다.
...
스크립트 및 스타일 태그에 [data-turbo-track](https://turbo.hotwired.dev/handbook/drive#reloading-when-assets-change)와 같은 추가 속성을 포함해야 한다면, `useScriptTagAttributes` 및 `useStyleTagAttributes` 메서드를 사용할 수 있습니다. 이 메서드는 보통 [서비스 프로바이더](/docs/master/providers)에서 호출합니다.
