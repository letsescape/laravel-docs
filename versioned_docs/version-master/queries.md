...
- [소개](#introduction)
- [실행 중인 데이터베이스 쿼리](#running-database-queries)
    - [청킹 결과](#chunking-results)
    - [결과를 느리게 스트리밍](#streaming-results-lazily)
    - [집계](#aggregates)
- [문 선택](#select-statements)
- [원시 표현](#raw-expressions)
- [조인](#joins)
- [조합](#unions)
- [기본 Where 절](#basic-where-clauses)
    - [Where 절](#where-clauses)
    - [또는 Where 절](#or-where-clauses)
    - [Where Not 조항](#where-not-clauses)
    - [Where Any / All / None 절](#where-any-all-none-clauses)
    - [JSON Where 절](#json-where-clauses)
    - [추가 Where 조항](#additional-where-clauses)
    - [논리적 그룹화](#logical-grouping)
- [고급 Where 절](#advanced-where-clauses)
    - [존재 조항](#where-exists-clauses)
    - [하위 쿼리 Where 절](#subquery-where-clauses)
    - [전문 Where 조항](#full-text-where-clauses)
    - [벡터 유사성 조항](#vector-similarity-clauses)
- [순서, 그룹화, 제한 및 오프셋](#ordering-grouping-limit-and-offset)
    - [주문](#ordering)
    - [그룹화](#grouping)
    - [한계 및 오프셋](#limit-and-offset)
- [조건절](#conditional-clauses)
- [문장 삽입](#insert-statements)
    - [업서트](#upserts)
- [업데이트 설명](#update-statements)
    - [JSON 열 업데이트](#updating-json-columns)
    - [증감](#increment-and-decrement)
- [삭제문의](#delete-statements)
...
## 데이터베이스 쿼리 실행 중 (Running Database Queries)
...
> [!NOTE]
> Laravel 컬렉션은 데이터 매핑 및 축소를 위한 다양하고 매우 강력한 방법을 제공합니다. Laravel 컬렉션에 대한 자세한 내용은 [컬렉션 문서](/docs/master/collections)를 확인하세요.
...
`chunkById` 및 `lazyById` 메소드는 실행 중인 쿼리에 자체 "where" 조건을 추가하므로 일반적으로 클로저 내에서 자체 조건을 [논리적으로 그룹화](#logical-grouping)해야 합니다.
...
`lazy` 메서드는 쿼리를 청크로 실행한다는 점에서 [청크 메서드](#chunking-results)와 유사하게 작동합니다. 그러나 각 청크를 콜백에 전달하는 대신 `lazy()` 메서드는 결과를 단일 스트림으로 상호 작용할 수 있는 [LazyCollection](/docs/master/collections#lazy-collections)을 반환합니다.
...
`whereFullText` 및 `orWhereFullText` 메서드는 [전체 텍스트 인덱스](/docs/master/migrations#available-index-types)가 있는 열의 쿼리에 전체 텍스트 "where" 절을 추가하는 데 사용할 수 있습니다. 이러한 메소드는 Laravel에 의해 기본 데이터베이스 시스템에 적합한 SQL로 변환됩니다. 예를 들어, MariaDB 또는 MySQL을 활용하는 애플리케이션에 대해 `MATCH AGAINST` 절이 생성됩니다.
...
> [!NOTE]
> 벡터 유사성 절은 현재 `pgvector` 확장을 사용하는 PostgreSQL 연결에서만 지원됩니다. 벡터 열 및 인덱스 정의에 대한 자세한 내용은 [마이그레이션 문서](/docs/master/migrations#available-column-types)를 참조하세요.
...
일반 문자열이 벡터 인수로 제공되면 Laravel는 [Laravel AI SDK](/docs/master/ai-sdk#embeddings)를 사용하여 자동으로 임베딩을 생성합니다.
...
## 순서 지정, 그룹화, 제한 및 오프셋 (Ordering, Grouping, Limit and Offset)
...
`insertOrIgnore` 메소드는 데이터베이스에 레코드를 삽입하는 동안 오류를 무시합니다. 이 방법을 사용할 경우 중복 레코드 오류는 무시되며, 데이터베이스 엔진에 따라 다른 유형의 오류도 무시될 수 있다는 점을 유의해야 합니다. 예를 들어, `insertOrIgnore`는 [MySQL의 엄격 모드를 우회](https://dev.mysql.com/doc/refman/en/sql-mode.html#ignore-effect-on-execution)합니다.
...
필수는 아니지만 [트랜잭션](/docs/master/database#database-transactions) 내에서 비관적 잠금을 래핑하는 것이 좋습니다. 이렇게 하면 검색된 데이터가 전체 작업이 완료될 때까지 데이터베이스에서 변경되지 않은 상태로 유지됩니다. 실패할 경우 트랜잭션은 모든 변경 사항을 롤백하고 자동으로 잠금을 해제합니다.
...
애플리케이션 전체에서 사용되는 공유 [페이지 매기기](/docs/master/pagination) 논리를 포함하는 다음 쿼리 개체를 고려하세요. 쿼리 조건을 쿼리에 적용하는 `DestinationFilter`와 달리, `Paginate` 객체는 쿼리를 실행하고 페이지네이터 인스턴스를 반환합니다.
