...
Laravel에는 문자열 값을 다루기 위한 다양한 함수가 포함되어 있습니다. 이 함수들 중 많은 부분은 프레임워크 내부에서도 사용됩니다. 하지만 편리하다고 느낀다면 여러분의 애플리케이션에서도 자유롭게 사용할 수 있습니다.
...
`__` 함수는 주어진 번역 문자열 또는 번역 키를 [language files](/docs/master/localization)을 사용하여 번역합니다.
...
지정한 번역 문자열이나 키가 존재하지 않으면 `__` 함수는 주어진 값을 그대로 반환합니다. 따라서 위 예제에서 해당 번역 키가 존재하지 않는다면 `__` 함수는 `messages.welcome`을 반환합니다.
...
`preg_replace_array` 함수는 배열을 사용하여 문자열 안의 지정된 패턴을 순서대로 교체합니다.
...
`Str::after` 메서드는 문자열에서 주어진 값 뒤의 모든 내용을 반환합니다. 문자열 안에 해당 값이 없으면 전체 문자열이 반환됩니다.
...
`Str::afterLast` 메서드는 문자열에서 주어진 값이 마지막으로 나타난 위치 뒤의 모든 내용을 반환합니다. 문자열 안에 해당 값이 없으면 전체 문자열이 반환됩니다.
...
`Str::apa` 메서드는 [APA guidelines](https://apastyle.apa.org/style-grammar-guidelines/capitalization/title-case)에 따라 주어진 문자열을 제목 대소문자(title case)로 변환합니다.
...
`Str::charAt` 메서드는 지정한 인덱스에 있는 문자를 반환합니다. 인덱스가 범위를 벗어나면 `false`가 반환됩니다:
...
`Str::chopStart` 메서드는 주어진 값이 문자열의 시작 부분에 있을 때만, 그 값이 처음 나타난 부분을 제거합니다:
...
`Str::chopEnd` 메서드는 주어진 값이 문자열의 끝부분에 있을 때만, 그 값이 마지막으로 나타난 부분을 제거합니다:
...
값의 배열을 전달하여, 주어진 문자열이 배열 안의 값 중 하나라도 포함하는지 확인할 수도 있습니다:
...
값의 배열을 전달하여, 주어진 문자열이 배열 안의 어떤 값도 포함하지 않는지 확인할 수도 있습니다:
...
값의 배열을 전달하여, 주어진 문자열이 배열 안의 어떤 값으로도 끝나지 않는지 확인할 수도 있습니다:
...
가능한 값들의 배열을 전달하면, 문자열이 주어진 값 중 어떤 것으로도 시작하지 않을 때 `doesntStartWith` 메서드는 `true`를 반환합니다:
...
값의 배열을 전달하여, 주어진 문자열이 배열 안의 값 중 하나로 끝나는지 확인할 수도 있습니다:
...
`Str::excerpt` 메서드는 주어진 문자열 안에서 특정 구문이 처음 나타나는 부분과 일치하는 발췌문을 추출합니다:
...
`Str::finish` 메서드는 문자열이 주어진 값으로 끝나지 않는 경우, 해당 값을 문자열에 한 번만 추가합니다:
...
`Str::headline` 메서드는 대소문자 구분, 하이픈, 밑줄로 나뉜 문자열을 공백으로 구분된 문자열로 변환하고, 각 단어의 첫 글자를 대문자로 만듭니다:
...
`Str::inlineMarkdown` 메서드는 [CommonMark](https://commonmark.thephpleague.com/)를 사용하여 GitHub 스타일 Markdown을 인라인 HTML로 변환합니다. 하지만 `markdown` 메서드와 달리 생성된 모든 HTML을 블록 수준 요소로 감싸지는 않습니다:
...
`isUrl` 메서드는 다양한 프로토콜을 유효한 것으로 간주합니다. 그러나 `isUrl` 메서드에 프로토콜을 전달하여 어떤 프로토콜을 유효한 것으로 간주할지 지정할 수 있습니다.
...
`Str::lcfirst` 메서드는 주어진 문자열의 첫 문자를 소문자로 변환하여 반환합니다.
...
잘린 문자열 끝에 덧붙일 문자열을 변경하려면 세 번째 인수를 전달할 수 있습니다.
...
문자열을 잘라낼 때 완전한 단어를 보존하려면 `preserveWords` 인수를 사용할 수 있습니다. 이 인수가 `true`이면 문자열은 가장 가까운 완전한 단어 경계에서 잘립니다.
...
<!-- By default, Markdown supports raw HTML, which will expose Cross-Site Scripting (XSS) vulnerabilities when used with raw user input. As per the [CommonMark Security documentation](https://commonmark.thephpleague.com/security/), you may use the `html_input` option to either escape or strip raw HTML, and the `allow_unsafe_links` option to specify whether to allow unsafe links. If you need to allow some raw HTML, you should pass your compiled Markdown through an HTML Purifier: -->
기본적으로 Markdown은 원시 HTML을 지원합니다. 따라서 원시 사용자 입력과 함께 사용하면 크로스 사이트 스크립팅(Cross-Site Scripting, XSS) 취약점에 노출됩니다. [CommonMark Security documentation](https://commonmark.thephpleague.com/security/)에 따르면, `html_input` 옵션을 사용하여 원시 HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 통과시켜야 합니다.
...
`Str::mask` 메서드는 문자열의 일부를 반복되는 문자로 마스킹합니다. 이메일 주소나 전화번호 같은 문자열의 일부 구간을 숨기는 데 사용할 수 있습니다.
...
필요하다면 `mask` 메서드의 세 번째 인수로 음수를 전달할 수 있습니다. 그러면 메서드는 문자열 끝에서부터 지정한 거리만큼 떨어진 위치에서 마스킹을 시작합니다.
...
표현식 안에 매칭 그룹을 지정하면, Laravel은 첫 번째 매칭 그룹에 해당하는 일치 항목들의 컬렉션을 반환합니다.
...
`Str::password` 메서드는 주어진 길이의 안전한 임의 비밀번호를 생성하는 데 사용할 수 있습니다. 비밀번호는 문자, 숫자, 기호, 공백의 조합으로 구성됩니다. 기본적으로 비밀번호 길이는 32자입니다.
...
<!-- #### `Str::plural()` -->
#### `Str::plural()`
<!-- The `Str::plural` method converts a singular word string to its plural form. This function supports [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language): -->
`Str::plural` 메서드는 단수 단어 문자열을 복수형으로 변환합니다. 이 함수는 [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language)를 지원합니다.
...
문자열의 단수형 또는 복수형을 가져오려면 함수의 두 번째 인수로 정수를 전달할 수 있습니다.
...
`prependCount` 인수를 제공하면 복수형 문자열 앞에 포맷된 `$count`를 붙일 수 있습니다:
...
<!-- #### `Str::pluralStudly()` -->
#### `Str::pluralStudly()`
<!-- The `Str::pluralStudly` method converts a singular word string formatted in studly caps case to its plural form. This function supports [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language): -->
`Str::pluralStudly` 메서드는 studly caps case 형식의 단수 단어 문자열을 복수형으로 변환합니다. 이 함수는 [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language)를 지원합니다:
...
`Str::position` 메서드는 문자열에서 부분 문자열이 처음 나타나는 위치를 반환합니다. 주어진 문자열에 해당 부분 문자열이 없으면 `false`가 반환됩니다:
...
테스트 중에는 `Str::random` 메서드가 반환하는 값을 "가짜로" 만드는 것이 유용할 수 있습니다. 이를 위해 `createRandomStringsUsing` 메서드를 사용할 수 있습니다:
...
`replaceMatches` 메서드는 주어진 패턴과 일치하는 문자열의 각 부분을 인수로 받아 호출되는 클로저도 받을 수 있습니다. 이를 통해 클로저 안에서 교체 로직을 수행하고 교체된 값을 반환할 수 있습니다:
...
`Str::replaceStart` 메서드는 주어진 값이 문자열의 시작 부분에 나타날 때에만, 그 첫 번째 항목을 교체합니다:
...
`Str::replaceEnd` 메서드는 주어진 값이 문자열의 끝 부분에 나타날 때에만, 그 마지막 항목을 교체합니다:
...
<!-- #### `Str::singular()` -->
#### `Str::singular()`
<!-- The `Str::singular` method converts a string to its singular form. This function supports [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language): -->
`Str::singular` 메서드는 문자열을 단수형으로 변환합니다. 이 함수는 [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language)를 지원합니다:
...
`Str::slug` 메서드는 주어진 문자열에서 URL에 적합한 "slug"를 생성합니다:
...
`Str::start` 메서드는 문자열이 이미 주어진 값으로 시작하지 않는 경우, 해당 값을 문자열 앞에 한 번만 추가합니다:
...
`Str::substr` 메서드는 시작 위치와 길이 매개변수로 지정된 문자열의 일부를 반환합니다:
...
`Str::substrReplace` 메서드는 문자열의 일부 구간에 있는 텍스트를 교체합니다. 세 번째 인수로 지정한 위치에서 시작하여, 네 번째 인수로 지정한 문자 수만큼 교체합니다. 메서드의 네 번째 인수에 `0`을 전달하면 문자열의 기존 문자를 교체하지 않고 지정한 위치에 문자열을 삽입합니다.
...
`Str::take` 메서드는 문자열의 시작 부분에서 지정한 수의 문자를 반환합니다.
...
`Str::rtrim` 메서드는 주어진 문자열의 끝 부분에서 공백(또는 다른 문자)을 제거합니다. PHP의 기본 `rtrim` 함수와 달리, `Str::rtrim` 메서드는 유니코드 공백 문자도 제거합니다.
...
선택적 매개변수로 `DateTimeInterface`를 전달할 수 있으며, 이 값은 정렬 가능한 UUID를 생성하는 데 사용됩니다.
...
`Str::words` 메서드는 문자열의 단어 수를 제한합니다. 이 메서드의 세 번째 인수로 추가 문자열을 전달하여, 잘린 문자열의 끝에 어떤 문자열을 덧붙일지 지정할 수 있습니다.
...
`trans` 함수는 [language files](/docs/master/localization)을 사용하여 주어진 번역 키를 번역합니다.
...
`trans_choice` 함수는 주어진 번역 키를 수량에 따른 형태 변화와 함께 번역합니다.
...
플루언트 문자열은 문자열 값을 다루기 위한 더 유연한 객체 지향 인터페이스를 제공합니다. 전통적인 문자열 작업 방식보다 읽기 쉬운 문법으로 여러 문자열 작업을 이어서 호출할 수 있습니다.

<a name="method-fluent-str-after"></a>
<!-- #### `after` -->
#### `after`
<!-- The `after` method returns everything after the given value in a string. The entire string will be returned if the value does not exist within the string: -->
`after` 메서드는 문자열에서 주어진 값 뒤의 모든 내용을 반환합니다. 해당 값이 문자열 안에 없으면 전체 문자열을 반환합니다.
...
`afterLast` 메서드는 문자열에서 주어진 값이 마지막으로 나타난 위치 뒤의 모든 내용을 반환합니다. 해당 값이 문자열 안에 없으면 전체 문자열을 반환합니다.
...
`before` 메서드는 문자열에서 주어진 값 앞에 있는 모든 내용을 반환합니다.
...
`beforeLast` 메서드는 문자열에서 주어진 값이 마지막으로 나타나는 위치 앞에 있는 모든 내용을 반환합니다.
...
`chopStart` 메서드는 주어진 값이 문자열의 시작 부분에 나타나는 경우에만, 그 값이 처음 나타난 부분을 제거합니다.
...
배열을 전달할 수도 있습니다. 문자열이 배열에 있는 값 중 하나로 시작하면, 해당 값이 문자열에서 제거됩니다.
...
`chopEnd` 메서드는 주어진 값이 문자열의 끝부분에 나타나는 경우에만, 그 값이 마지막으로 나타난 부분을 제거합니다.
...
배열을 전달할 수도 있습니다. 문자열이 배열에 있는 값 중 하나로 끝나면, 해당 값이 문자열에서 제거됩니다.
...
<!-- You may also pass an array of values to determine if the given string contains any of the values in the array: -->
주어진 문자열에 배열의 값 중 하나라도 포함되어 있는지 확인하려면 값 배열을 전달할 수도 있습니다.
...
<!-- #### `decrypt` -->
#### `decrypt`
<!-- The `decrypt` method [decrypts](/docs/master/encryption) the encrypted string: -->
`decrypt` 메서드는 암호화된 문자열을 [decrypts](/docs/master/encryption)합니다.
...
`deduplicate` 메서드는 주어진 문자열에서 연속해서 나타나는 문자를 해당 문자의 단일 인스턴스로 바꿉니다. 기본적으로 이 메서드는 공백을 중복 제거합니다.
...
<!-- You may specify a different character to deduplicate by passing it in as the second argument to the method: -->
중복 제거할 다른 문자를 지정하려면 해당 문자를 메서드의 두 번째 인수로 전달하면 됩니다.
...
필요한 경우 문자열에서 제거할 디렉터리 레벨 수를 지정할 수 있습니다.
...
주어진 문자열에 배열의 값이 하나도 포함되어 있지 않은지 확인하려면 값 배열을 전달할 수도 있습니다.
...
<!-- You may also pass an array of values to determine if the given string doesn't end with any of the values in the array: -->
주어진 문자열이 배열의 값 중 어떤 값으로도 끝나지 않는지 확인하려면 값 배열을 전달할 수도 있습니다.
...
주어진 문자열이 배열의 값 중 어떤 값으로도 시작하지 않는지 확인하려면 값 배열을 전달할 수도 있습니다.
...
<!-- #### `encrypt` -->
#### `encrypt`
<!-- The `encrypt` method [encrypts](/docs/master/encryption) the string: -->
`encrypt` 메서드는 문자열을 [encrypts](/docs/master/encryption):
...
`encrypt`의 반대 작업은 [decrypt](#method-fluent-str-decrypt) 메서드를 참고하십시오.
...
`excerpt` 메서드는 문자열 안에서 구문의 첫 번째 등장 위치와 일치하는 부분을 발췌합니다:
...
<!-- The `radius` option, which defaults to `100`, allows you to define the number of characters that should appear on each side of the truncated string. -->
기본값이 `100`인 `radius` 옵션을 사용하면 잘린 문자열의 양쪽에 표시할 문자 수를 지정할 수 있습니다.
...
`hash` 메서드는 주어진 [algorithm](https://www.php.net/manual/en/function.hash-algos.php)을 사용하여 문자열을 해시합니다:
...
`headline` 메서드는 대소문자 구분, 하이픈, 밑줄로 구분된 문자열을 공백으로 구분된 문자열로 변환하고, 각 단어의 첫 글자를 대문자로 만듭니다:
...
`inlineMarkdown` 메서드는 [CommonMark](https://commonmark.thephpleague.com/)를 사용하여 GitHub flavored Markdown을 인라인 HTML로 변환합니다. 하지만 `markdown` 메서드와 달리, 생성된 모든 HTML을 블록 수준 요소로 감싸지 않습니다:
...
<!-- The `isUrl` method considers a wide range of protocols as valid. However, you may specify the protocols that should be considered valid by providing them to the `isUrl` method: -->
`isUrl` 메서드는 다양한 프로토콜을 유효한 것으로 간주합니다. 하지만 `isUrl` 메서드에 프로토콜을 제공하여 유효한 것으로 간주할 프로토콜을 지정할 수 있습니다:
...
<!-- You may also validate that the given UUID matches a UUID specification by version (1, 3, 4, 5, 6, 7, or 8): -->
주어진 UUID가 버전(1, 3, 4, 5, 6, 7, 또는 8)에 따른 UUID 명세와 일치하는지도 검증할 수 있습니다:
...
두 번째 인수를 전달하여 잘린 문자열 끝에 추가될 문자열을 변경할 수도 있습니다:
...
<!-- By default, Markdown supports raw HTML, which will expose Cross-Site Scripting (XSS) vulnerabilities when used with raw user input. As per the [CommonMark Security documentation](https://commonmark.thephpleague.com/security/), you may use the `html_input` option to either escape or strip raw HTML, and the `allow_unsafe_links` option to specify whether to allow unsafe links. If you need to allow some raw HTML, you should pass your compiled Markdown through an HTML Purifier: -->
기본적으로 Markdown은 원시 HTML을 지원합니다. 원시 사용자 입력과 함께 사용하면 Cross-Site Scripting(XSS, 크로스 사이트 스크립팅) 취약점에 노출될 수 있습니다. [CommonMark Security documentation](https://commonmark.thephpleague.com/security/)에 따르면, `html_input` 옵션을 사용해 원시 HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 통과시켜야 합니다.
...
`mask` 메서드는 문자열의 일부를 반복되는 문자로 가립니다. 이메일 주소나 전화번호처럼 문자열의 특정 부분을 난독화할 때 사용할 수 있습니다.
...
필요하다면 `mask` 메서드의 세 번째 또는 네 번째 인수로 음수를 제공할 수 있습니다. 이렇게 하면 문자열 끝에서 주어진 거리만큼 떨어진 위치부터 마스킹을 시작하도록 메서드에 지시합니다.
...
`matchAll` 메서드는 주어진 정규 표현식 패턴과 일치하는 문자열 부분들을 담은 컬렉션을 반환합니다.
...
`padBoth` 메서드는 PHP의 `str_pad` 함수를 감싸는 메서드로, 최종 문자열이 원하는 길이에 도달할 때까지 문자열 양쪽을 다른 문자열로 채웁니다.
...
`padLeft` 메서드는 PHP의 `str_pad` 함수를 감싸는 메서드로, 최종 문자열이 원하는 길이에 도달할 때까지 문자열 왼쪽을 다른 문자열로 채웁니다.
...
`padRight` 메서드는 PHP의 `str_pad` 함수를 감싸는 메서드로, 최종 문자열이 원하는 길이에 도달할 때까지 문자열 오른쪽을 다른 문자열로 채웁니다.
...
`pipe` 메서드를 사용하면 현재 값을 주어진 callable에 전달해 문자열을 변환할 수 있습니다.
...
<!-- #### `plural` -->
#### `plural`
<!-- The `plural` method converts a singular word string to its plural form. This function supports [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language): -->
`plural` 메서드는 단수 단어 문자열을 복수형으로 변환합니다. 이 함수는 [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language)를 지원합니다.
...
문자열의 단수형 또는 복수형을 가져오기 위해 함수에 정수 인수를 제공할 수 있습니다.
...
`prependCount` 인수를 제공하면 형식이 적용된 `$count`를 복수화된 문자열 앞에 붙일 수 있습니다.
...
`position` 메서드는 문자열 안에서 부분 문자열이 처음 등장하는 위치를 반환합니다. 해당 부분 문자열이 문자열 안에 없으면 `false`가 반환됩니다.
...
문자열을 제거할 때 대소문자를 무시하려면 두 번째 파라미터로 `false`를 전달할 수도 있습니다.
...
`replaceMatches` 메서드는 패턴과 일치하는 문자열의 모든 부분을 주어진 교체 문자열로 교체합니다.
...
<!-- The `replaceMatches` method also accepts a closure that will be invoked with each portion of the string matching the given pattern, allowing you to perform the replacement logic within the closure and return the replaced value: -->
`replaceMatches` 메서드는 주어진 패턴과 일치하는 문자열의 각 부분을 인수로 받아 호출되는 클로저도 허용합니다. 이를 통해 클로저 안에서 교체 로직을 수행하고 교체된 값을 반환할 수 있습니다.
```php
use Illuminate\Support\Str;
...
`replaceStart` 메서드는 주어진 값이 문자열의 시작 부분에 나타날 때만 해당 값의 첫 번째 항목을 교체합니다.
...
`replaceEnd` 메서드는 주어진 값이 문자열의 끝부분에 나타날 때만 해당 값의 마지막 항목을 교체합니다.
...
`scan` 메서드는 [`sscanf` PHP function](https://www.php.net/manual/en/function.sscanf.php)가 지원하는 형식에 따라 문자열의 입력을 파싱하여 컬렉션으로 만듭니다.
...
<!-- #### `singular` -->
#### `singular`
<!-- The `singular` method converts a string to its singular form. This function supports [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language): -->
`singular` 메서드는 문자열을 단수형으로 변환합니다. 이 함수는 [any of the languages supported by Laravel's pluralizer](/docs/master/localization#pluralization-language)를 지원합니다.
...
`slug` 메서드는 주어진 문자열에서 URL에 적합한 "slug"를 생성합니다.
...
배열 안의 값 중 하나로 주어진 문자열이 시작하는지 확인하기 위해 값 배열을 전달할 수도 있습니다.
...
`substrReplace` 메서드는 두 번째 인수로 지정한 위치부터 시작하여, 세 번째 인수로 지정한 문자 수만큼 문자열의 일부 텍스트를 교체합니다. 메서드의 세 번째 인수에 `0`을 전달하면 문자열의 기존 문자를 교체하지 않고 지정한 위치에 문자열을 삽입합니다.
...
`swap` 메서드는 PHP의 `strtr` 함수를 사용하여 문자열 안의 여러 값을 교체합니다.
...
`tap` 메서드는 문자열을 주어진 클로저에 전달하여, 문자열 자체에는 영향을 주지 않으면서 문자열을 살펴보고 상호작용할 수 있게 합니다. 클로저가 무엇을 반환하든 `tap` 메서드는 원래 문자열을 반환합니다.
...
`toHtmlString` 메서드는 주어진 문자열을 `Illuminate\Support\HtmlString` 인스턴스로 변환합니다. 이 인스턴스는 Blade 템플릿에서 렌더링될 때 이스케이프 처리되지 않습니다.
...
<!-- #### `toUri` -->
#### `toUri`
<!-- The `toUri` method converts the given string to an instance of [Illuminate\Support\Uri](/docs/master/helpers#uri): -->
`toUri` 메서드는 주어진 문자열을 [Illuminate\Support\Uri](/docs/master/helpers#uri) 인스턴스로 변환합니다.
...
`transliterate` 메서드는 주어진 문자열을 가능한 한 가장 가까운 ASCII 표현으로 변환하려고 시도합니다.
...
`trim` 메서드는 주어진 문자열의 앞뒤를 잘라냅니다. PHP의 기본 `trim` 함수와 달리, Laravel의 `trim` 메서드는 Unicode 공백 문자도 제거합니다.
...
`ltrim` 메서드는 문자열의 왼쪽 부분을 잘라냅니다. PHP의 기본 `ltrim` 함수와 달리, Laravel의 `ltrim` 메서드는 Unicode 공백 문자도 제거합니다.
...
`rtrim` 메서드는 주어진 문자열의 오른쪽 부분을 잘라냅니다. PHP의 기본 `rtrim` 함수와 달리, Laravel의 `rtrim` 메서드는 Unicode 공백 문자도 제거합니다.
...
`ucfirst` 메서드는 주어진 문자열의 첫 번째 문자를 대문자로 바꾼 값을 반환합니다:
...
`ucsplit` 메서드는 주어진 문자열을 대문자를 기준으로 나누어 컬렉션으로 만듭니다:
...
`when` 메서드는 주어진 조건이 `true`이면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
필요한 경우 `when` 메서드의 세 번째 매개변수로 또 다른 클로저를 전달할 수 있습니다. 이 클로저는 조건 매개변수가 `false`로 평가될 때 실행됩니다.

<a name="method-fluent-str-when-contains"></a>
<!-- #### `whenContains` -->
#### `whenContains`
<!-- The `whenContains` method invokes the given closure if the string contains the given value. The closure will receive the fluent string instance: -->
`whenContains` 메서드는 문자열에 주어진 값이 포함되어 있으면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
필요한 경우 세 번째 매개변수로 또 다른 클로저를 전달할 수 있습니다. 이 클로저는 문자열에 주어진 값이 포함되어 있지 않을 때 실행됩니다.

<!-- You may also pass an array of values to determine if the given string contains any of the values in the array: -->
주어진 문자열이 배열 안의 값 중 하나라도 포함하는지 확인하려면 값의 배열을 전달할 수도 있습니다:
...
`whenContainsAll` 메서드는 문자열에 주어진 모든 부분 문자열이 포함되어 있으면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
필요한 경우 세 번째 매개변수로 또 다른 클로저를 전달할 수 있습니다. 이 클로저는 조건 매개변수가 `false`로 평가될 때 실행됩니다.
...
`whenDoesntEndWith` 메서드는 문자열이 주어진 부분 문자열로 끝나지 않으면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenDoesntStartWith` 메서드는 문자열이 주어진 부분 문자열로 시작하지 않으면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenEmpty` 메서드는 문자열이 비어 있으면 주어진 클로저를 실행합니다. 클로저가 값을 반환하면 그 값이 `whenEmpty` 메서드에서도 반환됩니다. 클로저가 값을 반환하지 않으면 플루언트 문자열 인스턴스가 반환됩니다:
...
`whenNotEmpty` 메서드는 문자열이 비어 있지 않으면 주어진 클로저를 실행합니다. 클로저가 값을 반환하면 그 값이 `whenNotEmpty` 메서드에서도 반환됩니다. 클로저가 값을 반환하지 않으면 플루언트 문자열 인스턴스가 반환됩니다:
...
`whenStartsWith` 메서드는 문자열이 주어진 부분 문자열로 시작하면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenEndsWith` 메서드는 문자열이 주어진 부분 문자열로 끝나면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenExactly` 메서드는 문자열이 주어진 문자열과 정확히 일치하면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenNotExactly` 메서드는 문자열이 주어진 문자열과 정확히 일치하지 않으면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenIs` 메서드는 문자열이 주어진 패턴과 일치하면 주어진 클로저를 실행합니다. 별표는 와일드카드 값으로 사용할 수 있습니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenIsAscii` 메서드는 문자열이 7 bit ASCII이면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenIsUlid` 메서드는 문자열이 유효한 ULID이면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenIsUuid` 메서드는 문자열이 유효한 UUID이면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`whenTest` 메서드는 문자열이 주어진 정규 표현식과 일치하면 주어진 클로저를 실행합니다. 클로저는 플루언트 문자열 인스턴스를 전달받습니다:
...
`words` 메서드는 문자열의 단어 수를 제한합니다. 필요한 경우 잘린 문자열 뒤에 덧붙일 추가 문자열을 지정할 수 있습니다:
