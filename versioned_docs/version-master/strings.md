...
`__` 함수는 주어진 번역 문자열 또는 번역 키를 [언어 파일](/docs/master/localization)을 사용하여 번역합니다.
...
`Str::apa` 메서드는 [APA 지침](https://apastyle.apa.org/style-grammar-guidelines/capitalization/title-case)에 따라 주어진 문자열을 제목 대소문자(title case)로 변환합니다.
...
기본적으로 Markdown은 원시 HTML을 지원하므로, 원시 사용자 입력과 함께 사용하면 Cross-Site Scripting (XSS) 취약점에 노출됩니다. [CommonMark 보안 문서](https://commonmark.thephpleague.com/security/)에 따르면, `html_input` 옵션을 사용하여 원시 HTML을 이스케이프하거나 제거할 수 있고, `allow_unsafe_links` 옵션을 사용하여 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 통과시켜야 합니다:
...
기본적으로 Markdown은 원시 HTML을 지원합니다. 따라서 원시 사용자 입력과 함께 사용하면 크로스 사이트 스크립팅(Cross-Site Scripting, XSS) 취약점에 노출됩니다. [CommonMark 보안 문서](https://commonmark.thephpleague.com/security/)에 따르면, `html_input` 옵션을 사용하여 원시 HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 통과시켜야 합니다.
...
`Str::plural` 메서드는 단수 단어 문자열을 복수형으로 변환합니다. 이 함수는 [Laravel의 복수형 변환기가 지원하는 모든 언어](/docs/master/localization#pluralization-language)를 지원합니다.
...
`Str::pluralStudly` 메서드는 studly caps case 형식의 단수 단어 문자열을 복수형으로 변환합니다. 이 함수는 [Laravel의 pluralizer가 지원하는 모든 언어](/docs/master/localization#pluralization-language)를 지원합니다:
...
`Str::singular` 메서드는 문자열을 단수형으로 변환합니다. 이 함수는 [Laravel의 pluralizer가 지원하는 모든 언어](/docs/master/localization#pluralization-language)를 지원합니다:
...
`trans` 함수는 [언어 파일](/docs/master/localization)을 사용하여 주어진 번역 키를 번역합니다.
...
`encrypt` 메서드는 문자열을 [암호화합니다](/docs/master/encryption):
...
기본적으로 Markdown은 원시 HTML을 지원하며, 원시 사용자 입력과 함께 사용할 경우 Cross-Site Scripting (XSS) 취약점에 노출될 수 있습니다. [CommonMark 보안 문서](https://commonmark.thephpleague.com/security/)에 따르면, `html_input` 옵션을 사용하여 원시 HTML을 이스케이프하거나 제거할 수 있고, `allow_unsafe_links` 옵션을 사용하여 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 통과시켜야 합니다:
...
기본적으로 Markdown은 원시 HTML을 지원합니다. 원시 사용자 입력과 함께 사용하면 Cross-Site Scripting(XSS, 크로스 사이트 스크립팅) 취약점에 노출될 수 있습니다. [CommonMark 보안 문서](https://commonmark.thephpleague.com/security/)에 따르면, `html_input` 옵션을 사용해 원시 HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 통과시켜야 합니다.
...
`plural` 메서드는 단수 단어 문자열을 복수형으로 변환합니다. 이 함수는 [Laravel의 pluralizer가 지원하는 모든 언어](/docs/master/localization#pluralization-language)를 지원합니다.
...
`scan` 메서드는 [`sscanf` PHP 함수](https://www.php.net/manual/en/function.sscanf.php)가 지원하는 형식에 따라 문자열의 입력을 파싱하여 컬렉션으로 만듭니다.
...
`singular` 메서드는 문자열을 단수형으로 변환합니다. 이 함수는 [Laravel의 복수형 변환기가 지원하는 모든 언어](/docs/master/localization#pluralization-language)를 지원합니다.
