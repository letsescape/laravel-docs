...
뷰는 컨트롤러/애플리케이션 로직과 프레젠테이션(출력) 로직을 분리해 주며, `resources/views` 디렉토리에 저장됩니다. Laravel을 사용할 때 뷰 템플릿은 주로 [Blade templating language](/docs/master/blade)를 이용해 작성합니다. 간단한 뷰 예시는 다음과 같습니다:
...
> [!NOTE]
> Blade 템플릿 작성법에 대해 더 알고 싶으시다면, [Blade documentation](/docs/master/blade)를 참고해 시작해 보시기 바랍니다.
...
Laravel의 [React, Svelte, and Vue application starter kits](/docs/master/starter-kits)를 활용하면, Inertia로 구동되는 새로운 Laravel 애플리케이션의 시작점을 빠르게 만들 수 있습니다.
...
`.blade.php` 확장자는 해당 파일이 [Blade template](/docs/master/blade)임을 프레임워크에 알려줍니다. Blade 템플릿에는 HTML과 Blade 지시문(디렉티브)이 함께 포함될 수 있어, 값을 출력하거나, if문 작성, 데이터 반복 등 다양한 로직을 쉽게 구현할 수 있습니다.
...
여기서 `view` 헬퍼의 첫 번째 인수는 `resources/views` 디렉토리 내 뷰 파일의 이름을 의미합니다. 두 번째 인수는 뷰에서 사용할 데이터를 배열로 전달하는 부분입니다. 이번 예시에서는 `name` 변수를 전달하고 있으며, 뷰에서는 [Blade syntax](/docs/master/blade)을 통해 출력합니다.
...
일반적으로 뷰 컴포저는 애플리케이션의 [service providers](/docs/master/providers)에서 등록합니다. 아래 예시에서는 `App\Providers\AppServiceProvider`에서 관련 로직을 관리한다고 가정합니다.
...
보시는 것처럼, 모든 뷰 컴포저는 [service container](/docs/master/container)를 통해 의존성 주입을 지원하므로, 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다.
