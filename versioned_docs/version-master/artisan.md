...
예제 명령어를 살펴보겠습니다. 명령어의 `handle` 메서드를 통해 필요한 의존성을 요청할 수 있다는 점에 주목하세요. Laravel [service container](/docs/master/container)는 이 메서드의 시그니처에 타입 힌트된 모든 의존성을 자동으로 주입합니다.
...
명령어의 인수와 옵션을 받는 것 외에도, 명령어 클로저는 [service container](/docs/master/container)에서 해결하고 싶은 추가 의존성을 타입 힌트할 수 있습니다.
...
Artisan이 부팅될 때, 애플리케이션의 모든 명령어는 [service container](/docs/master/container)를 통해 해석되고 Artisan에 등록됩니다.
...
`Artisan` facade의 `queue` 메서드를 사용하면 Artisan 명령어를 큐에 넣어 [queue workers](/docs/master/queues)가 백그라운드에서 처리하도록 할 수도 있습니다. 이 메서드를 사용하기 전에 큐를 설정하고 큐 리스너를 실행 중인지 확인하세요.
