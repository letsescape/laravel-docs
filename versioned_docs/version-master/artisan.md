...
- [입력 기대값 정의하기](#defining-input-expectations)
    ...
    - [누락된 입력 요청하기](#prompting-for-missing-input)
- [명령어 I/O](#command-io)
...
로컬 개발 환경으로 [Laravel Sail](/docs/master/sail)을 사용하고 있다면, Artisan 명령어를 호출할 때 `sail` 명령줄을 사용해야 한다는 점을 기억하세요. Sail은 애플리케이션의 Docker 컨테이너 안에서 Artisan 명령어를 실행합니다.
...
Artisan에서 제공하는 명령어 외에도 직접 커스텀 명령어를 만들 수 있습니다. 명령어는 일반적으로 `app/Console/Commands` 디렉터리에 저장되지만, Laravel에 [다른 디렉터리에서 Artisan 명령어를 스캔하도록](#registering-commands) 지시한다면 원하는 저장 위치를 자유롭게 선택할 수 있습니다.
...
명령어를 생성한 후에는 `Signature`와 `Description` 속성을 사용하여 명령어의 시그니처와 설명을 정의해야 합니다. `Signature` 속성을 사용하면 [명령어의 입력 기대값](#defining-input-expectations)도 정의할 수 있습니다. 명령어가 실행되면 `handle` 메서드가 호출됩니다. 이 메서드 안에 명령어 로직을 작성할 수 있습니다.

예제 명령어를 살펴보겠습니다. 명령어의 `handle` 메서드를 통해 필요한 의존성을 요청할 수 있다는 점에 주목하세요. Laravel [서비스 컨테이너](/docs/master/container)는 이 메서드의 시그니처에 타입 힌트된 모든 의존성을 자동으로 주입합니다.
...
`routes/console.php` 파일은 HTTP 라우트를 정의하지는 않지만, 애플리케이션으로 진입하는 콘솔 기반 진입점(라우트)을 정의합니다. 이 파일 안에서 `Artisan::command` 메서드를 사용하여 모든 클로저 기반 콘솔 명령어를 정의할 수 있습니다. `command` 메서드는 두 개의 인수를 받습니다. [명령어 시그니처](#defining-input-expectations)와 명령어의 인수 및 옵션을 받는 클로저입니다.
...
명령어의 인수와 옵션을 받는 것 외에도, 명령어 클로저는 [서비스 컨테이너](/docs/master/container)에서 해결하고 싶은 추가 의존성을 타입 힌트할 수 있습니다.
...
> [!NOTE]
> [Laravel Prompts](/docs/master/prompts)는 명령줄 애플리케이션에 아름답고 사용하기 쉬운 폼을 추가하기 위한 PHP 패키지입니다. 플레이스홀더 텍스트와 유효성 검증을 포함한 브라우저와 비슷한 기능을 제공합니다.
...
Artisan이 부팅될 때, 애플리케이션의 모든 명령어는 [서비스 컨테이너](/docs/master/container)를 통해 해석되고 Artisan에 등록됩니다.
...
`Artisan` facade의 `queue` 메서드를 사용하면 Artisan 명령어를 큐에 넣어 [큐 워커](/docs/master/queues)가 백그라운드에서 처리하도록 할 수도 있습니다. 이 메서드를 사용하기 전에 큐를 설정하고 큐 리스너를 실행 중인지 확인하세요.
