...
> [!NOTE]
> 모든 Middleware는 [service container](/docs/master/container)를 통해 해결되므로, Middleware의 생성자에서 필요한 의존성을 타입 힌트할 수 있습니다.
...
특정 Middleware 집합을 전체 라우트 정의 [group](/docs/master/routing#route-groups)에서 제외할 수도 있습니다.
...
Middleware의 `terminate` 메서드를 호출할 때 Laravel은 [service container](/docs/master/container)에서 Middleware의 새 인스턴스를 해결합니다. `handle` 메서드와 `terminate` 메서드가 호출될 때 동일한 Middleware 인스턴스를 사용하고 싶다면, 컨테이너의 `singleton` 메서드를 사용하여 Middleware를 컨테이너에 등록하십시오. 일반적으로 이 작업은 `AppServiceProvider`의 `register` 메서드에서 수행해야 합니다.
