...
- [브로드캐스트 수신하기](#receiving-broadcasts)
    - [이벤트 감지하기](#listening-for-events)
    - [채널 나가기](#leaving-a-channel)
    - [네임스페이스](#namespaces)
    - [React 또는 Vue 사용하기](#using-react-or-vue)
- [Presence 채널](#presence-channels)
    - [Presence 채널 인가하기](#authorizing-presence-channels)
    - [Presence 채널 참여하기](#joining-presence-channels)
    - [Presence 채널로 브로드캐스팅하기](#broadcasting-to-presence-channels)
- [모델 브로드캐스팅](#model-broadcasting)
    - [모델 브로드캐스팅 규칙](#model-broadcasting-conventions)
    - [모델 브로드캐스트 감지하기](#listening-for-model-broadcasts)
...
예를 들어, 애플리케이션이 사용자의 데이터를 CSV 파일로 내보내고 이메일로 보낼 수 있다고 가정해 보겠습니다. 하지만 이 CSV 파일을 생성하는 데 몇 분이 걸리므로, [큐 작업](/docs/master/queues) 안에서 CSV를 생성하고 메일로 보내도록 선택할 수 있습니다. CSV가 생성되어 사용자에게 메일로 발송되면, 이벤트 브로드캐스팅을 사용해 애플리케이션의 JavaScript가 수신하는 `App\Events\UserDataExported` 이벤트를 디스패치할 수 있습니다. 이벤트가 수신되면 사용자가 페이지를 새로고침하지 않아도 CSV가 이메일로 발송되었다는 메시지를 표시할 수 있습니다.

이러한 기능을 더 쉽게 만들 수 있도록 Laravel은 서버 측 Laravel [이벤트](/docs/master/events)를 WebSocket 연결을 통해 "브로드캐스트"하기 쉽게 해줍니다. Laravel 이벤트를 브로드캐스팅하면 서버 측 Laravel 애플리케이션과 클라이언트 측 JavaScript 애플리케이션 사이에서 동일한 이벤트 이름과 데이터를 공유할 수 있습니다.
...
> [!NOTE]
> 이벤트 브로드캐스팅을 자세히 살펴보기 전에 Laravel의 [이벤트와 리스너](/docs/master/events) 문서를 먼저 읽어보십시오.
...
이벤트 브로드캐스팅을 활성화했다면, 이제 [브로드캐스트 이벤트 정의하기](#defining-broadcast-events)와 [이벤트 감지하기](#listening-for-events)에 대해 더 알아볼 준비가 되었습니다. Laravel의 React 또는 Vue [스타터 키트](/docs/master/starter-kits)를 사용하는 경우 Echo의 [useEcho 훅](#using-react-or-vue)을 사용하여 이벤트를 감지할 수 있습니다.

> [!NOTE]
> 이벤트를 브로드캐스트하기 전에 먼저 [큐 워커](/docs/master/queues)를 설정하고 실행해야 합니다. 모든 이벤트 브로드캐스팅은 큐 작업을 통해 수행되므로, 이벤트 브로드캐스트 때문에 애플리케이션의 응답 시간이 크게 영향을 받지 않습니다.
...
> [!NOTE]
> 아래 문서는 Ably를 "Pusher compatibility" 모드로 사용하는 방법을 설명합니다. 하지만 Ably 팀은 Ably가 제공하는 고유한 기능을 활용할 수 있는 브로드캐스터와 Echo 클라이언트를 권장하고 유지 관리합니다. Ably가 유지 관리하는 드라이버 사용에 대한 자세한 내용은 [Ably의 Laravel 브로드캐스터 문서](https://github.com/ably/laravel-broadcaster)를 참고하십시오.
...
다음으로 `config/broadcasting.php` 설정 파일에서 Ably 자격 증명을 설정해야 합니다. 이 파일에는 Ably 설정 예제가 이미 포함되어 있으므로 key를 빠르게 지정할 수 있습니다. 일반적으로 이 값은 `ABLY_KEY` [환경 변수](/docs/master/configuration#environment-configuration)를 통해 설정해야 합니다:
...
`ShouldBroadcast` 인터페이스는 하나의 메서드인 `broadcastOn` 구현을 요구합니다. `broadcastOn` 메서드는 이벤트가 브로드캐스트될 채널 또는 채널 배열을 반환해야 합니다. 채널은 `Channel`, `PrivateChannel`, 또는 `PresenceChannel`의 인스턴스여야 합니다. `Channel` 인스턴스는 모든 사용자가 구독할 수 있는 공개 채널을 나타내며, `PrivateChannels`와 `PresenceChannels`는 [채널 인가](#authorizing-channels)가 필요한 비공개 채널을 나타냅니다.
...
`ShouldBroadcast` 인터페이스를 구현한 후에는 평소와 같이 [이벤트를 발생](/docs/master/events)시키기만 하면 됩니다. 이벤트가 발생하면 [대기열 작업](/docs/master/queues)이 지정한 브로드캐스트 드라이버를 사용하여 이벤트를 자동으로 브로드캐스트합니다.
...
> [!NOTE]
> 이러한 문제를 처리하는 방법에 대해 더 알아보려면 [대기열 작업과 데이터베이스 트랜잭션](/docs/master/queues#jobs-and-database-transactions)에 관한 문서를 참고하십시오.
...
HTTP 라우트와 마찬가지로, 채널 라우트도 암묵적 및 명시적 [라우트 모델 바인딩](/docs/master/routing#route-model-binding)을 활용할 수 있습니다. 예를 들어 문자열이나 숫자 형태의 주문 ID를 받는 대신, 실제 `Order` 모델 인스턴스를 요청할 수 있습니다.
...
> [!WARNING]
> HTTP 라우트 모델 바인딩과 달리, 채널 모델 바인딩은 자동 [암묵적 모델 바인딩 스코프 적용](/docs/master/routing#implicit-model-binding-scoping)을 지원하지 않습니다. 하지만 대부분의 채널은 단일 모델의 고유한 기본 키를 기준으로 범위를 제한할 수 있으므로, 이는 거의 문제가 되지 않습니다.
...
> [!NOTE]
> Laravel의 다른 많은 클래스와 마찬가지로, 채널 클래스도 [서비스 컨테이너](/docs/master/container)에 의해 자동으로 해결됩니다. 따라서 채널 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다.
...
`send` 메서드를 사용해 익명 이벤트를 브로드캐스트하면, 해당 이벤트는 처리를 위해 애플리케이션의 [큐](/docs/master/queues)로 디스패치됩니다. 하지만 이벤트를 즉시 브로드캐스트하려면 `sendNow` 메서드를 사용할 수 있습니다.
...
`ShouldRescue` 인터페이스를 구현한 이벤트는 브로드캐스트 시도 중 Laravel의 [rescue 헬퍼 함수](/docs/master/helpers#method-rescue)를 자동으로 사용합니다. 이 헬퍼는 모든 예외를 잡고, 로깅을 위해 애플리케이션의 예외 핸들러에 보고하며, 사용자의 작업 흐름을 중단하지 않고 애플리케이션이 정상적으로 계속 실행되도록 합니다.
...
[Laravel Echo를 설치하고 인스턴스화](#client-side-installation)했다면, 이제 Laravel 애플리케이션에서 브로드캐스트되는 이벤트를 수신할 준비가 된 것입니다. 먼저 `channel` 메서드를 사용하여 채널 인스턴스를 가져온 다음, `listen` 메서드를 호출하여 지정한 이벤트를 수신합니다.
...
[채널을 떠나지 않고](#leaving-a-channel) 특정 이벤트의 수신만 중지하려면 `stopListening` 메서드를 사용할 수 있습니다.
...
애플리케이션의 [Eloquent 모델](/docs/master/eloquent)이 생성, 수정 또는 삭제될 때 이벤트를 브로드캐스트하는 일은 흔합니다. 물론 이는 [Eloquent 모델 상태 변경에 대한 커스텀 이벤트를 정의](/docs/master/eloquent#events)하고 해당 이벤트에 `ShouldBroadcast` 인터페이스를 지정하여 쉽게 구현할 수 있습니다.
...
모델에 `BroadcastsEvents` 트레이트를 추가하고 모델의 `broadcastOn` 메서드를 정의했다면, 이제 클라이언트 측 애플리케이션에서 브로드캐스트된 모델 이벤트를 수신할 준비가 된 것입니다. 시작하기 전에 [이벤트 수신하기](#listening-for-events)에 대한 전체 문서를 참고하는 것이 좋습니다.

먼저 `private` 메서드를 사용하여 채널 인스턴스를 가져온 다음, `listen` 메서드를 호출하여 지정한 이벤트를 수신합니다. 일반적으로 `private` 메서드에 전달하는 채널 이름은 Laravel의 [모델 브로드캐스팅 규칙](#model-broadcasting-conventions)에 맞아야 합니다.

채널 인스턴스를 얻은 후에는 `listen` 메서드를 사용하여 특정 이벤트를 수신할 수 있습니다. 모델 브로드캐스트 이벤트는 애플리케이션의 `App\Events` 디렉터리에 있는 "실제" 이벤트와 연결되어 있지 않으므로, [이벤트 이름](#model-broadcasting-event-conventions) 앞에 `.`을 붙여 특정 네임스페이스에 속하지 않음을 나타내야 합니다. 각 모델 브로드캐스트 이벤트에는 모델의 모든 브로드캐스트 가능한 속성이 담긴 `model` 속성이 있습니다.
...
이벤트 브로드캐스팅을 [알림](/docs/master/notifications)과 함께 사용하면 JavaScript 애플리케이션은 페이지를 새로고침하지 않아도 새 알림이 발생하는 즉시 받을 수 있습니다. 시작하기 전에 [브로드캐스트 알림 채널](/docs/master/notifications#broadcast-notifications)을 사용하는 방법에 대한 문서를 반드시 읽어보십시오.
...
[채널을 나가지 않고](#leaving-a-channel) 알림 수신을 중지하고 싶다면 `stopListeningForNotification` 메서드를 사용할 수 있습니다.
