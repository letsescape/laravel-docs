...
시더를 생성하려면 `make:seeder` [Artisan 명령어](/docs/master/artisan)를 실행하세요. 프레임워크가 생성한 모든 시더는 `database/seeders` 디렉터리에 생성됩니다:

```shell
php artisan make:seeder UserSeeder
```

시더 클래스는 기본적으로 `run` 메서드 하나만 포함합니다. 이 메서드는 `db:seed` [Artisan 명령어](/docs/master/artisan)가 실행될 때 호출됩니다. `run` 메서드 안에서 자유롭게 데이터베이스에 데이터를 삽입할 수 있습니다. [쿼리 빌더](/docs/master/queries)를 사용해 수동으로 데이터를 삽입하거나, [Eloquent 모델 팩토리](/docs/master/eloquent-factories)를 사용할 수도 있습니다.
...
> [!NOTE]
> `run` 메서드의 시그니처에 필요한 의존성을 타입 힌트로 지정할 수 있으며, Laravel의 [서비스 컨테이너](/docs/master/container)를 통해 자동으로 주입됩니다.

<a name="using-model-factories"></a>
### 모델 팩토리 사용하기

각 모델 시드에 속성을 일일이 지정하는 것은 번거롭습니다. 대신, [모델 팩토리](/docs/master/eloquent-factories)를 사용하면 대량의 데이터 레코드를 간편하게 생성할 수 있습니다. 먼저, 팩토리를 정의하는 방법은 [모델 팩토리 문서](/docs/master/eloquent-factories)를 참고하세요.
