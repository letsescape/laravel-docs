...
컴포넌트에 주입된 콘텐츠는 내부적으로 컴포넌트의 `$slot` 변수에 전달된다는 점을 기억하세요. 또한, `layout` 컴포넌트는 `$title` 슬롯이 전달된 경우 이를 활용하고, 없을 경우에는 기본 제목을 출력합니다. 아래와 같이 [컴포넌트 문서](#components)에서 설명한 일반적인 슬롯 사용법을 통해 커스텀 제목을 별도로 지정할 수 있습니다.
