...
따라서, 반드시 `env` 함수는 오직 애플리케이션의 `config`(설정) 파일 내부에서만 호출해야 합니다. 이 원칙은 라라벨 기본 설정 파일의 예시를 참조하면 쉽게 이해할 수 있습니다. 애플리케이션 내 어디에서든 [위에서 설명한](#accessing-configuration-values) `config` 함수를 통해 설정값을 사용할 수 있습니다.
