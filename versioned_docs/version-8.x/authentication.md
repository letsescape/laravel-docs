...
인증 서비스는 `Auth` [facade](/docs/8.x/facades)를 통해 사용할 수 있습니다. 먼저 클래스 상단에 `Auth` 파사드를 임포트해야 합니다. 다음으로, `attempt` 메서드를 살펴봅니다. `attempt` 메서드는 일반적으로 애플리케이션의 "로그인" 폼에서 인증 시도 시 사용합니다. 인증에 성공하면, [session](https://en.wikipedia.org/wiki/Session_fixation) 공격을 방지하기 위해 사용자의 [session fixation](/docs/8.x/session)을 반드시 재생성해야 합니다.
