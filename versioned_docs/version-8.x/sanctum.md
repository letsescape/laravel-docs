...
그리고 Pusher의 인가 요청이 올바르게 처리되도록, [Laravel Echo](/docs/8.x/broadcasting#client-side-installation)를 초기화할 때 Pusher의 `authorizer` 옵션을 커스텀으로 지정해 주어야 합니다. 이렇게 하면 애플리케이션에서 [교차 도메인 요청에 맞게 올바르게 설정된](#cors-and-cookies) `axios` 인스턴스를 사용하도록 Pusher를 구성할 수 있습니다.
