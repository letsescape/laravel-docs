...
[Laravel Breeze](https://github.com/laravel/breeze)는 로그인, 회원가입, 비밀번호 재설정, 이메일 인증, 비밀번호 확인 등 Laravel의 모든 [authentication features](/docs/8.x/authentication)을 간결하고 단순하게 구현한 스타터 키트입니다. Breeze의 기본 뷰 레이어는 간단한 [Blade templates](/docs/8.x/blade)으로 구성되며, 이 템플릿은 [Tailwind CSS](https://tailwindcss.com)로 스타일링되어 있습니다.
