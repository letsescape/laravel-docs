...
> [!NOTE]
> [Eloquent](/docs/13.x/eloquent) 쿼리의 결과는 항상 `Collection` 인스턴스로 반환됩니다.
...
일반적으로 [service provider](/docs/13.x/providers)의 `boot` 메서드에서 컬렉션 매크로를 선언해야 합니다.
...
`chunkBy` 메서드는 주어진 키 또는 콜백에 대해 같은 값을 갖는 인접한 항목을 그룹화하여 컬렉션을 여러 개의 작은 컬렉션으로 나눕니다. 예를 들어, 같은 부모를 공유하는 인접한 상품을 그룹화할 수 있습니다.
...
`groupBy` 메서드와 달리 같은 값을 가지더라도 서로 인접하지 않은 항목은 별도의 청크에 배치됩니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent Collections](/docs/13.x/eloquent-collections#method-contains)을 사용할 때 수정됩니다.
...
`crossJoin` 메서드는 주어진 배열 또는 컬렉션 사이에서 컬렉션의 값을 교차 조인하여 가능한 모든 조합을 포함하는 데카르트 곱을 반환합니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent Collections](/docs/13.x/eloquent-collections#method-diff)을 사용할 때 수정됩니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent Collections](/docs/13.x/eloquent-collections#method-except)을 사용할 때 수정됩니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent Collections](/docs/13.x/eloquent-collections#method-intersect)을 사용할 때 수정됩니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent Collections](/docs/13.x/eloquent-collections#method-only)을 사용할 때 수정됩니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent collections](/docs/13.x/eloquent-collections#method-partition)과 상호 작용할 때 수정됩니다.
...
`reduceInto` 메서드는 주어진 초기값을 변경하여 컬렉션을 단일 값으로 줄입니다. `reduce` 메서드와 달리, 주어진 콜백은 누적된 값을 반환할 필요가 없습니다:
...
스칼라 또는 배열로 축약할 때는 콜백에서 해당 값을 참조로 받아야 변경 사항이 원래 값에 적용됩니다.
...
`toArray` 메서드는 컬렉션을 일반 PHP `array`로 변환합니다. 컬렉션의 값이 [Eloquent](/docs/13.x/eloquent) 모델인 경우 모델도 배열로 변환됩니다.
...
> [!NOTE]
> 이 메서드의 동작은 [Eloquent Collections](/docs/13.x/eloquent-collections#method-unique)을 사용할 때 수정됩니다.
...
이 메서드의 사용법을 설명하기 위해 커서를 사용하여 데이터베이스에서 송장을 제출하는 애플리케이션을 상상해 보십시오. 15분마다 실행되고 최대 14분 동안만 송장을 처리하는 [scheduled task](/docs/13.x/scheduling)을 정의할 수 있습니다.
