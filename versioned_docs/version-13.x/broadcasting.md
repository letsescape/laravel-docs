...
애플리케이션의 [Eloquent 모델](/docs/13.x/eloquent)가 생성, 업데이트 또는 삭제될 때 이벤트를 브로드캐스트하는 것이 일반적입니다. 물론 이는 수동으로 [Eloquent 모델 상태 변경에 대한 사용자 지정 이벤트 정의](/docs/13.x/eloquent#이벤트)하고 해당 이벤트를 `ShouldBroadcast` 인터페이스로 표시하여 쉽게 수행할 수 있습니다.
