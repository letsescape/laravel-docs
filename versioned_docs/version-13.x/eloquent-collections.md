...
`append` 메소드는 컬렉션의 모든 모델에 대해 속성이 [추가](/docs/13.x/eloquent-serialization#appending-values-to-json)되어야 함을 나타내는 데 사용될 수 있습니다. 이 메소드는 속성 배열 또는 단일 속성을 허용합니다.
...
`mergeVisible` 메소드는 기존 가시 속성을 유지하면서 [추가 속성을 표시](/docs/13.x/eloquent-serialization#hiding-attributes-from-json)합니다.
