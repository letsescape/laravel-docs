...
또는 파일 시스템 디스크의 설정 배열에 `report` 옵션을 정의할 수 있습니다. 이 옵션을 `true`로 설정하면 쓰기 작업이 실패할 때 Laravel이 애플리케이션의 예외 핸들러를 사용해 실패 원인인 예외를 로그에 기록합니다. 이때 예외를 던지거나 쓰기 작업의 반환 값을 바꾸지는 않습니다.
...
`throw`와 `report` 옵션 중 어느 것도 정의하지 않으면 디스크는 실패 시 별도의 알림 없이 `false`를 반환하며, 실패 원인인 예외를 던지거나 로그에 기록하지 않습니다.
...
`copyToDisk` 및 `moveToDisk` 메서드를 사용해 파일을 다른 디스크로 복사하거나 이동할 수 있습니다. 세 번째 인수를 제공하지 않으면 소스 파일의 경로가 대상 디스크에서 사용됩니다.
