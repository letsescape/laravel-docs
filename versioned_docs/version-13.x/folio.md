...
Folio를 사용할 때는 항상 [Laravel의 라우트 캐싱 기능](/docs/13.x/routing#route-caching)을 활용해야 합니다. Folio는 `route:cache` Artisan 명령어를 감지하여, Folio 페이지 정의와 라우트 이름이 최대 성능을 위해 제대로 캐싱되도록 합니다.
