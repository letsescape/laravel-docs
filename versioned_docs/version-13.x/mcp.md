...
Laravel MCP는 Laravel의 [validation features](/docs/13.x/validation)과 매끄럽게 통합됩니다. 도구의 `handle` 메서드 안에서 들어오는 도구 인수를 검증할 수 있습니다.
...
Laravel [service container](/docs/13.x/container)는 모든 도구를 해결하는 데 사용됩니다. 따라서 도구의 생성자에서 필요한 의존성을 타입 힌트할 수 있습니다. 선언된 의존성은 자동으로 해결되어 도구 인스턴스에 주입됩니다.
...
Laravel MCP는 Laravel의 [validation features](/docs/13.x/validation)과 자연스럽게 통합됩니다. 프롬프트의 `handle` 메서드 안에서 들어오는 프롬프트 인수의 유효성을 검증할 수 있습니다.
...
Laravel [service container](/docs/13.x/container)는 모든 프롬프트를 해결하는 데 사용됩니다. 따라서 프롬프트의 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다. 선언된 의존성은 자동으로 해결되어 프롬프트 인스턴스에 주입됩니다.
...
Laravel [service container](/docs/13.x/container)는 모든 리소스를 해결하는 데 사용됩니다. 따라서 리소스의 생성자에서 필요한 의존성을 타입 힌트로 지정할 수 있습니다. 선언된 의존성은 자동으로 해결되어 리소스 인스턴스에 주입됩니다.
...
전체 클라이언트 측 API와 스키마 세부 사항을 포함한 완전한 프로토콜 참조는 공식 [MCP Apps documentation](https://modelcontextprotocol.io/extensions/apps/overview)를 참고하십시오.
...
MCP 클라이언트는 서버와 그 primitive에 대한 아이콘을 표시할 수 있습니다. `Icon` 속성을 사용하여 서버, 도구, 리소스 또는 프롬프트에 아이콘을 선언할 수 있습니다.
...
`Icon` 속성은 반복 가능하므로, 여러 아이콘을 선언해 서로 다른 크기나 라이트/다크 테마 변형을 제공할 수 있습니다.
...
또는 런타임 조건에 따라 아이콘이 달라지는 경우처럼 프로그래밍 방식으로 아이콘을 정의해야 한다면, `icons` 메서드를 재정의할 수 있습니다.
...
`$request->user()` 메서드를 통해 현재 인증된 사용자에 접근할 수 있으며, 이를 사용해 MCP 도구와 리소스 안에서 [authorization checks](/docs/13.x/authorization)을 수행할 수 있습니다.
...
서버를 빌드하는 것 외에도, Laravel MCP에는 first-party 또는 third-party MCP 서버에 연결하기 위한 클라이언트가 포함되어 있습니다. 이 클라이언트를 사용하면 애플리케이션이 MCP 서버가 노출하는 도구를 발견하고 호출할 수 있으며, 특히 [AI agents](/docs/13.x/ai-sdk#mcp-tools)가 외부 MCP 서버에서 제공하는 기능에 접근하게 할 때 유용합니다.
...
HTTP로 접근 가능한 MCP 서버에는 서버 URL을 전달하여 `Client::web` 메서드로 연결할 수 있습니다.
...
명령어로 실행되는 로컬 MCP 서버에 연결하려면, 서버를 시작하는 데 필요한 명령과 인수를 제공하여 `Client::local` 메서드를 사용합니다.
...
필요할 때마다 클라이언트를 구성하는 대신, 재사용 가능한 이름이 지정된 클라이언트를 등록할 수 있습니다. 일반적으로 서비스 프로바이더의 `boot` 메서드에서 `Mcp` 파사드를 사용해 등록합니다.
...
등록한 뒤에는 애플리케이션 어디에서든 이름으로 클라이언트를 해결할 수 있습니다.
...
이름이 지정된 클라이언트는 요청마다 한 번만 해결되며, 요청 라이프사이클이 끝날 때 자동으로 연결이 해제됩니다.
...
[OAuth 2.1](#oauth)로 보호되는 서버의 경우 `withOAuth` 메서드를 사용해 클라이언트를 설정합니다. 이는 OAuth로 자체 서버를 보호하는 방식의 클라이언트 측 대응 기능입니다.
...
`tools` 메서드를 사용하여 MCP 서버가 노출하는 도구를 가져올 수 있습니다. 이 메서드는 이름을 키로 하는 도구 컬렉션을 반환합니다.
...
도구를 호출하려면 도구 이름과 인수 배열을 전달하여 `callTool` 메서드를 사용합니다. 반환되는 `ToolResult` 인스턴스는 도구 응답을 노출합니다.
...
또는 목록으로 조회한 도구 인스턴스에서 직접 도구를 호출할 수도 있습니다.
...
[Laravel AI SDK](/docs/13.x/ai-sdk)로 에이전트를 빌드하는 경우, MCP 클라이언트의 도구를 에이전트에 직접 제공하여 모델이 프롬프트에 응답하는 동안 해당 도구를 호출하게 할 수도 있습니다. 자세한 내용은 AI SDK 문서의 [MCP Tools](/docs/13.x/ai-sdk#mcp-tools) 섹션을 참고하십시오.
...
`prompts` 메서드를 사용하여 MCP 서버가 노출하는 프롬프트를 가져올 수 있습니다. 이 메서드는 이름을 키로 하는 프롬프트 컬렉션을 반환합니다.
...
프롬프트를 가져오려면 프롬프트 이름과 인수 배열을 전달하여 `getPrompt` 메서드를 사용합니다. 반환되는 `PromptResult` 인스턴스는 생성된 메시지를 노출합니다.
...
`resources` 메서드를 사용하여 MCP 서버가 노출하는 리소스를 가져올 수 있습니다. 이 메서드는 URI를 키로 하는 리소스 컬렉션을 반환합니다.
...
리소스를 읽으려면 리소스 URI를 전달하여 `readResource` 메서드를 사용합니다. 반환되는 `ResourceReadResult` 인스턴스는 리소스 콘텐츠를 노출합니다.
