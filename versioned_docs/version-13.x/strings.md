...
주어진 문자열에 배열의 값 중 하나라도 포함되어 있는지 확인하기 위해 값의 배열을 전달할 수도 있습니다.
...
메서드에 두 번째 인수로 전달하여 중복 제거에 사용할 다른 문자를 지정할 수 있습니다:
...
주어진 문자열이 배열의 어떤 값으로도 끝나지 않는지 확인하기 위해 값의 배열을 전달할 수도 있습니다:
...
`radius` 옵션은 기본값이 `100`이며, 잘린 문자열의 양쪽에 표시할 문자 수를 정의할 수 있습니다.
...
기본적으로 Markdown은 원시 HTML을 지원하므로, 원시 사용자 입력과 함께 사용하면 Cross-Site Scripting (XSS) 취약점이 노출될 수 있습니다. [CommonMark Security documentation](https://commonmark.thephpleague.com/security/)에 따라 `html_input` 옵션을 사용해 원시 HTML을 이스케이프하거나 제거하고, `allow_unsafe_links` 옵션을 사용해 안전하지 않은 링크의 허용 여부를 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면 컴파일된 Markdown을 HTML Purifier를 통해 처리해야 합니다:
...
`isUrl` 메서드는 다양한 프로토콜을 유효한 것으로 간주합니다. 하지만 `isUrl` 메서드에 프로토콜을 전달해 유효한 것으로 간주할 프로토콜을 지정할 수 있습니다.
...
주어진 UUID가 버전(1, 3, 4, 5, 6, 7 또는 8)에 따른 UUID 사양과 일치하는지도 검증할 수 있습니다.
...
문자열을 자를 때 완전한 단어를 유지하려면 `preserveWords` 인수를 사용할 수 있습니다. 이 인수가 `true`이면 문자열은 가장 가까운 완전한 단어 경계에서 잘립니다:
...
<!-- By default, Markdown supports raw HTML, which will expose Cross-Site Scripting (XSS) vulnerabilities when used with raw user input. As per the [CommonMark Security documentation](https://commonmark.thephpleague.com/security/), you may use the `html_input` option to either escape or strip raw HTML, and the `allow_unsafe_links` option to specify whether to allow unsafe links. If you need to allow some raw HTML, you should pass your compiled Markdown through an HTML Purifier: -->
기본적으로 Markdown은 원시 HTML을 지원하므로, 신뢰할 수 없는 사용자 입력과 함께 사용하면 교차 사이트 스크립팅(XSS) 취약점이 노출됩니다. [CommonMark Security documentation](https://commonmark.thephpleague.com/security/)에 따라 `html_input` 옵션을 사용해 원시 HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 원시 HTML을 허용해야 한다면 컴파일된 Markdown을 HTML Purifier에 전달해야 합니다:
...
표현식에 일치 그룹을 지정하면 Laravel은 첫 번째 일치 그룹의 일치 항목 컬렉션을 반환합니다.
...
<!-- You may provide an integer as a second argument to the function to retrieve the singular or plural form of the string: -->
문자열의 단수형 또는 복수형을 가져오기 위해 함수에 두 번째 인수로 정수를 전달할 수 있습니다:
...
`replaceMatches` 메서드는 지정한 패턴과 일치하는 문자열의 각 부분에 대해 호출되는 클로저도 인수로 받을 수 있습니다. 이를 통해 클로저 내부에서 대체 로직을 수행하고 대체된 값을 반환할 수 있습니다.
...
<!-- You may also pass an array of values to determine if the given string doesn't end with any of the values in the array: -->
주어진 문자열이 배열의 어떤 값으로도 끝나지 않는지 확인하기 위해 값 배열을 전달할 수도 있습니다:
...
<!-- The `radius` option, which defaults to `100`, allows you to define the number of characters that should appear on each side of the truncated string. -->
`100`이 기본값인 `radius` 옵션을 사용하면 잘린 문자열의 양쪽에 표시할 문자 수를 지정할 수 있습니다.
...
<!-- By default, Markdown supports raw HTML, which will expose Cross-Site Scripting (XSS) vulnerabilities when used with raw user input. As per the [CommonMark Security documentation](https://commonmark.thephpleague.com/security/), you may use the `html_input` option to either escape or strip raw HTML, and the `allow_unsafe_links` option to specify whether to allow unsafe links. If you need to allow some raw HTML, you should pass your compiled Markdown through an HTML Purifier: -->
기본적으로 Markdown은 raw HTML을 지원하므로, 신뢰할 수 없는 사용자 입력과 함께 사용하면 Cross-Site Scripting (XSS) 취약점이 발생할 수 있습니다. [CommonMark Security documentation](https://commonmark.thephpleague.com/security/)에 따라 `html_input` 옵션을 사용해 raw HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 raw HTML을 허용해야 한다면 컴파일된 Markdown을 HTML Purifier에 전달해야 합니다.
...
<!-- The `isUrl` method considers a wide range of protocols as valid. However, you may specify the protocols that should be considered valid by providing them to the `isUrl` method: -->
`isUrl` 메서드는 다양한 프로토콜을 유효한 것으로 간주합니다. 그러나 `isUrl` 메서드에 프로토콜을 전달하여 유효한 것으로 간주할 프로토콜을 지정할 수도 있습니다.
...
<!-- If you would like to preserve complete words when truncating the string, you may utilize the `preserveWords` argument. When this argument is `true`, the string will be truncated to the nearest complete word boundary: -->
문자열을 자를 때 완전한 단어를 유지하려면 `preserveWords` 인수를 사용할 수 있습니다. 이 인수가 `true`이면 문자열은 가장 가까운 완전한 단어 경계까지 잘립니다.
...
<!-- By default, Markdown supports raw HTML, which will expose Cross-Site Scripting (XSS) vulnerabilities when used with raw user input. As per the [CommonMark Security documentation](https://commonmark.thephpleague.com/security/), you may use the `html_input` option to either escape or strip raw HTML, and the `allow_unsafe_links` option to specify whether to allow unsafe links. If you need to allow some raw HTML, you should pass your compiled Markdown through an HTML Purifier: -->
기본적으로 Markdown은 raw HTML을 지원하므로, 신뢰할 수 없는 사용자 입력과 함께 사용하면 Cross-Site Scripting (XSS) 취약점이 발생할 수 있습니다. [CommonMark Security documentation](https://commonmark.thephpleague.com/security/)에 설명된 대로 `html_input` 옵션을 사용해 raw HTML을 이스케이프하거나 제거할 수 있으며, `allow_unsafe_links` 옵션으로 안전하지 않은 링크를 허용할지 지정할 수 있습니다. 일부 raw HTML을 허용해야 한다면, 컴파일된 Markdown을 HTML Purifier에 전달해야 합니다:
...
<!-- If you specify a matching group within the expression, Laravel will return a collection of the first matching group's matches: -->
표현식 내에서 일치 그룹을 지정하면 Laravel은 첫 번째 일치 그룹의 일치 결과 컬렉션을 반환합니다:
...
<!-- The `replaceMatches` method also accepts a closure that will be invoked with each portion of the string matching the given pattern, allowing you to perform the replacement logic within the closure and return the replaced value: -->
`replaceMatches` 메서드는 주어진 패턴과 일치하는 문자열의 각 부분에 대해 호출되는 클로저도 인수로 받을 수 있습니다. 이를 통해 클로저 내에서 대체 로직을 수행하고 대체된 값을 반환할 수 있습니다:
...
<!-- You may also pass an array of values to determine if the given string contains any of the values in the array: -->
주어진 문자열에 배열의 값 중 하나라도 포함되어 있는지 확인하기 위해 값 배열을 전달할 수도 있습니다:
