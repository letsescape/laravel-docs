...
> [!NOTE]
> [concept overview](#concept-overview)를 아직 읽지 않았다면, 이 문서를 계속 읽기 전에 먼저 읽어보는 것을 강력히 권장합니다.
...
> [!NOTE]
> 연관관계가 이미 로드된 경우에만 포함하고 싶다면 [conditional relationships](#conditional-relationships)에 대한 문서를 확인하세요.
...
가장 바깥쪽 리소스의 래핑을 비활성화하려면 기본 `Illuminate\Http\Resources\Json\JsonResource` 클래스에서 `withoutWrapping` 메서드를 호출해야 합니다. 일반적으로 이 메서드는 애플리케이션으로 들어오는 모든 요청에서 로드되는 `AppServiceProvider` 또는 다른 [service provider](/docs/13.x/providers)에서 호출해야 합니다.
...
페이지네이션 응답의 `links` 키나 `meta` 키에 포함되는 정보를 커스터마이징하고 싶다면, 리소스에 `paginationInformation` 메서드를 정의할 수 있습니다. 이 메서드는 `$paginated` 데이터와 `links` 키와 `meta` 키를 포함하는 배열인 `$default` 정보 배열을 받습니다.
...
연관관계에서 [custom intermediate table model](/docs/13.x/eloquent-relationships#defining-custom-intermediate-table-models)을 사용한다면, 중간 테이블 모델의 인스턴스를 `whenPivotLoaded` 메서드의 첫 번째 인수로 전달할 수 있습니다.
...
중간 테이블이 `pivot` 이외의 accessor를 사용한다면 `whenPivotLoadedAs` 메서드를 사용할 수 있습니다.
...
Laravel은 [JSON:API specification](https://jsonapi.org/)를 준수하는 응답을 생성하는 리소스 클래스인 `JsonApiResource`를 제공합니다. 이 클래스는 표준 `JsonResource` 클래스를 확장하며, 리소스 객체 구조, 연관관계, 희소 필드셋(sparse fieldsets), 포함 항목(includes), 지연 속성 평가(lazy attribute evaluation)를 자동으로 처리하고 `Content-Type` 헤더를 `application/vnd.api+json`으로 설정합니다.

> [!NOTE]
> Laravel의 JSON:API 리소스는 응답의 직렬화를 처리합니다. 필터나 정렬 같은 들어오는 JSON:API 쿼리 파라미터도 파싱해야 한다면, [Spatie's Laravel Query Builder](https://spatie.be/docs/laravel-query-builder)가 함께 사용하기 좋은 패키지입니다.
...
JSON:API 리소스는 [sparse fieldsets](https://jsonapi.org/format/#fetching-sparse-fieldsets)을 지원합니다. 이를 통해 클라이언트는 `fields` 쿼리 파라미터를 사용하여 각 리소스 타입에 대해 특정 속성만 요청할 수 있습니다.
