...
Laravel의 인증 서비스는 `Auth` [facade](/docs/13.x/facades)를 통해 사용하므로, 먼저 클래스 상단에서 `Auth` 파사드를 가져와야 합니다. 다음으로 `attempt` 메서드를 살펴보겠습니다. `attempt` 메서드는 보통 애플리케이션의 로그인 폼에서 들어온 인증 시도를 처리할 때 사용합니다. 인증에 성공했다면 [session](https://en.wikipedia.org/wiki/Session_fixation)을 방지하기 위해 사용자의 [session fixation](/docs/13.x/session)을 다시 생성해야 합니다.
