...
라라벨을 처음 접한다면 [스타터 키트](/docs/13.x/starter-kits)를 살펴볼 것을 권장합니다. 라라벨의 스타터 키트는 내부적으로 Fortify를 사용해 Tailwind CSS로 만든 사용자 인터페이스가 있는 인증 스캐폴딩을 제공합니다. 이를 통해 라라벨의 인증 기능을 쉽게 익힐 수 있습니다.
...
요청이 실패하면, 사용자는 로그인 화면으로 돌아가며 유효성 검증 오류 정보는 공유된 `$errors` [Blade 템플릿 변수](/docs/13.x/validation#quick-displaying-the-validation-errors)로 확인할 수 있습니다. XHR 요청의 경우 422 HTTP 응답과 함께 유효성 검증 오류가 반환됩니다.
...
실패할 경우 사용자는 회원가입 화면으로 되돌아가며, 유효성 검증 오류 정보는 공유된 `$errors` [Blade 템플릿 변수](/docs/13.x/validation#quick-displaying-the-validation-errors)로 제공됩니다. XHR 요청인 경우 422 HTTP 오류와 함께 반환됩니다.

<a name="customizing-registration"></a>
### 회원가입 사용자 지정

사용자 유효성 검증 및 생성 프로세스는 Fortify 설치 시 생성된 `App\Actions\Fortify\CreateNewUser` 액션 파일을 수정하여 사용자 지정할 수 있습니다.
