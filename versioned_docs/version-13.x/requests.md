...
현재 HTTP 요청 인스턴스를 의존성 주입(dependency injection)을 통해 얻으려면, 라우트 클로저 또는 컨트롤러 메서드에서 `Illuminate\Http\Request` 클래스를 타입힌트로 지정하면 됩니다. Laravel의 [서비스 컨테이너](/docs/13.x/container)는 자동으로 요청 인스턴스를 주입합니다.
...
`routeIs` 메서드를 사용하면 들어온 요청이 [네임드 라우트](/docs/13.x/routing#named-routes)와 일치하는지 확인할 수 있습니다.
...
AI 에이전트처럼 Markdown 응답을 소비하는 클라이언트에 응답할 때 요청이 명시적으로 Markdown을 선호하는지, 혹은 다른 콘텐츠 타입과 함께 Markdown도 허용하는지 판별해야 한다면 `wantsMarkdown` 및 `acceptsMarkdown` 메서드를 사용할 수 있습니다:
...
`collect` 메서드를 사용해 입력값을 [컬렉션](/docs/13.x/collections)으로 받을 수도 있습니다.
...
입력값을 원시 `string`이 아닌 [Illuminate\Support\Stringable](/docs/13.x/strings) 인스턴스로 받고 싶다면, `string` 메서드를 사용하세요.
...
Laravel에서는 한 번의 요청에서 제출한 입력값을 다음 요청까지 유지할 수 있습니다. 이 기능은 유효성 검증에서 에러가 발생한 후, 폼을 다시 채워주는 데 유용합니다. Laravel의 [유효성 검증 기능](/docs/13.x/validation)을 사용할 때는 해당 메서드를 직접 사용할 일이 줄어들 수 있습니다. 일부 내장 기능에서 자동으로 입력값을 세션에 플래시(flashing)하기 때문입니다.

<a name="flashing-input-to-the-session"></a>
#### 입력값을 세션에 플래시하기

`Illuminate\Http\Request` 클래스의 `flash` 메서드는, 현재의 입력값을 [세션](/docs/13.x/session)에 플래시하여 다음 요청 때 사용 가능하게 해줍니다.
...
이전 요청에서 플래시된 입력값을 가져오려면, `Illuminate\Http\Request` 인스턴스의 `old` 메서드를 호출합니다. 이 값들은 [세션](/docs/13.x/session)에서 꺼내옵니다.

```php
$username = $request->old('username');
```

Laravel은 전역 `old` 헬퍼도 제공합니다. [Blade 템플릿](/docs/13.x/blade)에서 폼을 다시 채워줄 때는 이 헬퍼를 사용하는 것이 더욱 편리합니다. 만약 해당 입력값이 없다면 `null`을 반환합니다.
...
업로드된 파일을 저장하려면 보통 미리 설정된 [파일 시스템](/docs/13.x/filesystem) 중 하나를 사용합니다. `UploadedFile` 클래스의 `store` 메서드는 업로드 파일을 로컬/클라우드 저장소(예: Amazon S3)의 디스크로 이동시킵니다.
...
> [!NOTE]
> Laravel 파일 저장소에 대한 자세한 정보는 [파일 저장 문서](/docs/13.x/filesystem)를 참고하세요.
