...
> [!NOTE]
> Eloquent 모델 및 컬렉션의 JSON 직렬화를 더욱 강력하게 처리하려면, [Eloquent API 리소스](/docs/13.x/eloquent-resources) 문서를 참고하시기 바랍니다.
...
모델과 로드된 [연관관계](/docs/13.x/eloquent-relationships)를 배열로 변환하려면 `toArray` 메서드를 사용합니다. 이 메서드는 재귀적으로 동작하여 모든 속성과 연관관계(연관관계의 연관관계까지 포함)를 배열로 변환합니다:
...
또한, 전체 [컬렉션](/docs/13.x/eloquent-collections)을 배열로 변환하려면 컬렉션 인스턴스에서 `toArray` 메서드를 호출하면 됩니다:
...
모델과 컬렉션은 문자열로 캐스팅될 때 자동으로 JSON으로 변환되므로, 애플리케이션의 라우트나 컨트롤러에서 Eloquent 객체를 바로 반환할 수 있습니다. Laravel은 라우트나 컨트롤러에서 반환된 Eloquent 모델과 컬렉션을 자동으로 JSON으로 직렬화합니다:
...
Eloquent 모델이 JSON으로 변환될 때, 로드된 연관관계도 자동으로 JSON 객체의 속성으로 포함됩니다. 참고로 Eloquent의 연관관계 메서드는 “카멜 케이스(camel case)”로 정의되어 있지만, JSON 속성명은 “스네이크 케이스(snake case)”로 변환됩니다.

<a name="hiding-attributes-from-json"></a>
## JSON에서 속성 숨기기 (Hiding Attributes From JSON)

경우에 따라 비밀번호처럼 모델의 배열 또는 JSON 표현에 포함하지 않고 싶은 속성이 있을 수 있습니다. 이럴 때는 모델에 `Hidden` 속성을 사용할 수 있습니다. `Hidden` 속성에 나열된 속성들은 직렬화된 표현에서 제외됩니다:
...
> [!NOTE]
> 연관관계를 숨기려면, 연관관계의 메서드명을 Eloquent 모델의 `Hidden` 속성에 추가하십시오.

반대로 `Visible` 속성을 사용하여 배열과 JSON 표현에 반드시 포함해야 할 속성의 "허용 목록"을 정의할 수도 있습니다. `Visible` 속성에 포함되지 않은 속성들은 모델이 배열이나 JSON으로 변환될 때 숨겨집니다:
...
가끔 모델을 배열이나 JSON으로 변환할 때, 데이터베이스 컬럼에 직접 대응되는 속성이 없는 값을 추가하고 싶을 수 있습니다. 이런 경우, 먼저 해당 값에 대한 [접근자(accessor)](/docs/13.x/eloquent-mutators)를 정의하세요:
...
접근자를 항상 모델의 배열 및 JSON 표현에 포함하고 싶다면 모델에 `Appends` 속성을 사용할 수 있습니다. 주의할 점은 속성명은 일반적으로 "스네이크 케이스" 형태로 참조된다는 것입니다. 접근자 메서드는 PHP에서 "카멜 케이스"로 정의됩니다.
...
특정 Eloquent 날짜 속성별로 직렬화 포맷을 정의하려면, 모델의 [캐스트 선언](/docs/13.x/eloquent-mutators#attribute-casting)에서 날짜 포맷을 지정하면 됩니다:
