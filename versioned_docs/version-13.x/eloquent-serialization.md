...
> [!NOTE]
> Eloquent 모델과 컬렉션의 JSON 직렬화를 더욱 견고하게 처리하려면 [Eloquent API 리소스](/docs/13.x/eloquent-resources) 문서를 확인하세요.
...
모델과 로드된 [연관관계](/docs/13.x/eloquent-relationships)를 배열로 변환하려면 `toArray` 메서드를 사용해야 합니다. 이 메서드는 재귀적으로 동작하므로 모든 속성과 모든 연관관계(연관관계의 연관관계 포함)가 배열로 변환됩니다.
...
컬렉션 인스턴스에서 `toArray` 메서드를 호출하여 모델의 전체 [컬렉션](/docs/13.x/eloquent-collections)을 배열로 변환할 수도 있습니다.
...
모델을 배열이나 JSON으로 변환할 때, 데이터베이스에 대응되는 컬럼이 없는 속성을 추가하고 싶을 때가 있습니다. 이를 위해 먼저 해당 값에 대한 [접근자](/docs/13.x/eloquent-mutators)를 정의합니다.
...
모델의 [cast 선언](/docs/13.x/eloquent-mutators#attribute-casting)에 날짜 형식을 지정하여 개별 Eloquent 날짜 속성의 직렬화 형식을 사용자 지정할 수 있습니다.
