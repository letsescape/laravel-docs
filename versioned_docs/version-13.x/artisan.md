...
[Laravel Sail](/docs/13.x/sail)을 로컬 개발 환경으로 사용하는 경우 Artisan 명령어를 실행할 때 `sail` 명령을 사용해야 합니다. Sail은 애플리케이션의 Docker 컨테이너 안에서 Artisan 명령어를 실행합니다:
...
명령어를 생성한 후에는 `Signature` 및 `Description` 속성을 사용해 명령어의 시그니처와 설명을 정의해야 합니다. `Signature` 속성을 통해 [명령어의 입력 기대값](#defining-input-expectations)도 정의할 수 있습니다. 명령어가 실행되면 `handle` 메서드가 호출되며, 명령어 로직은 이 메서드에 작성합니다.

다음은 명령어 예시입니다. `handle` 메서드에서 필요한 의존성을 요청할 수 있다는 점에 주목하세요. Laravel의 [서비스 컨테이너](/docs/13.x/container)는 명시적으로 타입힌트된 모든 의존성을 자동으로 주입합니다:
...
클로저 명령어에서도 명령어 인수 및 옵션 외에 추가적인 의존성을 [서비스 컨테이너](/docs/13.x/container)에서 타입힌트 방식으로 주입받을 수 있습니다:
...
> [!NOTE]
> [Laravel Prompts](/docs/13.x/prompts) 공식 문서에서 사용 가능한 프롬프트 유형들과 더 많은 옵션을 확인할 수 있습니다.
...
> [!NOTE]
> [Laravel Prompts](/docs/13.x/prompts)는 커맨드라인 애플리케이션에서 플레이스홀더, 유효성검사 등 브라우저와 유사한 사용자 경험을 제공하는 아름답고 사용하기 쉬운 폼 생성을 도와주는 PHP 패키지입니다.
...
Artisan이 부팅되면, 애플리케이션의 모든 명령어가 [서비스 컨테이너](/docs/13.x/container)에서 해결(resolve)되어 Artisan에 등록됩니다.
...
`Artisan` 파사드의 `queue` 메서드를 사용하면 Artisan 명령어를 [큐 워커](/docs/13.x/queues)에서 백그라운드로 처리하게 할 수 있습니다. 사용전에 큐 환경 설정과 큐 리스너 동작을 반드시 확인하세요:
