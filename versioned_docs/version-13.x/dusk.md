...
> [!NOTE]
> 로컬 개발 환경을 관리하기 위해 [Laravel Sail](/docs/13.x/sail)을 사용하고 있다면, [configuring and running Dusk tests](/docs/13.x/sail#laravel-dusk)에 관한 Sail 문서도 함께 참고하십시오.
...
> [!NOTE]
> 로컬 개발 환경을 관리하기 위해 [Laravel Sail](/docs/13.x/sail)을 사용하고 있다면, [configuring and running Dusk tests](/docs/13.x/sail#laravel-dusk)에 관한 Sail 문서를 참고하십시오.
...
`visitRoute` 메서드를 사용하여 [named route](/docs/13.x/routing#named-routes)로 이동할 수 있습니다.
...
여러 테스트에서 재사용할 수 있는 사용자 정의 브라우저 메서드를 정의하려면 `Browser` 클래스의 `macro` 메서드를 사용할 수 있습니다. 일반적으로 이 메서드는 [service provider's](/docs/13.x/providers)의 `boot` 메서드에서 호출해야 합니다.
...
> [!NOTE]
> `{command}`와 같은 모든 수정 키는 `{}` 문자로 감싸며, `Facebook\WebDriver\WebDriverKeys` 클래스에 정의된 상수와 일치합니다. 이 클래스는 [found on GitHub](https://github.com/php-webdriver/php-webdriver/blob/master/lib/WebDriverKeys.php).
...
테스트 스위트 전체에서 쉽게 재사용할 수 있는 사용자 정의 키보드 상호작용을 정의하고 싶다면 `Keyboard` 클래스가 제공하는 `macro` 메서드를 사용할 수 있습니다. 일반적으로 이 메서드는 [service provider's](/docs/13.x/providers) `boot` 메서드에서 호출해야 합니다.
...
[named route's](/docs/13.x/routing#named-routes)의 위치를 기다릴 수도 있습니다.
...
`waitUntilVue` 및 `waitUntilVueIsNot` 메서드는 [Vue component](https://vuejs.org) 속성이 지정한 값을 가질 때까지 기다리는 데 사용할 수 있습니다.
...
현재 URL이 지정한 [named route's](/docs/13.x/routing#named-routes)의 URL과 일치하는지 어설션합니다.
...
Dusk를 사용하면 [Vue component](https://vuejs.org) 데이터의 상태에 대해서도 검증을 수행할 수 있습니다. 예를 들어, 애플리케이션에 다음 Vue 컴포넌트가 포함되어 있다고 가정해 보겠습니다:
...
기본적으로 페이지에는 `url`, `assert`, `elements` 세 가지 메서드가 있습니다. 먼저 `url` 및 `assert` 메서드를 살펴보겠습니다. `elements` 메서드는 [discussed in more detail below](#shorthand-selectors).
...
데이터베이스 사용 방법을 포함하여 Chipper CI에서 Dusk 테스트를 실행하는 방법을 더 알아보려면 [official Chipper CI documentation](https://chipperci.com/docs/testing/laravel-dusk-new/)를 참고하십시오.
