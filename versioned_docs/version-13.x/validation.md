...
그렇다면 수신 요청 필드가 지정된 유효성 검사 규칙을 통과하지 못하면 어떻게 될까요? 이전에 언급했듯이 Laravel는 자동으로 사용자를 이전 위치로 다시 리디렉션합니다. 또한 모든 유효성 검사 오류와 [요청 입력](/docs/13.x/requests#retrieving-old-input)은 자동으로 [세션에 플래시](/docs/13.x/session#flash-data)됩니다.
...
또한 이 파일을 다른 언어 디렉터리에 복사하여 애플리케이션 언어에 대한 메시지를 번역할 수도 있습니다. Laravel 현지화에 대해 자세히 알아보려면 전체 [현지화 문서](/docs/13.x/localization)를 확인하세요.
...
`@error` [Blade](/docs/13.x/blade) 지시어를 사용하면 특정 속성에 대한 유효성 검사 오류 메시지가 있는지 빠르게 확인할 수 있습니다. `@error` 지시문 내에서 `$message` 변수를 에코하여 오류 메시지를 표시할 수 있습니다.
...
유효성 검사 오류로 인해 Laravel가 리디렉션 응답을 생성하면 프레임워크는 자동으로 [요청의 모든 입력을 세션에 플래시](/docs/13.x/session#flash-data)합니다. 이는 다음 요청 시 입력에 편리하게 액세스하고 사용자가 제출하려고 시도한 양식을 다시 채울 수 있도록 하기 위한 것입니다.

이전 요청에서 깜박인 입력을 검색하려면 `Illuminate\Http\Request` 인스턴스에서 `old` 메서드를 호출합니다. `old` 메서드는 [세션](/docs/13.x/session)에서 이전에 플래시된 입력 데이터를 가져옵니다.

```php
$title = $request->old('title');
```

Laravel는 글로벌 `old` 도우미도 제공합니다. [Blade 템플릿](/docs/13.x/blade) 내에서 이전 입력을 표시하는 경우 `old` 도우미를 사용하여 양식을 다시 채우는 것이 더 편리합니다. 해당 필드에 대한 이전 입력이 없으면 `null`가 반환됩니다.
...
> [!NOTE]
> `rules` 메서드의 서명 내에서 필요한 종속성을 유형 힌트로 지정할 수 있습니다. Laravel [서비스 컨테이너](/docs/13.x/container)를 통해 자동으로 해결됩니다.
...
> [!NOTE]
> Inertia 기반 Laravel 프론트엔드에 실시간 양식 요청 확인을 추가해야 합니까? [Laravel 예지](/docs/13.x/precognition)를 확인해보세요.
...
요청 클래스에 `StopOnFirstFailure` 속성을 추가하면 단 하나의 유효성 검사가 실패한 뒤 모든 속성의 유효성 검사를 중지하도록 validator에 알릴 수 있습니다.
...
양식 요청 유효성 검사가 실패하면 사용자를 이전 위치로 다시 보내기 위한 리디렉션 응답이 생성됩니다. 그러나 이 동작을 자유롭게 사용자 지정할 수 있습니다. 이를 위해 양식 요청에 `RedirectTo` 속성을 사용할 수 있습니다.
...
또는 사용자를 명명된 라우트로 리디렉션하려는 경우 대신 `RedirectToRoute` 속성을 사용할 수 있습니다.
...
양식 요청 유효성 검사가 실패하면 오류는 `default` 오류 백에 플래시됩니다. 오류를 다른 [명명된 오류 백](#named-error-bags)에 저장해야 한다면 양식 요청에 `ErrorBag` 속성을 사용할 수 있습니다:
...
양식 요청 클래스에는 `authorize` 메소드도 포함되어 있습니다. 이 방법 내에서 인증된 사용자가 실제로 특정 리소스를 업데이트할 권한이 있는지 확인할 수 있습니다. 예를 들어 사용자가 업데이트하려는 블로그 댓글을 실제로 소유하고 있는지 확인할 수 있습니다. 아마도 다음 방법 내에서 [승인 게이트 및 정책](/docs/13.x/authorization)과 상호 작용하게 될 것입니다.
...
따라서 애플리케이션이 [라우트 모델 바인딩](/docs/13.x/routing#route-model-binding)을 활용하는 경우 확인된 모델을 요청의 속성으로 액세스하여 코드를 더욱 간결하게 만들 수 있습니다.
...
> [!NOTE]
> `authorize` 메서드의 서명 내에서 필요한 종속성을 유형 힌트로 지정할 수 있습니다. Laravel [서비스 컨테이너](/docs/13.x/container)를 통해 자동으로 해결됩니다.
...
요청에 `validate` 메서드를 사용하지 않으려면 `Validator` [facade](/docs/13.x/facades)를 사용하여 수동으로 유효성 검사기 인스턴스를 생성할 수 있습니다. 파사드의 `make` 메소드는 새로운 유효성 검사기 인스턴스를 생성합니다:
...
검증된 데이터를 [컬렉션](/docs/13.x/collections) 인스턴스로 검색하려면 `collect` 메소드를 호출할 수 있습니다.
...
검증 중인 필드는 인증된 사용자의 비밀번호와 일치해야 합니다. 규칙의 첫 번째 매개변수를 사용하여 [인증 가드](/docs/13.x/authentication)를 지정할 수 있습니다.
...
편의를 위해 문자열 유효성 검사 규칙은 fluently 연결 가능한 `Rule::string()` 규칙 빌더를 사용해 구성할 수도 있습니다:
...
문자열 규칙 빌더는 `alpha`, `alphaDash`, `alphaNumeric`, `ascii`, `between`, `doesntEndWith`, `doesntStartWith`, `endsWith`, `exactly`, `lowercase`, `max`, `min`, `startsWith`, `uppercase` 등 일반적인 문자열 제약 조건용 메서드를 제공합니다. 또한 이 규칙 빌더는 조건부 적용이 가능하므로 `when` 및 `unless` 메서드를 사용해 제약 조건을 조건부로 적용할 수도 있습니다.
...
`$fail` 클로저에 리터럴 오류 메시지를 제공하는 대신 [번역 문자열 키](/docs/13.x/localization)를 제공하고 Laravel에 오류 메시지를 번역하도록 지시할 수도 있습니다.
