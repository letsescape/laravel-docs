...
- [대시보드](#dashboard)
    ...
    - [사용자 지정](#dashboard-customization)
...
개별 이벤트에 대한 심층적인 디버깅이 필요한 경우에는 [Laravel Telescope](/docs/13.x/telescope)를 참고하시기 바랍니다.
...
> [!NOTE]
> 인증된 사용자를 캡처하고 조회하는 방법을 완전히 사용자 지정하려면 `Laravel\Pulse\Contracts\ResolvesUsers` 계약을 구현하고, Laravel의 [서비스 컨테이너](/docs/13.x/container#binding-a-singleton)에 바인딩할 수 있습니다.
...
Pulse가 사용자 정보를 어떻게 조회하고 표시하는지 사용자 지정하는 방법은 [사용자 해석](#dashboard-resolving-users) 문서를 참고하세요.
...
`<livewire:pulse.slow-outgoing-requests />` 카드는 Laravel의 [HTTP 클라이언트](/docs/13.x/http-client)로 발생시킨 외부 요청 중 설정된 임계값(기본값 1,000ms)을 초과한 요청을 보여줍니다.
...
> [!NOTE]
> Pulse는 [캐시](/docs/13.x/cache)를 사용해 재시작 신호를 저장하므로, 이 기능을 사용하기 전에 애플리케이션에 캐시 드라이버가 제대로 구성되어 있는지 확인하세요.
...
`CacheInteractions` 레코더는 [캐시](/docs/13.x/cache)의 적중/실패 정보를 [캐시](#cache-card) 카드에 표시하기 위해 수집합니다.
...
`SlowOutgoingRequests` 레코더는 Laravel의 [HTTP 클라이언트](/docs/13.x/http-client)를 사용해 발생한 외부 요청 중 임계값을 초과한 요청 정보를 [느린 외부 요청](#slow-outgoing-requests-card) 카드에 표시합니다.
...
Pulse에서 사용하는 [데이터베이스 연결](/docs/13.x/database#configuration)은 `PULSE_DB_CONNECTION` 환경 변수를 통해 지정할 수 있습니다.
...
Pulse는 기본적으로 기본 [Redis 연결](/docs/13.x/redis#configuration)을 사용하지만, `PULSE_REDIS_CONNECTION` 환경 변수로 따로 지정할 수 있습니다:
...
> [!NOTE]
> Pulse는 [캐시](/docs/13.x/cache)를 이용해 재시작 신호를 저장하므로, 이 기능을 사용하기 전에 캐시 드라이버가 애플리케이션에 제대로 구성되어 있는지 확인하세요.
...
Pulse는 대시보드에서 설정한 기간을 벗어난 저장 엔트리를 자동으로 정리합니다. 트리밍은 데이터 인제스트 시 로터리 시스템을 통해 동작하며, Pulse [설정 파일](#configuration)에서 사용자 지정할 수 있습니다.
...
커스텀 카드가 애플리케이션 코드베이스 내에 있고, 라라벨 [Vite 통합](/docs/13.x/vite)을 사용한다면, `vite.config.js` 파일에서 카드용 CSS 엔트리포인트를 추가할 수 있습니다:
...
> [!NOTE]
> 현재 인증된 사용자 ID를 기록하는 카드 패키지를 만들 때는, 애플리케이션에서 [사용자 해석 사용자 지정](#dashboard-resolving-users)을 고려하여 반드시 `Pulse::resolveAuthenticatedUserId()` 메서드를 사용해야 합니다.
