...
번역 실행 제한 시간은 `TRANSLATION_RUN_TIMEOUT_SECONDS`의 검증된 값으로 계산한다. 계산 시점은 설정과 프롬프트 검증을 마친 직후이며, upstream 동기화를 시작하기 전이다. upstream 동기화 시간도 남은 provider 실행 시간에서 차감되지만, upstream 작업 자체는 각 작업의 timeout을 사용한다. 별도 상위 오케스트레이터 deadline은 사용하지 않는다.
...
- checkout 인증은 읽기 전용으로 사용하고 작업 트리에 유지하지 않는다. push 자격 증명은 커밋 단계에서만 주입한다.
