...
flowchart TD
    ...
    G --> H["문서/사이드바 검증"]
    H --> J["변경분 main 직접 커밋 (변경 없으면 생략)"]
    J --> K["배포 단계: 타입/빌드/앵커 검증 후 배포 (deploy.yml)"]
    K --> Z([동기화 완료])
```
...
```text
영어 원문 갱신 -> diff 산출 -> 전처리 -> 번역 -> 후처리 -> 사이드바 갱신 -> 검증 -> main 반영 -> 배포(타입/빌드/앵커 검증)
```
...
- 문서 검증을 통과한 변경분이 `main`에 직접 커밋되어 있다(변경이 없으면 커밋하지 않는다). 타입·빌드 검증은 커밋 이후 배포 단계(`deploy.yml`)에서 수행한다. 세부 기준은 8절을 따른다.

---

## 8. 산출물 git 반영 기준

동기화 산출물은 사람 개입 없이 자동으로 `main`에 직접 반영한다. 이로써 별도 조작 없이 `main`이 항상 최신 upstream 기준으로 갱신된다.

반영 기준:

- 문서 검증(`main.py`의 `verify`)을 통과한 변경분만 `main`에 커밋한다. 문서 검증이 실패하면 어떤 변경도 커밋하지 않는다.
- 반영할 변경분(diff)이 없으면 커밋을 만들지 않고 종료한다. 빈 커밋을 만들지 않는다.
- 변경분은 `main`에 직접 커밋하며, 별도 통합 브랜치나 Pull Request를 경유하지 않는다.
- 별도 통합 브랜치(예: `develop`)는 동기화 산출 경로로 사용하지 않는다.
- 타입 검사·빌드·앵커 검증은 커밋 전 게이트가 아니라 커밋 이후 배포 단계(`deploy.yml`)에서 수행한다. 기본 토큰 push는 배포를 자동 발화하지 않으므로 동기화 워크플로가 커밋 후 배포를 명시적으로 트리거하며, 빌드가 실패하면 배포만 중단되고 커밋은 그대로 남는다.
- 커밋이 끝난 상태가 동기화 완료 상태이며, 사이트 반영 여부는 배포 단계의 타입/빌드/앵커 검증 통과에 달려 있다.
