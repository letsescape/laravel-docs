...
동기화한 영어 `documentation.md`를 단일 기준으로 모든 버전의 sidebar JSON과 locale override 삭제 계획을 결정적으로 생성.
전체 계획의 issue 확인과 입력 hash 재확인을 통과한 뒤 각 출력을 작업 트리에 순차적으로 원자 기록하고 결과를 재검증.
...
| 항목 | 설명 |
|---|---|
| 동기화한 영어 원문 `documentation.md` | 각 버전의 사이드바 구조를 선언하는 단일 기준 문서 |
...
| sidebar 출력 계획 | 전체 JSON byte와 삭제 경로, 검증 입력 hash를 포함한 검증 단위 |
...
4. **Fail-closed**: 어느 버전에서든 parsing 또는 출력 issue 발생 시 sidebar 단계 실패 처리 및 전체 sidebar 출력 계획 거부.
5. **검증 후 기록**: 모든 버전의 sidebar JSON과 locale override 삭제 계획을 검증한 뒤, 각 파일을 순차적으로 원자 기록하거나 삭제. 적용 중 I/O 실패 시 이미 완료된 앞선 변경은 남을 수 있음.
...
3. 각 버전에 대해:
   ...
   c. issue 있으면 sidebar 출력 계획 전체 거부 및 단계 실패
...
4. 모든 버전이 통과하면 sidebar 출력 계획과 검증 입력 hash 확정
5. 계획을 다시 수립해 입력 hash 일치 재확인
6. sidebar 출력 계획을 파일별로 작업 트리에 순차적 원자 기록
...
- `versions`는 `versions.json`의 처리 순서와 같고 version당 entry 하나만 가짐.
  `documentation_sha256`와 `generated_sidebar_sha256`는 각각 동기화한 `documentation.md`와 생성 JSON의 정확한 byte를 hash함.
- `baseline_sidebar_sha256`는 기존 sidebar가 있으면 해당 byte의 SHA-256, 없으면 명시적 `null`.
...
- 생성 시작과 작업 트리 기록 직전에 모든 입력과 출력 byte를 각 소유 경로에서 다시 읽어 envelope와 hash를 재계산해야 함.
...
| `documentation.md` 파싱 issue | sidebar 출력 계획 거부, 단계 실패 |
| 출력 경로가 저장소 밖 또는 symlink | sidebar 출력 계획 거부, 단계 실패 |
| 기존 sidebar JSON 스키마 위반 | sidebar 출력 계획 거부, 단계 실패 |
| doc id에 대응하는 영어 원문 파일 부재 | sidebar 출력 계획 거부, 단계 실패 |
| 기록 전 입력 hash 변경 | sidebar 출력 계획 거부, 단계 실패 |
...
9. 전체 계획의 issue 확인과 입력 hash 재확인을 통과하기 전에는 작업 트리의 sidebar 파일을 변경하지 않아야 하며, 순차 적용 뒤 byte와 삭제 결과를 재검증해야 함.
10. 작업 트리에 기록된 sidebar 변경이 검증된 sidebar 출력 계획과 byte 단위로 일치해야 함.
11. 같은 입력에서 생성한 sidebar JSON byte가 process 재실행과 무관하게 같아야 함.

## 종료 상태 의미

sidebar 검증 실패는 번역 품질 문제가 아닌 sidebar sync 단계 자체의 실패로 분류.
provider 재시도 없이 실행을 실패 처리하고 전체 동기화를 새로 실행해야 함.

전체 동기화 진입점에서 sidebar 실패는 [공통 종료 코드 계약](./08-error-cases.md#7-진입점-종료-코드-계약)의 `1`(통제된 workflow 실패)로 반영.
