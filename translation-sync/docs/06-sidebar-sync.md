...
candidate의 영어 `documentation.md`를 단일 기준으로 모든 버전의 sidebar JSON과 locale override 삭제 계획을 결정적으로 생성한다. 전체 산출을 검증한 뒤에만 candidate snapshot에 한 번에 적재하며, 하나라도 실패하면 sidebar candidate 전체를 폐기한다.
...
3. **저장소 오염 방지**: 출력 경로가 저장소 밖을 가리키면 거부한다. symlink를 따라 외부에 쓰지 않는다. 기존 sidebar JSON이 symlink이면 읽거나 덮어쓰지 않는다. 검증 전에는 active worktree나 candidate snapshot을 변경하지 않는다.
4. **Fail-closed**: 어느 버전에서든 parsing 또는 산출 issue가 발생하면 sidebar 단계를 실패시키고 전체 sidebar candidate set을 폐기한다.
5. **원자적 공개**: 모든 버전의 sidebar JSON과 locale override 삭제 계획은 검증된 하나의 candidate tree 변경으로만 공개해야 한다.
...
검증 입력 hash는 다음 canonical JSON envelope byte의 SHA-256이다.
...
- `versions_sha256`는 정확한 `versions.json` byte의 SHA-256이다.
- `versions`는 `versions.json`의 처리 순서와 같고 version당 entry 하나만 가진다. `documentation_sha256`와 `generated_sidebar_sha256`는 각각 candidate `documentation.md`와 생성 JSON의 정확한 byte를 hash한다.
- `baseline_sidebar_sha256`는 승인 기준본 sidebar가 있으면 그 정확한 byte의 SHA-256, 없으면 명시적 `null`이다. 빈 파일의 hash와 부재를 같게 취급하지 않는다.
- `override_deletions`는 실제로 삭제할 locale override의 canonical 저장소 상대 경로를 UTF-8 byte 순으로 정렬한 배열이며 중복을 허용하지 않는다.
- top-level과 version entry는 예시의 필드만 가져야 한다. Object key는 UTF-8 byte 순으로 재귀 정렬하고, 불필요한 공백 없는 UTF-8 JSON 뒤에 LF 하나를 붙인다.
- 생성 시작과 candidate 적재 직전에 모든 입력과 산출 byte를 각 소유 경로에서 다시 읽어 envelope와 hash를 재계산해야 한다. 시작 snapshot 객체를 다시 hash하는 것으로 대신해서는 안 된다.
...
- category key는 `category:<label>`이다.
- doc key는 문서 순서상 첫 항목이면 `doc:<id>`, 같은 id의 두 번째부터는 1부터 센 전체 occurrence를 suffix로 붙인 `doc:<id>:<occurrence>`다.
- link key는 raw target byte의 SHA-256 lowercase hex를 사용한 `link:<digest>`이며 같은 target이 반복되면 doc key와 같은 occurrence suffix를 붙인다.
- 서로 다른 raw target이 같은 link digest를 만들면 key를 추정 재할당하지 않고 sidebar issue로 실패한다.
- 동일 doc id가 여러 category에 등장하는 것은 허용하며 위 occurrence 규칙으로 구별한다.
- 기존 category의 `collapsed`는 같은 category key의 boolean 값만 보존한다. key가 없으면 `true`, 값이 boolean이 아니면 schema issue다.
- `master` sidebar의 루트 API Documentation link URL은 `versions.json` 최신 안정 버전으로 정규화한다.
- `versions.json`은 `master` 다음에 안정 버전이 하나 이상 있어야 하며, 그 첫 항목을 최신 안정 버전으로 사용한다.
- 산출 JSON은 배열 순서를 유지하고 object key를 UTF-8 byte 순으로 재귀 정렬한 뒤 2-space indent, UTF-8, LF와 마지막 newline 1개로 직렬화한다.
...
7. category·doc·link의 translation key가 정의된 생성 규칙을 따르고 중복되지 않는다.
8. 지원하지 않는 문법이 조용히 생략되지 않았다.
9. 모든 산출 검증이 완료되기 전에 candidate snapshot이나 active worktree의 sidebar 파일을 변경하지 않았다.
10. candidate snapshot에 적재된 sidebar 변경이 검증된 sidebar candidate set과 byte 단위로 일치한다.
11. 같은 입력에서 생성한 sidebar JSON byte가 process 재실행과 무관하게 같다.

## 종료 상태 의미

sidebar 검증 실패는 번역 품질 문제가 아니라 sidebar sync 단계 자체의 실패로 분류한다. provider 재시도 없이 현재 candidate를 폐기하고 전체 워크플로우를 새로 실행해야 한다.

전체 동기화 진입점에서 sidebar 실패는 [공통 종료 코드 계약](./08-error-cases.md#8-진입점-종료-코드-계약)의 `1`(통제된 workflow 실패)로 반영된다.
