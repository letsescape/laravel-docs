...
이 문서는 `translation-sync` 디렉터리의 단계별 작업 문서를 전처리, 번역, 후처리, 검증, main 반영 순서로 요약한다.
...
flowchart TD
    ...
    E -- " 실패: 후처리 형식 오류 " --> D
    E -- " 실패: 사이드바 누락 / 라벨 불일치 / locale JSON 잔존 " --> S
    G -- " 변경 없음: 커밋 생략 " --> F
    F -- " 실패: 최종 산출 기준 미달 " --> E
```
...
```text
전처리 -> 번역 -> 후처리 -> 사이드바 갱신 -> 검증 -> main 반영 -> 최종 산출
```
...
|  4 | 사이드바 갱신 | [06-sidebar-sync.md](06-sidebar-sync.md)     | -                        | `documentation.md`를 기준으로 버전별 sidebar JSON을 갱신하고 locale sidebar JSON을 제거한다. |
|  5 | 검증      | [04-verification.md](04-verification.md)     | -                        | diff 반영, 링크, 앵커, 코드, 이미지, Markdown 구조, 사이드바 정합성을 검증한다. |
|  6 | main 반영  | [05-additional-work.md](05-additional-work.md) | -                      | 검증을 통과한 변경분을 `main`에 직접 커밋한다. 변경이 없으면 커밋하지 않는다. |
...
- 검증 결과가 `success`일 때만 main 반영(커밋)을 수행하고, `failure`면 어떤 변경도 커밋하지 않는다.
- 반영할 변경분이 없으면 빈 커밋을 만들지 않고 종료한다.
- 변경분은 `main`에 직접 커밋하며 별도 통합 브랜치나 PR을 경유하지 않는다. 세부 기준은 [05-additional-work.md](05-additional-work.md)의 git 반영 기준을 따른다.
