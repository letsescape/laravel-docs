...
`apply` mode에서는 actionable plan의 번역·결정적 블록을 정제·복원해 PatchPlan을 적용한다. `no-write` mode에서는 locale byte를 유지한다. 두 mode 모두 영어 verification view와 expected annotation map을 생성해 문서 검증에 전달한다.
...
번역이 완료된 locale 문서의 Markdown 형식을 정제하고 전처리 placeholder와 보존 markup을 복구하거나, no-write 입력 byte를 그대로 유지한다. 두 경로에서 현재 영어 원문 기반 검증 기준을 생성한다.

---

## 2. 범위

후처리 단계가 소유하는 책임은 다음 다섯 가지로 한정한다.
...
11. `no-write` mode의 candidate locale 문서는 기존 locale 문서와 byte 단위로 같아야 한다.
12. provider가 반환한 pipeline annotation은 형식 복구의 기준으로 사용하지 않는다. `apply` mode의 최종 annotation은 expected annotation map byte로 교체해야 한다.
...
`apply` mode는 1~11을 모두 수행한다. `no-write` mode는 locale 변환·계획 적용 단계 1~7과 9를 건너뛰고 기존 locale byte를 그대로 사용하며, 8의 expected annotation map 생성, 10의 영어 verification view 생성과 11의 검증 입력 결합만 수행한다. `no-write` 결과가 검증에 실패해도 locale 문서를 자동 정규화하지 않는다.
...
stale link 정규화 규칙의 유일한 소스는 version-controlled `translation-sync/stale-links.json`이다. 후처리 단계가 schema 검증과 적용을 소유하고, 문서 검증 단계는 이 단계가 만든 영어 verification view, expected annotation map과 registry digest만 소비한다.
...
- `version`과 `from` 조합은 유일해야 하며 `links`는 두 값의 UTF-8 byte 순으로 정렬해야 한다.
...
- `entries`는 영어 문서 순서이며 structural address가 같은 항목은 1부터 시작하는 `occurrence`로 구별한다.
- annotatable 여부와 source text 경계는 [번역 단계](./02-translation.md)의 최종 문서 소유 단위·annotation 규칙을 사용한다. Provider 요청의 delta별 임시 경계는 map에 포함하지 않으며, 표는 항상 현재 표 전체가 단일 entry다.
- `annotation`은 여는 `<!--`, 공백, 정규화된 source text, 공백, 닫는 `-->`까지 포함한 완전한 canonical comment byte다.
- top-level과 entry는 예시의 필드만 가져야 한다. map은 임시 검증 산출물이며 candidate 또는 publication tree에 기록하지 않는다.
- map은 2-space indent, UTF-8, LF, 마지막 newline 1개와 예시의 key 순서로 직렬화한다.
- 이 canonical JSON byte의 SHA-256 digest를 검증 입력 hash에 포함한다.

### 7.8 검증 입력 hash

다음 envelope의 object key를 UTF-8 byte 순으로 정렬하고 불필요한 공백 없는 UTF-8 JSON 뒤에 LF 하나를 붙인 byte를 SHA-256으로 계산한다.
...
각 digest는 대응하는 정확한 UTF-8 artifact byte의 SHA-256이다. 검증기는 시작 snapshot과 artifact 생성 직전에 호출할 독립 final snapshot loader를 함께 받는다. Loader는 candidate locale, 영어 verification view, expected annotation map, version과 stale-link registry를 각 소유 source에서 새로 읽어 envelope와 hash를 다시 만들어야 한다. 시작 때 전달받은 객체나 registry를 그대로 다시 hash하는 것은 종료 재계산으로 인정하지 않는다. 두 snapshot의 canonical envelope가 다르면 verified locale artifact를 만들지 않는다.

---

## 8. placeholder 복원 세부

- 전처리에서 생성한 현재 restore map을 사용하여 `__BASE64_IMAGE_<N>__`을 현재 원문의 data URI로 치환한다.
- 매핑이 없으면 임의로 데이터를 생성하지 않는다.
- fenced code 밖에 미복원 placeholder가 남은 결과는 이 단계에서 `POSTPROCESS_RESIDUE`로 차단하고 검증 입력으로 전달하지 않는다. [문서 검증 단계](./04-verification.md)의 잔존 검사는 이 경계를 우회한 결함을 막는 방어적 검사이며, 어느 경우에도 locale 파일로 기록해서는 안 된다.
...
후처리는 provider 호출 실패를 복구하지 않는다. 후처리 결과는 active worktree나 candidate snapshot에 직접 기록하지 않고 [문서 검증 단계](./04-verification.md)에 전달해야 한다.
...
6. candidate locale의 annotation byte·순서·구조 주소가 expected annotation map과 일치한다.
7. 계획 밖의 locale 블록과 source-authored 구조가 변경되지 않았다.
8. 영어 verification view와 expected annotation map이 현재 정규화 영어 작업 사본, current restore map, version과 stale-link registry digest에서 결정적으로 생성되었다.
9. `no-write` mode에서는 candidate locale 문서와 기존 locale 문서의 byte가 같다.
