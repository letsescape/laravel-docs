...
`Log` [facade](/docs/11.x/facades) を使用して、ログに情報を書き込むことができます。前述したように、ロガーは、[RFC 5424 specification](https://tools.ietf.org/html/rfc5424) で定義された 8 つのログ レベル (**緊急**、**アラート**、**クリティカル**、**エラー**、**警告**、**通知**、**情報**、**デバッグ**) を提供します。
...
> [!NOTE]
> キューに入れられたジョブの処理中にログ コンテキストを共有する必要がある場合は、[job middleware](/docs/11.x/queues#job-middleware) を利用できます。
...
> [!NOTE]
> すべての「タップ」クラスは [service container](/docs/11.x/container) によって解決されるため、必要なコンストラクターの依存関係は自動的に挿入されます。
