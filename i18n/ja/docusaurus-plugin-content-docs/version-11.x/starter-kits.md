...
まず、[新しいLaravelアプリケーションを作成する](/docs/{{version}}/installation) を実行する必要があります。 [Laravelインストーラー](/docs/{{version}}/installation#creating-a-laravel-project) を使用してアプリケーションを作成する場合、インストールプロセス中に Laravel Breeze をインストールするように求められます。それ以外の場合は、以下の手動インストール手順に従う必要があります。
