...
Folio を使用する場合は、常に [Laravel's route caching capabilities](/docs/11.x/routing#route-caching) を活用する必要があります。 Folio は、`route:cache` Artisan コマンドをリッスンして、最大限のパフォーマンスを得るために Folio ページ定義とルート名が適切にキャッシュされていることを確認します。
