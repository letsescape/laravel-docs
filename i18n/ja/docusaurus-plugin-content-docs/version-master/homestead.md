...
> [!WARNING]
> Laravel Homestead は、現在は積極的にメンテナンスされていないレガシー パッケージです。 [Laravel Sail](/docs/{{version}}/sail) は最新の代替手段として使用できます。
...
Laravel は、単一の `schedule:run` Artisan コマンドを毎分実行するようにスケジュールすることで、[cron ジョブをスケジュールする](/docs/{{version}}/scheduling) を実行する便利な方法を提供します。 `schedule:run` コマンドは、`routes/console.php` ファイルに定義されているジョブ スケジュールを調べて、実行するスケジュールされたタスクを決定します。
...
Homestead 内で [Laravel Dusk](/docs/{{version}}/dusk) テストを実行するには、Homestead 構成で [ウェブドライバ機能](#installing-optional-features) を有効にする必要があります。
