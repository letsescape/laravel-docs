...
<!-- [Laravel Reverb](https://github.com/laravel/reverb) brings blazing-fast and scalable real-time WebSocket communication directly to your Laravel application, and provides seamless integration with Laravel's existing suite of [event broadcasting tools](/docs/master/broadcasting). -->
[Laravel Reverb](https://github.com/laravel/reverb) は、超高速でスケーラブルなリアルタイム WebSocket 通信を Laravel アプリケーションに直接もたらし、Laravel の既存の [event broadcasting tools](/docs/master/broadcasting) スイートとのシームレスな統合を提供します。
...
ただし、ローカル開発中など、Reverb サーバーが安全な接続を直接処理すると便利な場合があります。 [Laravel Herd's](https://herd.laravel.com) のセキュア サイト機能を使用している場合、または [Laravel Valet](/docs/master/valet) を使用していてアプリケーションに対して [secure command](/docs/master/valet#securing-sites) を実行している場合は、サイト用に生成された Herd / Valet 証明書を使用して Reverb 接続を保護できます。これを行うには、`REVERB_HOST` 環境変数をサイトのホスト名に設定するか、Reverb サーバーの起動時にホスト名オプションを明示的に渡します。
...
<!-- Reverb may be monitored via an integration with [Laravel Pulse](/docs/master/pulse). By enabling Reverb's Pulse integration, you may track the number of connections and messages being handled by your server. -->
Reverbは、[Laravel Pulse](/docs/master/pulse) との統合を通じてモニタリングできます。 Reverb の Pulse 統合を有効にすると、サーバーによって処理される接続とメッセージの数を追跡できます。

<!-- To enable the integration, you should first ensure you have [installed Pulse](/docs/master/pulse#installation). Then, add any of Reverb's recorders to your application's `config/pulse.php` configuration file: -->
統合を有効にするには、まず [installed Pulse](/docs/master/pulse#installation) があることを確認する必要があります。次に、Reverb のレコーダーのいずれかをアプリケーションの `config/pulse.php` 構成ファイルに追加します。
...
<!-- Next, add the Pulse cards for each recorder to your [Pulse dashboard](/docs/master/pulse#dashboard-customization): -->
次に、各レコーダーの Pulse カードを [Pulse dashboard](/docs/master/pulse#dashboard-customization) に追加します。
...
> [!NOTE]
> [Laravel Cloud](https://cloud.laravel.com) は、Laravel Reverb クラスターを活用したフルマネージド WebSocket インフラストラクチャを提供し、インフラストラクチャを管理せずに Reverb 対応アプリケーションを拡張および出荷できるようにします。
...
<!-- Next, you should have a dedicated, central Redis server to which all of the Reverb servers will communicate. Reverb will use the [default Redis connection configured for your application](/docs/master/redis#configuration) to publish messages to all of your Reverb servers. -->
次に、すべての Reverb サーバーが通信する専用の中央 Redis サーバーを用意する必要があります。 Reverb は [default Redis connection configured for your application](/docs/master/redis#configuration) を使用して、すべての Reverb サーバーにメッセージをパブリッシュします。
...
<!-- Reverb dispatches internal events during the lifecycle of a connection and message handling. You may [listen for these events](/docs/master/events) to perform actions when connections are managed or messages are exchanged. -->
Reverb は、接続とメッセージ処理のライフサイクル中に内部イベントをディスパッチします。接続が管理されているとき、またはメッセージが交換されているときに、[listen for these events](/docs/master/events) を使用してアクションを実行できます。

<!-- The following events are dispatched by Reverb: -->
次のイベントが Reverb によって送出されます。
...
チャネルの作成時に送出されます。これは通常、最初の接続が特定のチャネルにサブスクライブするときに発生します。イベントは `Laravel\Reverb\Protocols\Pusher\Channel` インスタンスを受け取ります。
...
チャネルが削除されたときに送出されます。これは通常、最後の接続がチャネルからサブスクライブを解除したときに発生します。イベントは `Laravel\Reverb\Protocols\Pusher\Channel` インスタンスを受け取ります。
...
古い接続がサーバーによって削除されるときに送出されます。イベントは `Laravel\Reverb\Contracts\Connection` インスタンスを受け取ります。
...
クライアント接続からメッセージを受信したときに送出されます。イベントは、`Laravel\Reverb\Contracts\Connection` インスタンスと生の文字列 `$message` を受け取ります。
...
メッセージがクライアント接続に送信されるときに送出されます。イベントは、`Laravel\Reverb\Contracts\Connection` インスタンスと生の文字列 `$message` を受け取ります。
