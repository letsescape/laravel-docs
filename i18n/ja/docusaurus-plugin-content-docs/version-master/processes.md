...
非同期プロセスの実行中に、`ensureNotTimedOut` メソッドを使用してプロセスがタイムアウトしていないことを確認できます。プロセスがタイムアウトした場合、このメソッドは [timeout exception](#timeouts) をスローします。
...
このクロージャ内で、プールに属するプロセスを定義できます。 `start` メソッドを介してプロセス プールが開始されると、`running` メソッドを介して実行中のプロセスの [collection](/docs/master/collections) にアクセスできます。
