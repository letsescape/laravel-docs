...
Laravel は [Amazon SQS FIFO (先入れ先出し)](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-fifo-queues.html) キューをサポートしているため、メッセージの重複排除によって正確に 1 回の処理を保証しながら、送信された正確な順序でジョブを処理できます。
