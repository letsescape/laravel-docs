...
最も外側のリソースのラッピングを無効にしたい場合は、基本 `Illuminate\Http\Resources\Json\JsonResource` クラスで `withoutWrapping` メソッドを呼び出す必要があります。通常、このメソッドは、`AppServiceProvider` またはアプリケーションへのリクエストごとにロードされる別の [service provider](/docs/master/providers) から呼び出す必要があります。
...
リレーションシップで [custom intermediate table model](/docs/master/eloquent-relationships#defining-custom-intermediate-table-models) を使用している場合は、中間テーブル モデルのインスタンスを最初の引数として `whenPivotLoaded` メソッドに渡すことができます。
...
Laravel には、[JSON:API specification](https://jsonapi.org/) に準拠した応答を生成するリソース クラス `JsonApiResource` が付属しています。これは、標準の `JsonResource` クラスを拡張し、リソース オブジェクトの構造、関係、スパース フィールドセットを自動的に処理し、`Content-Type` ヘッダーを `application/vnd.api+json` に組み込み、設定します。

> [!NOTE]
> Laravel の JSON:API リソースは、応答のシリアル化を処理します。受信した JSON:API クエリ パラメーター (フィルターや並べ替えなど) も解析する必要がある場合、[Spatie's Laravel Query Builder](https://spatie.be/docs/laravel-query-builder/v6/introduction) は優れたコンパニオン パッケージです。
