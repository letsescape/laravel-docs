...
Laravel には、[JSON:API仕様](https://jsonapi.org/) に準拠した応答を生成するリソース クラス `JsonApiResource` が付属しています。これは、標準の `JsonResource` クラスを拡張し、リソース オブジェクトの構造、関係、スパース フィールドセットを自動的に処理し、`Content-Type` ヘッダーを `application/vnd.api+json` に組み込み、設定します。

> [!NOTE]
> Laravel の JSON:API リソースは、応答のシリアル化を処理します。受信した JSON:API クエリ パラメーター (フィルターや並べ替えなど) も解析する必要がある場合、[Spatie の Laravel クエリビルダ](https://spatie.be/docs/laravel-query-builder/v6/introduction) は優れたコンパニオン パッケージです。
