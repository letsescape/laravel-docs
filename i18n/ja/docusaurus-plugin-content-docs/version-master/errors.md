...
<!-- In Laravel, exception reporting is used to log exceptions or send them to an external service like [Sentry](https://github.com/getsentry/sentry-laravel) or [Flare](https://flareapp.io). By default, exceptions will be logged based on your [logging](/docs/master/logging) configuration. However, you are free to log exceptions however you wish. -->
Laravel では、例外レポートを使用して例外をログに記録したり、例外を [Sentry](https://github.com/getsentry/sentry-laravel) や [Flare](https://flareapp.io) などの外部サービスに送信したりします。デフォルトでは、例外は [logging](/docs/master/logging) 構成に基づいて記録されます。ただし、例外を自由にログに記録することができます。
...
> [!NOTE]
> 特定の例外の例外レポートをカスタマイズするには、[reportable exceptions](/docs/master/errors#renderable-exceptions) を利用することもできます。
...
メッセージがアプリケーションの [logs](/docs/master/logging) に書き込まれる場合、メッセージは指定された [log level](/docs/master/logging#log-levels) に書き込まれます。これは、記録されるメッセージの重大度または重要性を示します。
...
Laravel は内部的に、404 HTTP エラーから生じる例外、オリジンの不一致によって生成された 403 HTTP 応答、無効な CSRF トークンによって生成された 419 HTTP 応答など、一部のタイプのエラーをすでに無視します。特定のタイプの例外の無視を停止するように Laravel に指示したい場合は、アプリケーションの `bootstrap/app.php` ファイルで `stopIgnoring` 例外メソッドを使用できます。
...
> [!NOTE]
> `report` メソッドの必要な依存関係をタイプヒントで指定すると、それらは Laravel の [service container](/docs/master/container) によってメソッドに自動的に挿入されます。
