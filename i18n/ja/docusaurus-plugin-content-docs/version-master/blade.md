...
`@session` ディレクティブは、[session](/docs/{{version}}/session) 値が存在するかどうかを判断するために使用できます。セッション値が存在する場合、`@session` および `@endsession` ディレクティブ内のテンプレートの内容が評価されます。 `@session` ディレクティブの内容内で、`$value` 変数をエコーし​​てセッション値を表示できます。
...
`@context` ディレクティブは、[context](/docs/{{version}}/context) 値が存在するかどうかを判断するために使用できます。コンテキスト値が存在する場合、`@context` および `@endcontext` ディレクティブ内のテンプレートの内容が評価されます。 `@context` ディレクティブの内容内で、`$value` 変数をエコーし​​てコンテキスト値を表示できます。
...
> [!NOTE]
> マージされた属性を受け取るべきではない他の HTML 要素のクラスを条件付きでコンパイルする必要がある場合は、[@classディレクティブ](#conditional-classes) を使用できます。
