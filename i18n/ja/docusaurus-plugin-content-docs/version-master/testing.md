...
テストを実行すると、`phpunit.xml` ファイルで定義された環境変数により、Laravel は自動的に [configuration environment](/docs/master/configuration#environment-configuration) を `testing` に設定します。また、Laravel はセッションとキャッシュを `array` ドライバに自動的に構成するため、テスト中にセッションやキャッシュのデータは保持されません。
...
> [!NOTE]
> テスト スタブは、[stub publishing](/docs/master/artisan#stub-customization) を使用してカスタマイズできます。
