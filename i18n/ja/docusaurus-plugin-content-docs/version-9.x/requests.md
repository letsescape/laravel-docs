...
リクエストの入力データをプリミティブ `string` として取得する代わりに、`string` メソッドを使用してリクエスト データを [`Illuminate\Support\Stringable`](/docs/{{version}}/helpers#fluent-strings) のインスタンスとして取得することもできます。
...
> **注記**
> AWS Elastic Load Balancing を使用している場合、`$headers` 値は `Request::HEADER_X_FORWARDED_AWS_ELB` である必要があります。 `$headers` プロパティで使用できる定数の詳細については、[信頼できるプロキシ](https://symfony.com/doc/current/deployment/proxies.html) にある Symfony のドキュメントを確認してください。
