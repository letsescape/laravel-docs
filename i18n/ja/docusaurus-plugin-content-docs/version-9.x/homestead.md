...
> [!WARNING]
> Apple Silicon を使用している場合は、`box: laravel/homestead-arm` を `Homestead.yaml` ファイルに追加する必要があります。 Apple Silicon には Parallels プロバイダが必要です。
...
Vagrant の [Synced Folders](https://www.vagrantup.com/docs/synced-folders/basic_usage.html) でサポートされているオプションを `options` キーの下にリストすることで渡すこともできます。
...
Homestead は、同じ仮想マシン上で複数のバージョンの PHP の実行をサポートします。 `Homestead.yaml` ファイル内の特定のサイトで使用する PHP のバージョンを指定できます。利用可能な PHP バージョンは、「5.6」、「7.0」、「7.1」、「7.2」、「7.3」、「7.4」、「8.0」、「8.1」、および「8.2」 (デフォルト) です。
...
`Homestead.yaml` ファイルで CLI で使用する PHP のバージョンを指定することもできます。
...
または、Homestead 仮想マシン内から次のコマンドを発行して手動で変更することもできます。
...
Homestead は、アプリケーションに必要なデータベースを自動的に作成できます。プロビジョニング プロセス中にデータベース サービスが実行されている場合、Homestead は、`Homestead.yaml` 構成ファイル内の各データベースがまだ存在していない場合は作成されることを確認します。
...
Laravel は、単一の `schedule:run` Artisan コマンドを毎分実行するようにスケジュールすることで、[schedule cron jobs](/docs/9.x/scheduling) を実行する便利な方法を提供します。 `schedule:run` コマンドは、`App\Console\Kernel` クラスで定義されたジョブ スケジュールを調べて、実行するスケジュールされたタスクを決定します。
...
Homestead 内で [Laravel Dusk](/docs/9.x/dusk) テストを実行するには、Homestead 構成で [`webdriver` feature](#installing-optional-features) を有効にする必要があります。
...
デフォルトでは、Xdebug はすでに実行されており、接続を受け入れる準備ができています。 CLI で Xdebug を有効または無効にする必要がある場合は、Homestead 仮想マシン内で `sudo phpenmod xdebug` または `sudo phpdismod xdebug` コマンドを実行します。
...
次に、IDE の指示に従ってデバッグを有効にします。最後に、拡張機能または [bookmarklet](https://www.jetbrains.com/phpstorm/marklets/) を使用して Xdebug をトリガーするようにブラウザを設定します。
