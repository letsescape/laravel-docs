...
> [!WARNING]
> pluralizer の言語をカスタマイズする場合は、Eloquent モデルの [table names](/docs/9.x/eloquent#table-names) を明示的に定義する必要があります。
...
[Blade templating engine](/docs/9.x/blade) を使用している場合は、`{{ }}` エコー構文を使用して翻訳文字列を表示できます。
