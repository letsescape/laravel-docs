...
受信リクエスト インスタンスを使用して署名付き URL を検証する代わりに、`Illuminate\Routing\Middleware\ValidateSignature` [middleware](/docs/{{version}}/middleware) をルートに割り当てることができます。まだ存在していない場合は、このミドルウェアに HTTP カーネルの `routeMiddleware` 配列内のキーを割り当てる必要があります。
