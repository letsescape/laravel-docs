...
言い換えれば、Valet は約 7 MB の RAM を使用する非常に高速な Laravel 開発環境です。 Valet は、[Sail](/docs/9.x/sail) または [Homestead](/docs/9.x/homestead) の完全な代替品ではありませんが、柔軟な基本が必要な場合、極端な速度を好む場合、または RAM の量が限られているマシンで作業している場合に優れた代替手段となります。
...
> [!NOTE]
> `valet share --region=eu` などの追加の Ngrok パラメーターを共有コマンドに渡すことができます。詳細については、[ngrok documentation](https://ngrok.com/docs) を参照してください。
