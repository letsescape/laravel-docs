...
> [!WARNING]
> 次の列タイプを変更できます: `bigInteger`、`binary`、`boolean`、`char`、`date`、`dateTime`、`dateTimeTz`、`decimal`、`double`、 `integer`、`json`、`longText`、`mediumText`、`smallInteger`、`string`、`text`、`time`、`tinyText`、 `unsignedBigInteger`、`unsignedInteger`、`unsignedSmallInteger`、および `uuid`。  `timestamp` 列タイプを変更するには、[Doctrine type must be registered](#prerequisites)の手順に従う必要があります。
