...
> [!NOTE]
> Laravel Horizon について詳しく知る前に、Laravel のベース [queue services](/docs/9.x/queues) についてよく理解しておく必要があります。 Horizon は、Laravel が提供する基本的なキュー機能にまだ慣れていない場合、混乱を招く可能性がある追加機能で Laravel のキューを強化します。
...
[Laravel Horizon](https://github.com/laravel/horizon) は、Laravel を利用した [Redis queues](/docs/9.x/queues) に美しいダッシュボードとコード駆動の構成を提供します。 Horizon を使用すると、ジョブのスループット、実行時間、ジョブの失敗など、キュー システムの主要なメトリクスを簡単に監視できます。
...
> [!WARNING]
> `horizon` 構成ファイルの `environments` 部分に、Horizon を実行する予定の各 [environment](/docs/9.x/configuration#environment-configuration) のエントリが含まれていることを確認する必要があります。
...
> [!WARNING]
> Slack または SMS 通知を送信するように Horizon を構成する場合は、[prerequisites for the relevant notification channel](/docs/9.x/notifications) を確認する必要があります。
...
Horizon には、ジョブとキューの待機時間とスループットに関する情報を提供するメトリクス ダッシュボードが含まれています。このダッシュボードにデータを入力するには、アプリケーションの [scheduler](/docs/9.x/scheduling) 経由で Horizon の `snapshot` Artisan コマンドを 5 分ごとに実行するように構成する必要があります。
