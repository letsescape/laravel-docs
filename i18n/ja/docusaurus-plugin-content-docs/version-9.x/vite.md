...
スクリプトとスタイル タグに [`nonce` attribute](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) の一部として [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/nonce) を含めたい場合は、カスタム [middleware](/docs/9.x/middleware) 内で `useCspNonce` メソッドを使用してノンスを生成または指定できます。
...
Laravel の [Ziggy `@route` directive](/docs/9.x/starter-kits) に含まれる [starter kits](https://github.com/tighten/ziggy#using-routes-with-a-content-security-policy) など、他の場所で nonce を指定する必要がある場合は、 `cspNonce` メソッドを使用して取得できます。
