...
Laravel の HTTP クライアントは Guzzle を利用しているため、[ガズルミドルウェア](https://docs.guzzlephp.org/en/stable/handlers-and-middleware.html) を利用して送信リクエストを操作したり、受信レスポンスを検査したりできます。送信リクエストを操作するには、`withMiddleware` メソッドを使用して Guzzle の `mapRequest` ミドルウェア ファクトリと組み合わせて Guzzle ミドルウェアを登録します。
