...
Laravel の HTTP クライアントは Guzzle を利用しているため、[Guzzle Middleware](https://docs.guzzlephp.org/en/stable/handlers-and-middleware.html) を利用して送信リクエストを操作したり、受信レスポンスを検査したりできます。送信リクエストを操作するには、`withMiddleware` メソッドを使用して Guzzle の `mapRequest` ミドルウェア ファクトリと組み合わせて Guzzle ミドルウェアを登録します。
...
同様に、`withMiddleware` メソッドと Guzzle の `mapResponse` ミドルウェア ファクトリを組み合わせてミドルウェアを登録することで、受信 HTTP 応答を検査できます。
