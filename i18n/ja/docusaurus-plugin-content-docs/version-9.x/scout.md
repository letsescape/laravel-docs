...
データベース エンジンを優先ドライバとして指定したら、[検索可能なデータを構成する](#configuring-searchable-data) を実行する必要があります。その後、モデルに対して [検索クエリの実行](#searching) を開始できます。データベース エンジンを使用する場合、Algolia インデックスや Meil​​iSearch インデックスのシードに必要なインデックス作成など、検索エンジンのインデックス作成は不要です。
