...
`Provider`、`Model`、および `Timeout` 属性に加えて、実行時にこれらの値を解決するために、エージェントで `provider`、`model`、および `timeout` メソッドを定義することもできます。これは、構成がデータベース レコード、構成値、またはその他の実行時の状態に依存する場合に便利です。 `maxSteps`、`maxTokens`、および `temperature` にも同じことが当てはまります。
...
メソッドと属性の両方が同じオプションに定義されている場合は、メソッドが優先されます。
...
Laravel AI SDK は、次のようなさまざまな [events](/docs/12.x/events) をディスパッチします。
