...
> [!WARNING]
> `onQueue` によるコンストラクターベースのキュー割り当ては、ジョブ クラスに対してのみ機能します。 [キューに入れられたイベントリスナ](/docs/{{version}}/events#customizing-the-queue-connection-queue-name) の場合は、代わりにリスナ クラスで `viaQueue` メソッドまたは `$queue` プロパティを定義します。
...
> [!NOTE]
> [キューに入れられたイベントリスナ](/docs/{{version}}/events#queued-event-listeners) および [キューに入れられた通知](/docs/{{version}}/notifications#queueing-notifications) で `tries` プロパティまたは `retryUntil` メソッドを定義することもできます。
