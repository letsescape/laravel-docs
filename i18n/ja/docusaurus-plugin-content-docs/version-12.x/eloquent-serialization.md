...
> [!NOTE]
> Eloquent モデルとコレクションの JSON シリアル化を処理するさらに堅牢な方法については、[Eloquent API resources](/docs/12.x/eloquent-resources) のドキュメントを確認してください。
...
モデルとそのロードされた [relationships](/docs/12.x/eloquent-relationships) を配列に変換するには、`toArray` メソッドを使用する必要があります。このメソッドは再帰的であるため、すべての属性とすべての関係 (関係の関係を含む) が配列に変換されます。
...
コレクション インスタンスで `toArray` メソッドを呼び出して、モデルの [collections](/docs/12.x/eloquent-collections) 全体を配列に変換することもできます。
...
場合によっては、モデルを配列または JSON に変換するときに、データベースに対応する列がない属性を追加したい場合があります。これを行うには、まず値の [accessor](/docs/12.x/eloquent-mutators) を定義します。
...
モデルの [cast declarations](/docs/12.x/eloquent-mutators#attribute-casting) で日付形式を指定することで、個々の Eloquent 日付属性のシリアル化形式をカスタマイズできます。
