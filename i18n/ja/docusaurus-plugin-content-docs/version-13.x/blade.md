...
Blade ビューは、グローバル `view` ヘルパを使用してルートまたはコントローラから返される場合があります。もちろん、[views](/docs/13.x/views) のドキュメントで説明されているように、`view` ヘルパの 2 番目の引数を使用してデータをBlade ビューに渡すこともできます。
...
Blade テンプレートを次のレベルに引き上げて、動的なインターフェイスを簡単に構築したいですか? [Laravel Livewire](https://livewire.laravel.com) をチェックしてください。 Livewire を使用すると、通常は React、Svelte、Vue などのフロントエンド フレームワークを介してのみ可能となる動的機能で拡張された Blade コンポーネントを作成できます。これにより、多くの JavaScript フレームワークの複雑さ、クライアント側のレンダリング、ビルド手順を必要とせずに、最新のリアクティブ フロントエンドを構築するための優れたアプローチが提供されます。
...
ただし、`json_encode` を手動で呼び出す代わりに、`Illuminate\Support\Js::from` メソッドを使用することもできます。 `from` メソッドは、PHP の `json_encode` 関数と同じ引数を受け入れます。ただし、結果の JSON が HTML 引用符内に含めるために適切にエスケープされていることを確認します。 `from` メソッドは、指定されたオブジェクトまたは配列を有効な JavaScript オブジェクトに変換する文字列 `JSON.parse` JavaScript ステートメントを返します。
...
`@auth` および `@guest` ディレクティブを使用すると、現在のユーザーが [authenticated](/docs/13.x/authentication) であるかゲストであるかを迅速に判断できます。
...
`@session` ディレクティブは、[session](/docs/13.x/session) 値が存在するかどうかを判断するために使用できます。セッション値が存在する場合、`@session` および `@endsession` ディレクティブ内のテンプレートの内容が評価されます。 `@session` ディレクティブの内容内で、`$value` 変数をエコーし​​てセッション値を表示できます。
...
`@context` ディレクティブは、[context](/docs/13.x/context) 値が存在するかどうかを判断するために使用できます。コンテキスト値が存在する場合、`@context` および `@endcontext` ディレクティブ内のテンプレートの内容が評価されます。 `@context` ディレクティブの内容内で、`$value` 変数をエコーし​​てコンテキスト値を表示できます。
...
親ビューから変数を継承せずにビューを含めたい場合は、`@includeIsolated` ディレクティブを使用できます。組み込まれたビューは、明示的に渡した変数にのみアクセスできます。
...
2 つの別個の Blade テンプレートから重複したコンテンツをプッシュする場合は、コンテンツが 1 回だけレンダリングされるように、`@pushOnce` ディレクティブの 2 番目の引数として一意の識別子を指定する必要があります。
...
`@use` ディレクティブは、インポート パスの前に `function` または `const` 修飾子を付けることで、PHP 関数と定数のインポートもサポートします。
...
クラスのインポートと同様に、関数と定数のエイリアスもサポートされています。
...
グループ化されたインポートは function 修飾子と const 修飾子の両方でもサポートされており、単一のディレクティブで同じ名前空間から複数のシンボルをインポートできます。
...
[Laravel's Vite font optimization](/docs/13.x/vite#working-with-fonts)を使用している場合は、`@fonts` ディレクティブを使って、設定したフォントのプリロードリンクとインラインフォント CSS をアプリケーションのレイアウトにレンダリングできます。
...
`@fonts` ディレクティブは、`vite.config.js` ファイルで設定したすべてのフォントファミリーをレンダリングします。このディレクティブは通常、それらのフォントを使用するコンテンツよりも前の、アプリケーションのルートレイアウトの `<head>` 内に配置します。

<!-- If a page only needs some of your configured fonts, you may pass one or more font aliases to the directive: -->
ページが設定済みフォントの一部だけを必要とする場合は、1つ以上のフォントエイリアスをディレクティブに渡せます。
...
フォントエイリアスは、Vite の設定でフォントを定義する際に `alias` オプションを使って設定します。`@fonts` ディレクティブは `Vite` ファサードが提供する `fonts` メソッドを呼び出しており、このメソッドは直接呼び出すこともできます。
...
ルート `Card` コンポーネントは `Card` ディレクトリ内にネストされているため、`<x-card.card>` を介してコンポーネントをレンダリングする必要があることが予想されるかもしれません。ただし、コンポーネントのファイル名がコンポーネントのディレクトリ名と一致する場合、Laravel はコンポーネントが「ルート」コンポーネントであると自動的に想定し、ディレクトリ名を繰り返さずにコンポーネントをレンダリングできるようにします。
...
Blade コンポーネントを使用すると、クラスの render メソッド内のコンポーネント名、属性、スロットにアクセスすることもできます。ただし、このデータにアクセスするには、コンポーネントの `render` メソッドからクロージャを返す必要があります。
...
コンポーネントの `render` メソッドによって返されるクロージャは、唯一の引数として `$data` 配列を受け取ることもあります。この配列には、コンポーネントに関する情報を提供するいくつかの要素が含まれます。
...
コンポーネントが Laravel の [service container](/docs/13.x/container) からの依存関係を必要とする場合、コンポーネントのデータ属性の前に依存関係をリストすると、それらはコンテナーによって自動的に挿入されます。
...
`only` メソッドは、指定されたキーを持つ属性のみを取得するために使用できます。
...
`except` メソッドは、指定されたキーを持つ属性を除くすべての属性を取得するために使用できます。
...
Artisan 経由で匿名コンポーネントを作成するには、`make:component` コマンドを呼び出すときに `--view` フラグを使用できます。
...
ありがたいことに、Blade では、コンポーネントのディレクトリ名に一致するファイルをコンポーネントのディレクトリ自体内に配置できます。このテンプレートが存在する場合、ディレクトリ内にネストされている場合でも、コンポーネントの「ルート」要素としてレンダリングできます。したがって、上記の例で示した同じ Blade 構文を引き続き使用できます。ただし、ディレクトリ構造は次のように調整します。
...
> [!WARNING]
> `@aware` ディレクティブは、HTML 属性を介して親コンポーネントに明示的に渡されていない親データにはアクセスできません。親コンポーネントに明示的に渡されないデフォルトの `@props` 値には、`@aware` ディレクティブからアクセスできません。
...
`anonymousComponentPath` メソッドは、匿名コンポーネントの場所への「パス」を最初の引数として受け入れ、コンポーネントを配置する必要があるオプションの「名前空間」を 2 番目の引数として受け入れます。通常、このメソッドは、アプリケーションの [service providers](/docs/13.x/providers) の 1 つの `boot` メソッドから呼び出す必要があります。
...
接頭辞が指定されている場合、コンポーネントがレンダリングされるときに、コンポーネントの名前空間をコンポーネント名に接頭辞として付けることによって、その「名前空間」内のコンポーネントがレンダリングされることがあります。
...
アプリケーションで HTML フォームを定義するときは常に、[the CSRF protection](/docs/13.x/csrf) ミドルウェアがリクエストを検証できるように、フォームに非表示の CSRF トークン フィールドを含める必要があります。 `@csrf` Blade ディレクティブを使用してトークン フィールドを生成できます。
...
`@error` ディレクティブを使用すると、特定の属性に [validation error messages](/docs/13.x/validation#quick-displaying-the-validation-errors) が存在するかどうかをすばやく確認できます。 `@error` ディレクティブ内で、`$message` 変数をエコーし​​てエラー メッセージを表示できます。
...
[the name of a specific error bag](/docs/13.x/validation#named-error-bags) を `@error` ディレクティブの 2 番目のパラメーターとして渡して、複数のフォームを含むページの検証エラー メッセージを取得できます。
...
`@hasstack` ディレクティブは、スタックが空かどうかを判断するために使用できます。
...
`@inject` ディレクティブは、Laravel [service container](/docs/13.x/container) からサービスを取得するために使用できます。 `@inject` に渡される最初の引数はサービスが配置される変数の名前であり、2 番目の引数は解決するサービスのクラス名またはインターフェイス名です。
