...
- [Resources](#resources)
    ...
    - [リソースの注釈](#resource-annotations)
    - [条件付きリソースの登録](#conditional-resource-registration)
    - [リソースの応答](#resource-responses)
- [Apps](#apps)
    - [アプリリソースの作成](#creating-app-resources)
    - [ツールからアプリをレンダリングする](#rendering-apps-from-tools)
    - [アプリツールの可視性](#app-tool-visibility)
    - [アプリの構成](#app-configuration)
    - [Boost を使用したアプリの構築](#building-apps-with-boost)
- [Metadata](#metadata)
...
ローカル サーバーは Artisan コマンドとして実行され、[Laravelブースト](/docs/{{version}}/installation#installing-laravel-boost) のようなローカル AI アシスタント統合の構築に最適です。 `local` メソッドを使用してローカル サーバーを登録します。
...
一度登録すると、通常は `mcp:start` Artisan コマンドを自分で手動で実行する必要はありません。代わりに、サーバーを起動するか、[MCP検査官](#mcp-inspector) を使用するように MCP クライアント (AI エージェント) を構成します。
...
ツールは [出力スキーマ](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#output-schema) を定義して応答の構造を指定できます。これにより、解析可能なツール結果を必要とする AI クライアントとの統合が可能になります。 `outputSchema` メソッドを使用して、ツールの出力構造を定義します。
...
ツールは、`structured` メソッドを使用して [構造化されたコンテンツ](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#structured-content) を返すことができます。これにより、JSON エンコードされたテキスト表現との下位互換性を維持しながら、AI クライアントに解析可能なデータが提供されます。
...
[リソーステンプレート](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#resource-templates) を使用すると、サーバーは URI パターンと変数を照合する動的リソースを公開できるようになります。リソースごとに静的 URI を定義する代わりに、テンプレート パターンに基づいて複数の URI を処理する単一のリソースを作成できます。
...
| `#[LastModified]` | 弦        | リソースが最後に更新された時間を示す ISO 8601 タイムスタンプ。           |
...
Laravel MCP は、サポートされているホストのサンドボックス iframe 内でツールがインタラクティブな HTML アプリケーションをレンダリングできるようにするモデル コンテキスト プロトコルの拡張機能である [MCP アプリ](https://modelcontextprotocol.io/extensions/apps/overview) をサポートしています。これにより、プレーン テキストの応答を超えたダッシュボード、フォーム、ビジュアライゼーション、その他の豊富なエクスペリエンスを構築できます。
...
`createMcpApp` グローバルはバンドルされた SDK によって提供され、iframe のサーバーへの接続、ホスト テーマの適用、`callServerTool`、`sendMessage`、`openLink` などのヘルパの公開、およびイベント コールバックを処理します。完全なクライアント側 API については、[MCP アプリの仕様](https://modelcontextprotocol.io/extensions/apps/overview) を参照してください。
...
Laravel MCP には、MCP アプリを構築するための専用の [Boost](/docs/{{version}}/boost) スキル リファレンスが含まれています。 [Boost](/docs/{{version}}/boost) がインストールされている場合、AI コーディング エージェントは `mcp-development` スキルを呼び出し、アプリ リソース、Blade ビュー、およびリンクされたツールをスキャフォールディングするように依頼できます。

完全なクライアント側 API とスキーマの詳細を含む完全なプロトコル リファレンスについては、公式 [MCP アプリのドキュメント](https://modelcontextprotocol.io/extensions/apps/overview) を参照してください。

<a name="metadata"></a>
## メタデータ (Metadata)

Laravel MCP は、特定の MCP クライアントまたは統合で必要な [MCP仕様](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta) で定義されている `_meta` フィールドもサポートしています。メタデータは、ツール、リソース、プロンプトとその応答を含むすべての MCP プリミティブに適用できます。
...
MCP サーバーへのアクセスを認証するには 2 つの方法があります。[Laravel Sanctum](/docs/{{version}}/sanctum) を介した単純なトークン ベースの認証、または `Authorization` HTTP ヘッダーを介して渡されるトークンです。または、[Laravel Passport](/docs/{{version}}/passport) を使用して OAuth 経由で認証することもできます。

<a name="oauth"></a>
### OAuth 2.1

Web ベースの MCP サーバーを保護する最も堅牢な方法は、[Laravel Passport](/docs/{{version}}/passport) を使用した OAuth を使用することです。
...
アプリケーションがまだ Laravel Passport を使用していない場合は、Passport の [インストールおよび展開ガイド](/docs/{{version}}/passport#installation) に従って、Passport をアプリケーションに追加します。次に進む前に、`OAuthenticatable` モデル、新しい認証ガード、およびPassport キーを用意する必要があります。
...
[Sanctum](/docs/{{version}}/sanctum) を使用して MCP サーバーを保護したい場合は、`routes/ai.php` ファイルで Sanctum の認証ミドルウェアをサーバーに追加するだけです。次に、認証が成功するように、MCP クライアントが `Authorization: Bearer <token>` ヘッダーを提供していることを確認します。
