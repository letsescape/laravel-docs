...
> [!NOTE]
> Fortify は、Laravel の認証機能の実装をいち早く開始できるようにすることを目的としたパッケージであることを忘れないでください。 **これを使用する必要はありません。** [authentication](/docs/13.x/authentication)、[password reset](/docs/13.x/passwords)、および [email verification](/docs/13.x/verification) ドキュメントにあるドキュメントに従って、いつでも自由に Laravel の認証サービスと手動で対話できます。
...
**Laravel の認証機能を使用するために Fortify を使用する必要はありません。** [authentication](/docs/13.x/authentication)、[password reset](/docs/13.x/passwords)、および [email verification](/docs/13.x/verification) ドキュメントにあるドキュメントに従って、いつでも自由に Laravel の認証サービスを手動で操作できます。
...
Laravel を初めて使用する場合は、[our application starter kits](/docs/13.x/starter-kits) を検討してみてください。 Laravel のアプリケーション スターター キットは、内部で Fortify を使用して、[Tailwind CSS](https://tailwindcss.com) で構築されたユーザー インターフェイスを含むアプリケーションの認証スキャフォールディングを提供します。これにより、Laravel の認証機能を学習し、慣れることができます。
...
Laravel Fortify をいつ使用するのが適切なのか疑問に思われるかもしれません。まず、Laravel の [application starter kits](/docs/13.x/starter-kits) のいずれかを使用している場合、Laravel のアプリケーション スターター キットはすべて Fortify を使用しており、完全な認証実装がすでに提供されているため、Laravel Fortify をインストールする必要はありません。
...
Fortify を使用する代わりに Laravel の認証サービスと手動で対話することを選択した場合は、[authentication](/docs/13.x/authentication)、[password reset](/docs/13.x/passwords)、および [email verification](/docs/13.x/verification) ドキュメントで入手可能なドキュメントに従って行うことができます。
...
開発者の中には、[Laravel Sanctum](/docs/13.x/sanctum) と Laravel Fortify の違いについて混乱する人もいます。 2 つのパッケージは 2 つの異なるが関連する問題を解決するため、Laravel Fortify と Laravel Sanctum は相互に排他的または競合するパッケージではありません。
...
まず、「ログイン」ビューを返す方法を Fortify に指示する必要があります。 Fortify はヘッドレス認証ライブラリであることを思い出してください。すでに完成している Laravel の認証機能のフロントエンド実装が必要な場合は、[application starter kit](/docs/13.x/starter-kits) を使用する必要があります。
...
リクエストが成功しなかった場合、ユーザーはログイン画面にリダイレクトされ、共有 `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors) を介して検証エラーが表示されます。または、XHR リクエストの場合、検証エラーは 422 HTTP レスポンスで返されます。
...
Laravel Fortify は、呼び出し可能なクラスのパイプラインを通じてログインリクエストを認証します。必要に応じて、ログイン要求がパイプされるクラスのカスタム パイプラインを定義できます。各クラスには、受信 `Illuminate\Http\Request` インスタンスを受け取る `__invoke` メソッドと、[middleware](/docs/13.x/middleware) と同様に、パイプライン内の次のクラスにリクエストを渡すために呼び出される `$next` 変数が必要です。
...
一部のアプリケーションでは、IP アドレスのみによるスロットルなど、認証試行をスロットルするための別のアプローチが必要な場合があります。したがって、Fortify では、`fortify.limiters.login` 構成オプションを介して独自の [rate limiter](/docs/13.x/routing#rate-limiting) を指定できます。もちろん、この構成オプションはアプリケーションの `config/fortify.php` 構成ファイルにあります。

> [!NOTE]
> スロットリング、[two-factor authentication](/docs/13.x/fortify#two-factor-authentication)、および外部 Web アプリケーション ファイアウォール (WAF) を組み合わせて利用すると、正規のアプリケーション ユーザーに最も堅牢な防御が提供されます。
...
この動作の高度なカスタマイズが必要な場合は、`LoginResponse` および `LogoutResponse` コントラクトの実装を Laravel [service container](/docs/13.x/container) にバインドできます。通常、これはアプリケーションの `App\Providers\FortifyServiceProvider` クラスの `register` メソッド内で行う必要があります。
...
リクエストが成功しなかった場合、ユーザーは 2 要素チャレンジ画面にリダイレクトされ、共有 `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors) を介して検証エラーが表示されます。または、XHR リクエストの場合、検証エラーは 422 HTTP レスポンスで返されます。
...
Fortify は、WebAuthn を使用したパスキー認証をサポートしています。パスキーを使用すると、ユーザーは、Face ID、Touch ID、Windows Hello、またはハードウェア セキュリティ キーなどのプラットフォーム認証システムを使用して、パスワードなしで認証できるようになります。
...
開始するには、アプリケーションの `fortify` 構成ファイルで `passkeys` 機能が有効になっていることを確認してください。
...
`confirmPassword` オプションは、パスキーを登録または削除する前に Fortify が [password confirmation](#password-confirmation) を必要とするかどうかを決定します。
...
次に、アプリケーションの `App\Models\User` モデルが `Laravel\Fortify\Contracts\PasskeyUser` を実装し、`Laravel\Fortify\PasskeyAuthenticatable` 特性を使用していることを確認します。
...
Fortify のパスキー構成オプションは、アプリケーションの `config/fortify.php` ファイル内の `passkeys` 構成配列を使用してカスタマイズできます。
...
> [!NOTE]
> Fortify は、`laravel/passkeys` Composer パッケージをラップし、構成します。 Fortify のパスキー機能を使用している場合は、アプリケーションの `config/fortify.php` ファイルを使用してパスキーを構成する必要があります。 `laravel/passkeys` 構成ファイルを公開する必要はありません。そこで定義されている値はすべて Fortify によって上書きされます。
...
`relying_party_id` はアプリケーションのドメインと一致する必要があります。 `allowed_origins` 配列には、パスキーの登録と認証を完了できるブラウザーのオリジンがリストされます。 `user_handle_secret` は、不透明なユーザー ID を導出するために使用され、複数のパスキー登録にわたって同じユーザーが確実に認識されるようにします。 `timeout` オプションは、パスキーの登録と認証操作をアクティブにしておく時間を制御します。
...
Fortify は、パスキーのログイン、確認、登録ルートに専用のパスキー レート リミッターを適用します。必要に応じて、`fortify.limiters.passkeys` 構成オプションと対応する `RateLimiter::for(...)` 定義を使用してカスタマイズできます。
...
ブラウザ側スクリプトを備えた Blade アプリケーションなどのカスタム フロントエンドを構築している場合は、公式 [`@laravel/passkeys`](https://www.npmjs.com/package/@laravel/passkeys) パッケージを使用できます。このパッケージはブラウザの WebAuthn セレモニーを処理し、Fortify のパスキー エンドポイントにリクエストを送信します。
...
アプリケーションがカスタム パスキー エンドポイント URI を使用している場合は、呼び出しごとにルートをオーバーライドできます。
...
このパッケージは、`@laravel/passkeys/react`、`@laravel/passkeys/vue`、および `@laravel/passkeys/svelte` を介して React、Vue、および Svelte ヘルパも提供します。
...
パスキーを使用してユーザーを認証するには、アプリケーションはまず `/passkeys/login/options` エンドポイントに対して GET リクエストを行う必要があります。このエンドポイントは、フロントエンドが `navigator.credentials.get(...)` に渡す必要がある WebAuthn チャレンジ オプションを返します。
...
ブラウザが資格情報を返した後、アプリケーションは資格情報ペイロードを使用して `/passkeys/login` に対して POST リクエストを行う必要があります。ブール値の `remember` フィールドを含めることもできます。
...
認証されたセッションの場合、Fortify は、現在のセッションに対する Laravel のパスワード確認要件を満たすパスキー確認エンドポイントを提供します。
...
パスキーで確認するには、アプリケーションはまず `/passkeys/confirm/options` に対して GET リクエストを行う必要があります。このエンドポイントは、フロントエンドが `navigator.credentials.get(...)` に渡す必要がある WebAuthn チャレンジ オプションを返します。
...
ブラウザが資格情報を返した後、アプリケーションは資格情報ペイロードを使用して `/passkeys/confirm` に対して POST リクエストを行う必要があります。
...
認証されたユーザーのパスキーを登録するには、アプリケーションはまず `/user/passkeys/options` に対して GET リクエストを行う必要があります。このエンドポイントは、フロントエンドが `navigator.credentials.create(...)` に渡す必要がある WebAuthn 作成オプションを返します。
...
ブラウザーが認証情報を返した後、アプリケーションは、`name` フィールドと、`navigator.credentials.create(...)` によって返されたシリアル化された [`PublicKeyCredential`](https://developer.mozilla.org/en-US/docs/Web/API/PublicKeyCredential) オブジェクトを含む `credential` フィールドを使用して、`/user/passkeys` に対して POST リクエストを行う必要があります。
...
パスキーを削除するには、アプリケーションは `/user/passkeys/{passkey}` に対して DELETE リクエストを行う必要があります。
...
アプリケーションの登録機能の実装を開始するには、「登録」ビューを返す方法を Fortify に指示する必要があります。 Fortify はヘッドレス認証ライブラリであることを思い出してください。すでに完成している Laravel の認証機能のフロントエンド実装が必要な場合は、[application starter kit](/docs/13.x/starter-kits) を使用する必要があります。
...
リクエストが成功しなかった場合、ユーザーは登録画面にリダイレクトされ、共有 `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors) を介して検証エラーが表示されます。または、XHR リクエストの場合、検証エラーは 422 HTTP レスポンスで返されます。
...
アプリケーションのパスワードリセット機能の実装を開始するには、「パスワードを忘れた場合」ビューを返す方法を Fortify に指示する必要があります。 Fortify はヘッドレス認証ライブラリであることを思い出してください。すでに完成している Laravel の認証機能のフロントエンド実装が必要な場合は、[application starter kit](/docs/13.x/starter-kits) を使用する必要があります。
...
`$status` セッション変数の値は、アプリケーションの `passwords` [language file](/docs/13.x/localization) 内で定義された変換文字列の 1 つと一致します。この値をカスタマイズしたいが、Laravel の言語ファイルを公開していない場合は、`lang:publish` Artisan コマンドを使用してカスタマイズできます。
...
リクエストが成功しなかった場合、ユーザーはパスワードリセット リンクのリクエスト画面にリダイレクトされ、共有 `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors) 経由で検証エラーを確認できるようになります。または、XHR リクエストの場合、検証エラーは 422 HTTP レスポンスで返されます。
...
リクエストが成功しなかった場合、ユーザーはパスワードのリセット画面にリダイレクトされ、共有 `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors) を介して検証エラーが表示されます。または、XHR リクエストの場合、検証エラーは 422 HTTP レスポンスで返されます。
...
パスワード確認機能の実装を開始するには、アプリケーションの「パスワード確認」ビューを返す方法を Fortify に指示する必要があります。 Fortify はヘッドレス認証ライブラリであることを思い出してください。すでに完成している Laravel の認証機能のフロントエンド実装が必要な場合は、[application starter kit](/docs/13.x/starter-kits) を使用する必要があります。
