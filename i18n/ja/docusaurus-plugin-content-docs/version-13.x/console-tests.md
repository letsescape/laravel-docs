...
HTTP テストの簡素化に加えて、Laravel はアプリケーションの [custom console commands](/docs/13.x/artisan) をテストするためのシンプルな API を提供します。
...
[Laravel Prompts](/docs/13.x/prompts) によって提供される `search` 関数または `multisearch` 関数を利用している場合は、`expectsSearch` アサーションを使用してユーザーの入力、検索結果、および選択を模擬できます。
