...
- [コントローラミドルウェア](#controller-middleware)
    - [ミドルウェアの属性](#middleware-attributes)
...
この例では、`photos` リソースはすべての [標準リソースルート](#actions-handled-by-resource-controllers) を受け取ります。ただし、`thumbnail` リソースは、次のルートを持つシングルトン リソースになります。
