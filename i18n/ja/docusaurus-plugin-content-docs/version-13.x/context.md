...
コンテキストに追加された情報は、リクエスト全体に書き込まれるすべての [log entries](/docs/13.x/logging) にメタデータとして自動的に追加されます。コンテキストをメタデータとして追加すると、個々のログ エントリに渡される情報を、`Context` を介して共有される情報と区別できるようになります。たとえば、次のログ エントリを書き込むとします。
