...
Laravel のセッションは通常、現在認証されているユーザーの状態を維持するために使用されます。したがって、`actingAs` ヘルパ メソッドは、特定のユーザーを現在のユーザーとして認証する簡単な方法を提供します。たとえば、[model factory](/docs/13.x/eloquent-factories) を使用してユーザーを生成および認証できます。
...
リクエストが認証されていないことを確認したい場合は、`actingAsGuest` メソッドを使用できます。
...
あるいは、`dd`、`ddHeaders`、`ddBody`、`ddJson`、および `ddSession` メソッドを使用して、応答に関する情報をダンプしてから実行を停止することもできます。
...
`assertDoesntThrow` メソッドは、特定のクロージャ内のコードが例外をスローしないことをアサートするために使用できます。
...
複数の JSON パスを一度にアサートする必要がある場合は、`assertJsonPaths` メソッドを使用できます。各パスの期待値はクロージャである場合もあります。
...
`assertJsonMissingPaths` メソッドを使用して、応答に複数の JSON パスが欠落していることをアサートできます。
...
JSON コレクション内のすべての項目に対して同じアサーションを行いたい場合は、`each` メソッドを使用できます。
...
一部のビューは、[global error bag provided by Laravel](/docs/13.x/validation#quick-displaying-the-validation-errors) で共有されたエラーに依存する場合があります。エラー バッグにエラー メッセージを追加するには、`withViewErrors` メソッドを使用します。
...
必要に応じて、`blade` メソッドを使用して、生の [Blade](/docs/13.x/blade) 文字列を評価およびレンダリングできます。 `view` メソッドと同様に、`blade` メソッドは `Illuminate\Testing\TestView` のインスタンスを返します。
...
`component` メソッドを使用して、[Blade component](/docs/13.x/blade#components) を評価およびレンダリングできます。 `component` メソッドは、`Illuminate\Testing\TestComponent` のインスタンスを返します。
...
応答にクライアント エラー (>= 400、< 500) HTTP ステータス コードがあることをアサートします。
...
応答に失敗した依存関係 (424) HTTP ステータス コードがあることをアサートします。
...
指定されたヘッダーに指定された部分文字列値が含まれていることをアサートします。
...
応答が前のページにリダイレクトされているかどうかを確認します。
...
応答が前のページにリダイレクトされていて、セッションにエラー メッセージが含まれていないことを確認します。
...
応答が指定された [named route](/docs/13.x/routing#named-routes) へのリダイレクトであることをアサートします。
...
応答が指定された [signed route](/docs/13.x/urls#signed-urls) へのリダイレクトであることをアサートします。
...
セッションの [flashed input array](/docs/13.x/responses#redirecting-with-flashed-session-data) に指定された値があることをアサートします。
...
セッションに特定の [error bag](/docs/13.x/validation#named-error-bags) 内の指定された `$keys` のエラーが含まれていることをアサートします。 `$keys` が連想配列の場合、セッションのエラー バッグ内に各フィールド (キー) に特定のエラー メッセージ (値) が含まれていることをアサートします。
...
フラッシュされた入力配列に指定された入力キーがセッションに欠落していることをアサートします。
...
指定されたフィールドが検証エラーのある唯一のフィールドであることを主張したい場合は、`assertOnlyInvalid` メソッドを使用できます。
