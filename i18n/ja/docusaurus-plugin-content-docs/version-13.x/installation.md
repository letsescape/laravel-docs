...
開発サーバーを起動すると、Web ブラウザ ([http://localhost:8000](http://localhost:8000)) でアプリケーションにアクセスできるようになります。次に、[start taking your next steps into the Laravel ecosystem](#next-steps) の準備が整いました。もちろん、[configure a database](#databases-and-migrations) して、必要なマイグレーションを実行することもできます。
...
Laravel フレームワークのすべての構成ファイルは、`config` ディレクトリに保存されています。各オプションは文書化されているので、ファイルに目を通して、利用可能なオプションをよく理解してください。
