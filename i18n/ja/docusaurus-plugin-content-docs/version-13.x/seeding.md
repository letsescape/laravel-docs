...
> [!NOTE]
> [Mass assignment protection](/docs/13.x/eloquent#mass-assignment) は、データベースのシード処理中に自動的に無効になります。
...
シーダーを生成するには、`make:seeder` [Artisan command](/docs/13.x/artisan) を実行します。フレームワークによって生成されたすべてのシーダーは、`database/seeders` ディレクトリに配置されます。
...
シーダー クラスには、デフォルトでメソッド `run` が 1 つだけ含まれています。このメソッドは、`db:seed` [Artisan command](/docs/13.x/artisan) が実行されるときに呼び出されます。 `run` メソッド内では、必要に応じてデータベースにデータを挿入できます。 [query builder](/docs/13.x/queries) を使用してデータを手動で挿入することも、[Eloquent model factories](/docs/13.x/eloquent-factories) を使用することもできます。
...
> [!NOTE]
> `run` メソッドのシグネチャ内で必要な依存関係をタイプヒントで指定できます。これらは、Laravel [service container](/docs/13.x/container) を通じて自動的に解決されます。
...
もちろん、各モデル シードの属性を手動で指定するのは面倒です。代わりに、[model factories](/docs/13.x/eloquent-factories) を使用すると、大量のデータベース レコードを簡単に生成できます。まず、[model factory documentation](/docs/13.x/eloquent-factories) を確認して、ファクトリを定義する方法を学びます。
