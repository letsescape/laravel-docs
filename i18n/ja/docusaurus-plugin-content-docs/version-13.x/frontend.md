...
Inertia と React / Svelte / Vue を使用してフロントエンドを構築したい場合は、[React、Svelte、または Vue アプリケーション スターター キット](/docs/{{version}}/starter-kits) を活用してアプリケーションの開発を開始できます。これらのスターター キットはすべて、Inertia、React / Svelte / Vue、[Tailwind](https://tailwindcss.com)、[Vite](https://vitejs.dev) を使用してアプリケーションのバックエンドとフロントエンドの認証フローを足場にするため、次の大きなアイデアの構築を開始できます。
