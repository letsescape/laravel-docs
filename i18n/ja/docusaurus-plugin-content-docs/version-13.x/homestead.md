...
> [!WARNING]
> Laravel Homestead は、現在は積極的にメンテナンスされていないレガシー パッケージです。 [Laravel Sail](/docs/13.x/sail) は最新の代替手段として使用できます。
...
Laravel は、単一の `schedule:run` Artisan コマンドを毎分実行するようにスケジュールすることで、[schedule cron jobs](/docs/13.x/scheduling) を実行する便利な方法を提供します。 `schedule:run` コマンドは、`routes/console.php` ファイルに定義されているジョブ スケジュールを調べて、実行するスケジュールされたタスクを決定します。
...
Homestead 内で [Laravel Dusk](/docs/13.x/dusk) テストを実行するには、Homestead 構成で [webdriver feature](#installing-optional-features) を有効にする必要があります。
