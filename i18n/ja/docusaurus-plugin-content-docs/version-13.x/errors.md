...
> [!NOTE]
> 特定の例外の例外レポートをカスタマイズするには、[reportable exceptions](/docs/13.x/errors#renderable-exceptions) を利用することもできます。
...
メッセージがアプリケーションの [logs](/docs/13.x/logging) に書き込まれる場合、メッセージは指定された [log level](/docs/13.x/logging#log-levels) に書き込まれます。これは、記録されるメッセージの重大度または重要性を示します。
