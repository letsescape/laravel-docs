...
テストを実行すると、`phpunit.xml` ファイルで定義された環境変数により、Laravel は自動的に [configuration environment](/docs/13.x/configuration#environment-configuration) を `testing` に設定します。また、Laravel はセッションとキャッシュを `array` ドライバに自動的に構成するため、テスト中にセッションやキャッシュのデータは保持されません。
...
> [!NOTE]
> テスト スタブは、[stub publishing](/docs/13.x/artisan#stub-customization) を使用してカスタマイズできます。
...
テストを実行すると、Laravel はテストメソッドごとにアプリケーションを起動します。設定ファイルをキャッシュしていない場合、テスト開始時にアプリケーションの各設定ファイルを読み込む必要があります。設定を一度構築し、1回の実行ですべてのテストに再利用するには、`Illuminate\Foundation\Testing\WithCachedConfig` トレイトを使用できます。
