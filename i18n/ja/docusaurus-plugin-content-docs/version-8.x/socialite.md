...
> [!TIP]
> 他のプラットフォーム用のアダプターは、コミュニティ主導の [Socialite Providers](https://socialiteproviders.com/) ウェブサイトにリストされています。
...
OAuth プロバイダからユーザーを取得したら、そのユーザーがアプリケーションのデータベースと [authenticate the user](/docs/8.x/authentication#authenticate-a-user-instance) に存在するかどうかを確認できます。ユーザーがアプリケーションのデータベースに存在しない場合は、通常、ユーザーを表す新しいレコードをデータベースに作成します。
...
> [!TIP]
> 特定の OAuth プロバイダから入手できるユーザー情報の詳細については、[retrieving user details](#retrieving-user-details) のドキュメントを参照してください。
...
> [!NOTE]
> ステートレス認証は、認証に OAuth 1.0 を使用する Twitter ドライバでは使用できません。
