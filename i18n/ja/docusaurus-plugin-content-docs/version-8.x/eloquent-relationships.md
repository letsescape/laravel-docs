...
- [関係のクエリ](#querying-relations)
    - [関係メソッドと動的プロパティ](#relationship-methods-vs-dynamic-properties)
...
- [関連モデルの集約](#aggregating-related-models)
    ...
    - [関係へのモーフで関連モデルをカウントする](#counting-related-models-on-morph-to-relationships)
...
> {note} PostgreSQL は UUID 列に対する `MAX` 関数の実行をサポートしていないため、現在、PostgreSQL UUID 列と組み合わせて 1/2 の関係を使用することはできません。

<a name="advanced-has-one-of-many-relationships"></a>
#### アドバンスには多くの関係のうちの 1 つがあります
...
> {note} Eloquent の自動的に維持されるタイムスタンプを利用する中間テーブルには、`created_at` と `updated_at` の両方のタイムスタンプ列が必要です。
...
> {note} Pivot モデルは `SoftDeletes` トレイトを使用できない場合があります。ピボット レコードをソフト デリートする必要がある場合は、ピボット モデルを実際の Eloquent モデルに変換することを検討してください。
...
> {tip} より高度な「多数のうちの 1 つ」の関係を構築することが可能です。詳細については、[多くのドキュメントのうちの 1 つを持っています](#advanced-has-one-of-many-relationships) を参照してください。
...
> {tip} ポリモーフィックな多対多の関係に入る前に、一般的な [多対多の関係](#many-to-many) に関するドキュメントを読むと役に立つ場合があります。
...
> {note} 既存のアプリケーションに「モーフ マップ」を追加する場合、完全修飾クラスがまだ含まれているデータベース内のすべてのモーフ可能な `*_type` 列値をその「マップ」名に変換する必要があります。
...
> {note} 動的リレーションシップを定義するときは、常に明示的なキー名の引数を Eloquent リレーションシップ メソッドに指定してください。
...
> {note} Eloquent は現在、データベース間の関係の存在に関するクエリをサポートしていません。関係は同じデータベース内に存在する必要があります。
...
> {note} この機能を使用する場合は、取得する列のリストに `id` 列と関連する外部キー列を常に含める必要があります。
...
> {note} `limit` および `take` クエリビルダ メソッドは、即時読み込みを制限する場合には使用できません。

<a name="constraining-eager-loading-of-morph-to-relationships"></a>
#### `morphTo` 関係の積極的な読み込みの制限
...
## 関連モデルの挿入と更新 (Inserting & Updating Related Models)
...
> {tip} `create` メソッドを使用する前に、必ず [一括割り当て](/docs/{{version}}/eloquent#mass-assignment) ドキュメントを確認してください。
...
> {note} 親モデルのタイムスタンプは、Eloquent の `save` メソッドを使用して子モデルが更新された場合にのみ更新されます。
