...
`resources` ディレクトリには、[views](/docs/{{version}}/views) と、CSS や JavaScript などの生の未コンパイル アセットが含まれています。このディレクトリには、すべての言語ファイルも格納されます。
...
> {tip} `app` ディレクトリ内のクラスの多くは、Artisan がコマンドを使用して生成できます。使用可能なコマンドを確認するには、ターミナルで `php artisan list make` コマンドを実行します。
