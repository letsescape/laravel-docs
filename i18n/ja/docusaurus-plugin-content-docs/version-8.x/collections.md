...
> [!TIP]
> [Eloquent](/docs/8.x/eloquent) クエリの結果は、常に `Collection` インスタンスとして返されます。
...
通常、コレクション マクロは、[service provider](/docs/8.x/providers) の `boot` メソッドで宣言する必要があります。
...
この方法は、[views](/docs/8.x/views) などのグリッド システムを操作する場合、[Bootstrap](https://getbootstrap.com/docs/4.1/layout/grid/) で特に便利です。たとえば、グリッドに表示したい [Eloquent](/docs/8.x/eloquent) モデルのコレクションがあるとします。
...
> [!TIP]
> [Eloquent Collections](/docs/8.x/eloquent-collections#method-contains) を使用すると、このメソッドの動作が変更されます。
...
> [!TIP]
> [Eloquent Collections](/docs/8.x/eloquent-collections#method-diff) を使用すると、このメソッドの動作が変更されます。
...
> [!TIP]
> [Eloquent Collections](/docs/8.x/eloquent-collections#method-except) を使用すると、このメソッドの動作が変更されます。
...
> [!TIP]
> [Eloquent Collections](/docs/8.x/eloquent-collections#method-intersect) を使用すると、このメソッドの動作が変更されます。
...
> [!NOTE]
> 他のほとんどのコレクション メソッドと同様、`map` は新しいコレクション インスタンスを返します。呼び出されるコレクションは変更されません。元のコレクションを変換する場合は、[`transform`](#method-transform) メソッドを使用します。
...
> [!TIP]
> [Eloquent Collections](/docs/8.x/eloquent-collections#method-only) を使用すると、このメソッドの動作が変更されます。
...
`toArray` メソッドは、コレクションをプレーンな PHP `array` に変換します。コレクションの値が [Eloquent](/docs/8.x/eloquent) モデルの場合、モデルも配列に変換されます。
...
> [!TIP]
> [Eloquent Collections](/docs/8.x/eloquent-collections#method-unique) を使用すると、このメソッドの動作が変更されます。
...
このメソッドの使用法を説明するために、カーソルを使用してデータベースから請求書を送信するアプリケーションを想像してください。 15 分ごとに実行し、最大 14 分間請求書のみを処理する [scheduled task](/docs/8.x/scheduling) を定義できます。
