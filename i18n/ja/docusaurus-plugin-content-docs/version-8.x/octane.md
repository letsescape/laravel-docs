...
> [!NOTE]
> Laravel Octane には [PHP 8.0+](https://php.net/releases/) が必要です。
...
[Laravel Sail](/docs/8.x/sail) を使用してアプリケーションを開発する場合は、次のコマンドを実行して Octane と RoadRunner をインストールする必要があります。
...
> [!NOTE]
> Sail 経由で Octane アプリケーションを提供する前に、Laravel Sail の最新バージョンがあることを確認し、アプリケーションのルート ディレクトリ内で `./vendor/bin/sail build --no-cache` を実行してください。
...
あるいは、Laravel の公式 Docker ベース開発環境である [Laravel Sail](/docs/8.x/sail) を使用して、Swoole ベースの Octane アプリケーションを開発することもできます。 Laravel Sail にはデフォルトで Swoole 拡張機能が含まれています。ただし、アプリケーションの実行を継続するには、Sail で使用される `supervisor.conf` ファイルを調整する必要があります。開始するには、`sail:publish` Artisan コマンドを実行します。
...
この機能を使用する前に、[Node](https://nodejs.org) がローカル開発環境にインストールされていることを確認する必要があります。さらに、[Chokidar](https://github.com/paulmillr/chokidar) ファイル監視ライブラリを project:library 内にインストールする必要があります。
...
漂遊メモリ リークを防ぐために、Octane は、指定された数のリクエストを処理した後、ワーカーを正常に再起動できます。 Octane にこれを行うように指示するには、`--max-requests` オプションを使用できます。
...
> [!NOTE]
> コントローラ メソッドとルート クロージャで `Illuminate\Http\Request` インスタンスをタイプヒントすることは許容されます。
