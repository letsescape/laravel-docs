...
- [応答の作成](#creating-responses)
    ...
    - [Cookieと暗号化](#cookies-and-encryption)
- [Redirects](#redirects)
...
> {tip} ルートまたはコントローラから [Eloquent コレクション](/docs/{{version}}/eloquent-collections) を返すこともできることをご存知ですか?これらは自動的に JSON に変換されます。試してみてください!
...
> {note} ファイルのダウンロードを管理する Symfony HttpFoundation では、ダウンロードされるファイルに ASCII ファイル名が付いている必要があります。
