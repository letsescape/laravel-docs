...
Laravel には、アプリケーションの [cache](/docs/8.x/cache) と組み合わせて、指定された時間枠内のアクションを制限する簡単な方法を提供する、使いやすいレート制限抽象化が含まれています。

> [!TIP]
> 受信 HTTP リクエストのレート制限に興味がある場合は、[rate limiter middleware documentation](/docs/8.x/routing#rate-limiting) を参照してください。
