...
> {tip} 便宜上、[テストの実行](/docs/{{version}}/testing) の場合、CSRF ミドルウェアはすべてのルートに対して自動的に無効になります。
...
> {tip} デフォルトでは、`resources/js/bootstrap.js` ファイルには、`X-XSRF-TOKEN` ヘッダーを自動的に送信する Axios HTTP ライブラリが含まれています。
