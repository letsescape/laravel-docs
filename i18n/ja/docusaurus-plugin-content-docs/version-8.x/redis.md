...
[Redis](https://redis.io) は、オープンソースの高度な Key-Value ストアです。キーには [strings](https://redis.io/topics/data-types#strings)、[hashes](https://redis.io/topics/data-types#hashes)、[lists](https://redis.io/topics/data-types#lists)、[sets](https://redis.io/topics/data-types#sets)、[ソートされたセット](https://redis.io/topics/data-types#sorted-sets) を含めることができるため、データ構造サーバーと呼ばれることがよくあります。
...
> {note} Redis トランザクションを定義する場合、Redis 接続から値を取得することはできません。トランザクションは単一のアトミックな操作として実行され、その操作はクロージャー全体がコマンドの実行を完了するまで実行されないことに注意してください。
...
> {note} Redis スクリプトの詳細については、[Redis のドキュメント](https://redis.io/commands/eval) を参照してください。
