...
アプリケーションが [event discovery](/docs/10.x/events#event-discovery) を利用している場合は、展開プロセス中にアプリケーションのイベントとリスナのマッピングをキャッシュする必要があります。これは、デプロイメント中に `event:cache` Artisan コマンドを呼び出すことで実現できます。
