...
- [デバッグモード](#debug-mode)
- [Forge / Vapor による簡単な導入](#deploying-with-forge-or-vapor)
...
アプリケーションが [イベントの発見](/docs/{{version}}/events#event-discovery) を利用している場合は、展開プロセス中にアプリケーションのイベントとリスナのマッピングをキャッシュする必要があります。これは、デプロイメント中に `event:cache` Artisan コマンドを呼び出すことで実現できます。
...
## Forge / Vapor による簡単な導入 (Easy Deployment With Forge / Vapor)
