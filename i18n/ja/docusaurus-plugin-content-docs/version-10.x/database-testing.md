...
テストする場合、テストを実行する前にデータベースにいくつかのレコードを挿入する必要がある場合があります。このテストデータを作成するときに各列の値を手動で指定する代わりに、Laravel では、[Eloquent models](/docs/10.x/eloquent) を使用して、[model factories](/docs/10.x/eloquent-factories) ごとにデフォルト属性のセットを定義できます。
