...
Laravel を初めて使用する場合は、お気軽に [Laravelブートキャンプ](https://bootcamp.laravel.com) にアクセスしてください。 Laravel ブートキャンプでは、Eloquent を使用して最初の Laravel アプリケーションを構築する手順を説明します。これは、Laravel と Eloquent が提供するすべての機能を紹介する素晴らしい方法です。
...
デフォルトでは、`HasUuids` トレイトはモデルに [「順序付けられた」UUID](/docs/{{version}}/strings#method-str-ordered-uuid) を生成します。これらの UUID は辞書順に並べ替えることができるため、インデックス付きデータベース ストレージにとってより効率的です。
