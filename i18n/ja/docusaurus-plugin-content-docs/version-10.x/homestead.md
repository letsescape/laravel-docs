...
Laravel は、単一の `schedule:run` Artisan コマンドを毎分実行するようにスケジュールすることで、[schedule cron jobs](/docs/10.x/scheduling) を実行する便利な方法を提供します。 `schedule:run` コマンドは、`App\Console\Kernel` クラスで定義されたジョブ スケジュールを調べて、実行するスケジュールされたタスクを決定します。
...
Homestead 内で [Laravel Dusk](/docs/10.x/dusk) テストを実行するには、Homestead 構成で [`webdriver` feature](#installing-optional-features) を有効にする必要があります。
